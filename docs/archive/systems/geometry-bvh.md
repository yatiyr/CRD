> **Dated systems reference.** Current order/status: [ROADMAP](../../ROADMAP.md). Read [AGENTS](../../../AGENTS.md) for current rules.
...
| v1i-c+ | **v1i-c debt payment (same-day).** Three deferred items paid: (1) caller-owned `DynamicBvhPairScratch{walk, cross}` POD + `find_overlapping_pairs(Fn&&, scratch)` overload — eylem v1c broadphase no longer pays per-tick alloc churn; (2) `Vec4f ray_vs_4_aabb_inflated` SIMD kernel in `bvh4_simd.hpp/cpp` + `bvh4_inflated_raycast` switched to it — ~2-3× per-node throughput on AVX2; (3) AABB-equivalence shift-invariance assertion in `test_validation.cpp` decouples the test from builder permutation. 2000-trial lane-by-lane test for the new SIMD kernel; 5-iter mutation-loop test for scratch reuse. Zero new debt. | ✅ 2026-05-13 |
| v1i-c | Broadphase pairs + validation discipline (ADR-0076 §15). New `DynamicBvhPair{u32 a; u32 b}` POD (a < b invariant). New member template `DynamicBvh::find_overlapping_pairs(Fn&& on_pair) const` — Catto GDC 2019 dual-descent self-overlap (outer DFS over interior nodes + cross-walk on (child1, child2) at each interior node; emits `on_pair(min, max)` for leaf-pair AABB overlaps; `O(n + \|pairs\|)` vs `O(n²)` brute force). `Array<DynamicBvhPair>&` append form + facade overloads `crd::geometry::find_overlapping_pairs(...)`. Work stacks use `crd::containers::Array<...>` allocated from the tree's own allocator (unbounded cross-stack growth safe on pathological dense corpora; validated by n=10000 soak). Static-tree variants (BvhTree/Bvh4Tree) deliberately NOT shipped — static leaves don't carry user_data. Companion: `tests/geometry-primitives/include/test_corpus.hpp` (shared header — both test binaries) with degenerate-AABB/triangle/sphere/ray generators + large-coordinate `shift()` helpers + `ulp_tolerance_for(magnitude)` for per-origin-magnitude ULP slack. Drive-by: fixed `crd-no-non-ascii-test-names` guard (silently broken since v0e — both param-default scope and regex range syntax). | ✅ 2026-05-13 |
| v1i-b | Shapecast (closed-form TOI) — `bvh_shapecast_sphere` / `bvh_shapecast_box` over `BvhTree` and `Bvh4Tree`; `cast_ray` / `cast_sphere` / `cast_box` facade overloads in `crd/geometry/queries.hpp` against primitive shapes (AABB / OBB / Sphere / Plane) AND BVH backends. Sphere-vs-AABB is **conservative** (square corners — GJK-cast in `-convex` v2 is the exact-corners answer); sphere-vs-sphere + sphere-vs-plane + box-vs-AABB are exact. **Argument convention pinned (propagates to v2 GJK-cast and v6 CCD):** primitive `(moving, dir, tmax, target)`; BVH `(tree, prims, moving, dir, tmax)`. `Vec4f` ray-vs-4-inflated-AABB SIMD kernel for `bvh4_shapecast_*` deferred (~40 LOC follow-up; mirror of v1g pattern). | ✅ 2026-05-13 |
| v1i-a | Unified query facade (ADR-0076 §15, §16 pin #2). New `crd/geometry/result_types.hpp` — templated `RayHit<P>{t, payload}` / `ClosestPointResult<P>{point, distance_squared, payload}`. New `crd/geometry/queries.hpp` — `raycast`/`overlap`/`closest_point` as compile-time-overload-polymorphic free functions over `{BvhTree, Bvh4Tree, DynamicBvh}` (inline forwards, zero overhead); primitive overloads flow through transitively from `closest_point.hpp` / `intersect.hpp` / `signed_distance.hpp`. `BvhRayHit` / `BvhClosestPoint` are now `using` aliases of `RayHit<u32>` / `ClosestPointResult<u32>` — field rename `.prim_index` → `.payload`, field-order swap to `{t, payload}` / `{point, distance_squared, payload}`. New `bvh4_closest_point` (free function, ≤4-child near-first insertion-sort) and `DynamicBvh::closest_point` (member; broadphase by fat AABB; payload = `user_data`). | ✅ 2026-05-13 |
...
- `optional<BvhRayHit> bvh_raycast(const BvhTree&, ConstSpan<AABB3<f32>> prims, const Ray3<f32>&, f32 tmax = ∞)`
  ...
  `BvhRayHit = RayHit<u32>` (= `{ f32 t; u32 payload; }`; payload is the prim index). After v1i-a the BVH-side result types are aliases of the templated `crd::geometry::RayHit<P>` / `ClosestPointResult<P>` (ADR-0076 §16 pin #2; `crd/geometry/result_types.hpp`). Uses the v0f precomputed Williams/Ize
...
- `bvh_closest_point(const BvhTree&, ConstSpan<AABB3<f32>> prims, const Vec3<f32>& query, f32 max_dist = ∞) → optional<BvhClosestPoint>`
  ...
  only primitives within `max_dist`. `BvhClosestPoint = ClosestPointResult<u32>`
  (= `{ Vec3<f32> point; f32 distance_squared; u32 payload; }`; payload is the
  prim index — same templated-substrate / alias pattern as `BvhRayHit`). Branch-and-bound: the per-node AABB distance
