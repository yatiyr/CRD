...
| v11 | **Transform-aware shape helpers + typed boundary** (ADR-0076 §28, D217-D233) — `transform.hpp` (14 3D + 7 2D entry points: `transform_aabb` 8-corner method, `transform_obb` three-tier cascade (rigid-exact → uniform-scale-exact → conservative-via-axis-aligned), `transform_sphere`/`capsule3`/`cylinder3` (max-axial-scale for non-uniform / uniform-scale-exact for the common case), `transform_triangle3`/`tetrahedron`/`segment3`/`line3`, `transform_plane` via inverse-transpose-3 with `CRD_ASSERT` on singular matrices, `transform_ray3` (direction NOT renormalized — preserves t-scale), `transform_ray3_to_local` inverse-direction pattern with rigid+uniform-scale precondition, `transform_frustum` with batched inverse-transpose reused across 6 planes, 2D peers: `transform_aabb2`/`obb2`/`circle`/`capsule2`/`segment2`/`ray2`/`triangle2`), **`TransformedShape<Shape>` composition wrapper** with `shape_scalar<Shape>::type` trait-based scalar deduction (21 specialisations) — catches wrong-scalar composition at compile time. **`transform_typed.hpp`** mirrors the entire surface with `*_typed` wrappers (strip-compute-retag pattern; Mat4/Mat3 stay raw, shape input+output are typed). 5 advisor-pinned discriminators in test corpus: identity bit-exact, 45°-rotation diagonal grows by sqrt(2), plane normal stays perpendicular, ray-to-local round-trip, negative-determinant volume preservation. 26 cases / 95 assertions. | ✅ 2026-05-19 |
