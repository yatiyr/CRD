...
> these blocks had accumulated to ~2,000 lines. They are moved here **verbatim, unedited, in their original
> order** — nothing was rewritten.
>
> **How to read it.** Every block below is a **historical snapshot**: it was true when written (each block is
> dated) and uses present-tense "current focus" / "NEXT" language relative to *its own date*, not today.
> Do not read any block as current state — for that, see `context.md`.
>
> **Coverage note.** Some blocks duplicate detail that also lives in dated session logs under
> `docs/sessions/`; others (notably 2026-07-06…07-16 — the GPU FFT/sort/NRC crush campaigns, the B14/B15/B16
> gold-standard CKIR passes — and the 2026-08-01/02 REN-40 closes) are the **only narrative record** of those
> days, which is why this file preserves the whole range rather than deleting it. Benchmark numbers referenced
> here live canonically in `docs/bench/`.
