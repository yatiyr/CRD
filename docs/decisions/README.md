...
| 0098  | **crd-hesap-gpu — the GPU compute cluster: the CPU hesap suite (v5–v16) mirrored onto backend-neutral compute, the determinism moat's hardest test (T1/T2/T3 + computation certificates), and the reserved cross-vendor kernel seam (Phase 3.1.6 v17).** ONE cluster `crd-hesap-gpu` (sub-areas gemm/reduce/fft/sparse/la/nn/autodiff/determinism) rides `crd-rhi-compute` (ADR-0080) + `crd-shader` (GLSL→SPIR-V) + ADR-0085 staging/ring allocators + the v14 tensor surface. **API-AGNOSTIC BY CONSTRUCTION** — hesap-gpu never touches Vulkan (link-isolation smoke); Vulkan-first, the **Slang kernel-language track + `crd-rhi-cuda` kernel-ABI SEAMED not built** (drop-in later). ⭐⭐ **NO float atomics anywhere — fixed reduction trees the standing rule** (the GPU expression of the CPU moat). ⭐⭐ THE DETERMINISM TIERS (the research crown): T1 run-to-run same-device (fixed trees) · T2 cross-ARCHITECTURE reproducible reductions (binned/RFA, vs CCCL) · T3 cross-VENDOR bit-exact IEEE-only (SPIR-V NoContraction + `VK_KHR_shader_float_controls` audit) **+ crd::math deterministic transcendentals PORTED TO GLSL** ⇒ cross-vendor deterministic transcendentals on GPU (publishable) · **computation CERTIFICATES** (hash-chained execution transcripts = verifiable-compute/DO-178C evidence). Gold standards portable-first, honest: vkFFT · CLBlast · MAGMA · ncnn + llama.cpp-Vulkan/ggml · merge-SpMV · `vk_cooperative_matrix_perf` methodology; vendor cuBLAS/cuSPARSE/CUTLASS/CUB-RFA ONLY if CUDA present (kickoff: RTX 4070 Ti SUPER but CUDA absent ⇒ portable peers). Portable-vs-vendor gap named up front (ADR-0082-class honesty); crush axes = portable-peer parity + the determinism/certification column no GPU library carries. DoD §6 every slice: ValidationCapture 0 · bit/ulp GPU-vs-CPU oracle · gpu_determinism_check ×3 · CRD_PERF_BUDGET_LE. Substrate proven via `geometry-bvh-gpu`. Consumes ADR-0080 + ADR-0085 + ADR-0096; sibling to ADR-0097. | arch, hesap, gpu, vulkan, rhi-compute, determinism, reproducibility, certificates, portability, module-edges, substrate | Proposed |
