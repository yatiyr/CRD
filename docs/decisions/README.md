...
| 0078  | `crd-units` substrate (dimensional types + 6-layer conversion system); §2 v0b adoption A; §3 v0c adoption B; §4 v0d adoption C + Phase 3.1.7.5 CLOSE 2026-05-15 (Vec reductions widened without DimRoot, geometry-primitives struct + boundary-wrapper layer, renderer raw-Mat4f contract, Layer-6 UnitPreferences with 11 discipline presets, ImGui inspector, Phase close marker) | arch, substrate, units, dimensional-analysis, type-safety, determinism, physics, eylem, geometry, format-parse, ui | Accepted  |
