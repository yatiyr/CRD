...
| 0076  | `crd-geometry` substrate (BVH + GJK/EPA + mesh queries + polygon ops + Delaunay); §19 amendment 2026-05-16 — `-mesh` v4 cluster CLOSED (5 slices: closest_point + raycast(Woop) + raycast_simd(MT) + winding(Jacobson) + validate); 5th of 11 sub-modules COMPLETE | arch, substrate, computational-geometry, bvh, gjk-epa, mesh-processing, determinism | Accepted  |
