...
| 0076  | `crd-geometry` substrate (BVH + GJK/EPA + mesh queries + polygon ops + Delaunay); §19 amendment 2026-05-16 — `-mesh` v4 cluster CLOSED; §20 amendment 2026-05-16 — `-spatial` v5 cluster CLOSED (8 slices: KdTree + LooseOctree + R*-tree + SpatialHash + UniformGrid + thread-safety pass + scene-index bringup + unified-facade extension); §21 amendment 2026-05-16 — `-polygon` v6 cluster CLOSED (6 slices: substrate + ear-clip-w-holes + CDT (Bowyer-Watson + Domiter-Zalik carve-and-retriangulate) + Vatti polygon Boolean + Bentley-Ottmann sweep + close); **7 of 11 sub-modules COMPLETE** | arch, substrate, computational-geometry, bvh, gjk-epa, mesh-processing, spatial-acceleration, polygon-ops, cdt, determinism | Accepted  |
