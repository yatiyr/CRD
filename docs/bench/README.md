...
- **2026-07-29-ren40-million-instance-baseline.md** — REN-40 START-LINE scaling curve (Cerid-internal, not a peer crush): 10k / 100k / **1M** instances + 24 skinned, 4-cascade CSM, Vulkan, 4070 Ti SUPER. **1M = 337 ms CPU / 90 ms GPU (~3 fps), CPU-BOUND** — extract 171 ms (the O(entities) scene walk), CPU cull ~130 ms (frustum over 1M AABBs × camera + 4 cascades), uploads 32 ms; 256k instances drawn at FULL DETAIL. 10k = 11.8 / 5.2 ms. Needs ~20× CPU and ~6× GPU to reach 16.6 ms. The wall is per-instance-per-frame CPU work + no LOD, NOT draw submission. ── **REN-40-A APPENDED (2026-07-30): the device-side cull.** CPU render 281 → **121 ms** on Vulkan (2.33×) and 279 → **117 ms** on DX12 (2.38×) at 1M, frame **BIT-IDENTICAL** to the CPU-cull arm on both (0/921600 pixels), every per-view survivor count asserted EQUAL. GPU unchanged ~86 ms (that wall is LOD). Remaining CPU term: the extract walk, 77–98 ms. ⛔ Its harness needs `--fixed-dt` — a wall-clock camera makes an A/B measure the pose.
- **2026-07-30-ren40b-incremental-extract.md** — REN-40-B: the extract walk driven by a CHUNK INDEX instead of an O(entities) walk. `sync` at 1M **80.2 → 1.11 ms on Vulkan** and **96.4 → 19.9 ms on DX12** (extract itself **171.4 → 0.9 ms**); the device-cull CPU frame goes 197.6 → **119.1 ms**. A steady frame walks 9,092 chunks, re-extracts **11**, touches **1,203 of 1,000,000** entities. ⭐ Gated by COUNTERS, not timings — and the gate is PROVEN to fail on the old walk (801 KB of signature hashing per static frame at 20k instances vs 11.6 KB). ⚠ Open: DX12's remaining 19 ms of `sync` is its UPLOAD path (18.3 ms vs Vulkan's 0.11), not its extract. GPU ~87 ms unchanged — that wall is LOD (40-C). ⛔ `--gpu-cull` / `--lod` now EXIT when their asset does not install: the first run of this board reported `gpu 0.344 ms` at 1M, which was an empty canvas.
- **2026-07-24-ren1-frame-graph-batching.md** — REN-1 frame graph: the one-submission batching vs the synchronous per-draw substrate (Cerid-internal before/after, CPU submit-path wall-clock, win-debug). Collapsing N `vkQueueSubmit`/fence-waits to one scales with draw count: **Vulkan 2.5× (1 draw) → 6.14× (64 draws)**, **DX12 1.22× → 38.76×** (heavier D3D12 per-submit cost ⇒ larger win; frame time near-flat in N). Sandbox ~58 → 65.2 fps. Both backends correctness-gated (Vulkan 33 asserts, DX12 35 incl. the per-draw descriptor-ring proof).
