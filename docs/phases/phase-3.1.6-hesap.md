...
| ↳ **v10-c** ✅ (Bluestein DONE 2026-06-20) | **Bluestein (chirp-z) → ANY size O(n log n).** SHIPPED `bluestein.hpp` `BluesteinPlan<T>`: arbitrary n (prime/composite/non-pow-2) via the chirp-z identity n·k=(n²+k²−(k−n)²)/2 ⇒ linear convolution over a single pow-2 engine `FftPlan<T>` of M=2^⌈log2(2n−1)⌉; FFT_M(b) precomputed/shared (deterministic, read-only); inverse via the forward-trick conj(FFT(conj·))/n; the engine's unnormalized inverse folded as 1/M. GATE = brute-force O(n²) DFT (NOT round-trip) + round-trip, **machine-eps (~1e-15) for n∈{1,2,3,5,7,17,100,360,1009,1031,4000,10000} incl. primes**; integrated into `test_fft.cpp` (suite **160/23**, GREEN). **Rader (prime (p−1)-convolution) is a deferred optimization** — Bluestein already covers all sizes correctly; Rader only trims the prime constant factor. PocketFFT/FFTW prime shootout = a v10-z bench row. | ~400 | ~14 | — |
...
| ↳ **v10-e** ✅ (N-D DONE 2026-06-20) | **Multi-dim 2D/3D/N-D FFT.** SHIPPED `nd_fft.hpp` `NdFftPlan<T>`: arbitrary-rank row-major N-D FFT by row-column decomposition over the 1D engine — pow-2 axes use `FftPlan<T>`, non-pow-2 axes use `BluesteinPlan<T>` (v10-c) ⇒ ANY shape. Only the FORWARD DFT is applied per axis (both engines unnormalized) and the inverse uses the N-D forward-trick conj(FFT(conj·))/∏dᵢ (so the engines' inverse normalizations never clash). GATE = a direct full N-D DFT (O(total²), independent of the row-column structure) + round-trip, **machine-eps (~1e-16) for 2D/3D/4D incl. non-pow-2 axes** {4×4, 8×6, 16×3, 4×4×4, 8×4×2, 6×5×2, 2×2×2×2}; integrated into `test_fft.cpp` (suite **172/24**, GREEN). Result is thread-count-independent by construction (each 1D line is an independent transform). **Follow-ons (deferred): `crd-jobs` parallelism over the independent lines (needs per-worker plans — FftPlan scratch is per-plan) + the standalone batched-1D API + the 3D real-to-complex shootout.** | ~450 | ~16 | — |
...
| **v11** | **DSP — the MAXIMAL subject** (ADR-0093; **design rationale + honest-gate + benchmark policy below, § "v11 — DSP … DETAILED PLAN"** — the sub-slices live ONLY in this master table, the `↳` rows below). 3 isolated modules + a `crd-units` add: **`crd-hesap-dsp`** (core+adaptive, v11-a…t) + **`crd-hesap-wavelet`** (v11w-a…e) + **`crd-hesap-comms`** (v11c-a…g) + `crd-units` `Decibel`/`NormalizedFrequency`. Gold standards: scipy.signal + MATLAB-SP-Toolbox + Intel-IPP + liquid-dsp + CMSIS-DSP + PyWavelets. ⭐ Honest gate: DESIGN = spec-compliance · APPLICATION = bit-exact + {1..16} MOAT. | ~12000 | ~425 | multi-session |
...
| ↳ **v11-g** ✅ | IIR digital: lp2hp/bp/bs freq-transforms + order-est (`*ord`) + `iirdesign`/`iirfilter` + notch/peak/comb. Faithful scipy ports incl. the `fminbound` Brent port for bandpass order-est. **Gate: exact N + Wn + spec-compliance + response-equality vs scipy.** | ~430 | — | ✅ |
| ↳ **v11-h** ✅ | **AUDIO biquads** (DAW): RBJ cookbook (LPF/HPF/BPF/notch/APF/peaking/low-/high-shelf) + cascade EQ chain. Raw f64 (kernel layer; typed wrapping is a consumer's job). **Gate: coeffs vs independent cookbook transcription 1e-12 + spec properties (DC/Nyq/centre gains).** | ~180 | — | ✅ |
...
| ↳ **v11-k** ✅ | MULTIRATE: `upfirdn` (reversed polyphase bank) + `resample_poly` + `decimate`. **PERF: resample_poly 1.17× scipy / 2.18× MATLAB / beats liquid (f64 vs f32).** (Farrow/CIC/half-band follow-on.) | ~400 | — | ✅ |
| ↳ **v11-l** ✅ | HILBERT: analytic signal + envelope + inst freq/phase + plan-cached `HilbertTransformer`. **PERF: 1.45× scipy / 1.81× MATLAB.** | ~150 | — | ✅ |
...
| ↳ **v11-n** ✅ | **MULTITAPER** (Thomson, `multitaper.hpp`): K DPSS tapers (eig_sym) + averaged eigenspectra. Gate: tone peak + variance-reduction vs single periodogram. (Adaptive weighting + efficient K-taper DPSS = follow-on; taper-setup is eig_sym-bound.) | ~120 | — | ✅ |
| ↳ **v11-o** ✅ | PARAMETRIC AR (`ar.hpp`): `aryule` (Levinson) + `arburg` + `ar_psd` + AIC/MDL. Gate: known-AR(2) recovery + |k|<1 + order selection. **PERF: arburg 4.55× MATLAB.** | ~200 | — | ✅ |
| ↳ **v11-p** ✅ | **SUBSPACE** (`subspace.hpp`): `music_spectrum` + `root_music` (eig_sym noise subspace + poly roots). ⭐ Gate: resolve Δf=0.006 tones **below the FFT bin 1/N** (super-resolution). (ESPRIT = follow-on.) | ~200 | — | ✅ |
| ↳ **v11-q** ✅ | TRANSFORMS (`transforms.hpp`): `goertzel` + `czt` (zoom/chirp-z) + `rceps`/`cceps` + `fwht`. Gate: Goertzel==DFT bin · czt==DFT + zoom · rceps echo · fwht self-inverse. **PERF: czt 3.59× MATLAB (0.49× scipy — M=N shortcut, CztPlan follow-on).** | ~250 | — | ✅ |
| ↳ **v11-r** ✅ | WAVEFORMS: chirp (4 methods) + sawtooth/square + gausspulse + sweep_poly + unit_impulse, vs scipy 1e-10. (MLS/Gold/Kasami follow-on.) | ~300 | — | ✅ |
| ↳ **v11-s** ✅ | DETECTION + MEASUREMENTS (`measurements.hpp`): `find_peaks`/`peak_prominences`/`peak_widths`/`argrelextrema` (exact vs scipy) + `detrend` + `rms`/`crest_factor` + `thd`/`snr`/`sinad`/`sfdr`/`enob` + `cspline1d`. **PERF: detrend 13.3× scipy · thd/snr ~6× MATLAB · find_peaks parity.** | ~400 | — | ✅ |
| ↳ **v11-t** ✅ | ADAPTIVE (`adaptive.hpp`): `LmsFilter`/`NlmsFilter`/`SignLmsFilter`/`RlsFilter` (alloc-free stateful, run-twice determinism moat) + `wiener_hopf`. Gate: known-plant recovery + Wiener-Hopf normal eqns. **PERF: LMS 22.6ms vs liquid eqlms 1.87× (f64 vs f32) / MATLAB dsp.LMSFilter 1.18×.** (Affine-projection follow-on.) | ~350 | — | ✅ |
| ↳ **v11w-a** ✅ | WAVELET families + filter banks: Haar/Daubechies(1-20)/Symlets(2-20)/Coiflets(1-5)/(reverse-)biorthogonal+dmey (76 wavelets, coeffs GENERATED from pywt → machine-precision) + QMF/orthogonal_filter_bank. Gate: lookup vs pywt 1e-15 + QMF reproduces stored bank + orthonormality. | ~280 | — | ✅ |
| ↳ **v11w-b** ✅ | DWT/IDWT (1-D) + **all 9 pywt boundary modes** (zero/constant/symmetric/reflect/periodic/periodization/smooth/antisymmetric/antireflect) + multilevel `wavedec`/`waverec`. Gate: per-mode coeffs vs pywt 1e-11 (BIT-MATCH) + perfect reconstruction + run-twice moat. **PERF: wavedec db4 1.14-1.21× pywt-C-core.** | ~370 | — | ✅ |
| ↳ **v11w-c** ✅ | SWT (à trous, undecimated) + iSWT (pywt phase-averaging) + WaveletPacket (full tree, node-by-path) + best-basis (Shannon-entropy). Gate: per-level vs pywt + WP nodes vs pywt + PR + best-basis optimality. **PERF: swt 1.18-1.19× pywt.** | ~430 | — | ✅ |
| ↳ **v11w-d** ✅ | CWT **full family** (mexh/morl/cmor/gaus1-8/cgau1-8/shan/fbsp/paul, closed-form ψ = pywt; paul analytic) + FFT convolution (shared data spectrum) + **generic central_frequency** (FFT-peak) + ridge. **⭐ MULTI-THREADED batched over scales ({1,4,16} bit-identical moat).** Gate: coeffs vs pywt.cwt 1e-8 (precision-12) + freqs + tone→fc/f. **PERF: CWT morl 1.17× / cmor 2.94× pywt.** | ~640 | — | ✅ |
| ↳ **v11w-e** ✅ | 2-D DWT (dwt2/idwt2/wavedec2/waverec2, separable, **MULTI-THREADED rows+cols, {1,4,16} bit-identical**) + denoising (soft/hard/garrote + **VisuShrink/BayesShrink/SureShrink** + MAD-σ) + **MODWT/imodwt** (Percival-Walden). Gate: subbands+threshold vs pywt + PR + denoise SNR + MODWT energy partition. **PERF: dwt2 1024² 1.49× pywt.** | ~700 | — | ✅ |
| ↳ **v11c-a** ✅ | COMMS modulation: Gray-coded PSK/QAM/PAM (unit-energy) + FSK + soft LLR + bit packing. Gate: Gray property + noise-free round-trip + unit energy + **BER vs theoretical AWGN** (BPSK/QPSK exact, QAM/PAM ≤1.6× approx) + liquid-dsp peer. | ~340 | — | ✅ |
| ↳ **v11c-b** ✅ | pulse shaping: RRC/RC (via dsp) + Gaussian (GFSK) + pulse_shape (upsample+FIR) + matched filter + eye_segments + peak_distortion. Gate: **RRC-TX⊛RRC-RX Nyquist zero-ISI** (ISI→0 as span grows) + Gaussian props + matched-filter SNR. | ~250 | — | ✅ |
| ↳ **v11c-c** ✅ | timing sync: Gardner + Mueller-Müller TEDs + interpolating `SymbolSync` (cubic + PI loop). Gate: Gardner S-curve + the loop LOCKS onto a fractional delay (BER 0). | ~250 | — | ✅ |
| ↳ **v11c-d** ✅ | carrier recovery: `CostasLoop` (decision-directed) + `Pll` + M-th-power AFC + loop-filter design. Gate: removes a phase+freq offset (BER 0); AFC estimate matches. | ~250 | — | ✅ |
| ↳ **v11c-e** ✅ | equalizers: LMS-DD + **CMA (blind)** + DFE + **MLSE (Viterbi)**. Gate: open a multipath channel (BER→0); MLSE == brute-force ML. **PERF: eqlms 2.5× liquid.** | ~360 | — | ✅ |
...
| ↳ **v12-l** ✅ (DONE 2026-06-27) | **Distributions — autodiff-ready log-densities.** analytic ∂logp/∂x and ∂logp/∂θ for every distribution (25+12+8 + MVN/MVt; exp-family suff-stats O(1), StudentT near-parity 1.14× vs JAX XLA). Gate: FD grad-check 348 asrt. | ~600 | ~25 | — |
| ↳ **v12-m** ✅ (DONE 2026-06-27) | **Inference — descriptive + estimators.** moments (+robust: MAD, trimmed) · quantiles (9 R types + Harrell-Davis beta-weighted) · ECDF · histogram bin rules (FD/Scott/Sturges) · weighted mean/var · cov/corr matrices. Gate: scipy/numpy/MATLAB <1e-9. 82 asrt. | ~500 | ~20 | — |
| ↳ **v12-n** ✅ (DONE 2026-06-28) | **Inference — the full test suite.** 121 asrt vs scipy/statsmodels, crushes scipy/R/MATLAB 37–1142×: t/z/Welch/ANOVA(1-way·2-way·repeated)/Bartlett/Levene · MannWhitney/Wilcoxon/KW/Friedman/sign/Mood · KS/AD/CvM/Shapiro/JB/D'Agostino/Lilliefors · χ²/Fisher/McNemar/G · Pearson/Spearman/Kendall/dCor · Tukey/Games-Howell/Scheffé/Dunnett/Holm/BH/Bonferroni · Cohen's d/η²/Cramér's V. (honest scope: Mood no-tie variance · Dunnett balanced · CvM asymptotic p.) | ~1100 | 121 | — |
| ↳ **v12-o** ✅ (DONE 2026-06-28) | **Inference — resampling.** bootstrap (basic/percentile/**BCa**/studentized) · block · jackknife (+delete-d) · permutation · CV · **parallel over crd-jobs (bit-identical to serial = the determinism moat)**. 52 asrt (deterministic core bit-for-bit vs scipy/statsmodels; RNG CI MC-gated). ⭐ **per-thread AND parallel crush: serial 2.08×scipy/21.4×R/13.8×MATLAB, parallel 5.05×/51.9×/33.4×** (Philox-u32 + Lemire turned a 0.74× serial loss into a per-thread win). | ~700 | 52 | — |
| ↳ **v12-p** ✅ (DONE 2026-06-28) | **KDE + robust + streaming.** KDE Gaussian/Epanechnikov (+Scott/Silverman/rot/CV bw) · robust (Huber/Tukey-M + proposal-2, Theil-Sen, Hodges-Lehmann) · cov shrinkage (Ledoit-Wolf/OAS) + exact MCD · online (Welford/P²/t-digest/HLL/count-min). 49 asrt: deterministic bit-for-bit vs scipy/statsmodels/sklearn/numpy, sketches within-accuracy. ⭐ CRUSH: kde 5872×scipy · huber 503×sm · oas 437×skl · LW 257×. | ~1100 | 49 | — |
| ↳ **v12-q** ✅ (DONE 2026-06-28) | **MCMC/Bayesian** (uses v12-l). Metropolis/adaptive-Haario/Gibbs/**HMC**/**NUTS**(+dual-averaging = full Stan)/slice/SMC + diagnostics (R-hat-rank/ESS-Geyer/autocorr/Geweke). 36 asrt: diagnostics bit-for-bit vs ArviZ, samplers recover targets + determinism moat + bit-exact leapfrog. ⭐ **NUTS 104× ess/s vs PyMC** (907K vs 8.7K). | ~1100 | 36 | — |
| ↳ **v12-r** ✅ (DONE 2026-06-28) | **regression/GLM/multivariate.** OLS/WLS/GLS · ridge/lasso/elastic-net · GLM-IRLS (logistic/Poisson/gamma) · robust-Huber/quantile/**RANSAC** · PCA/LDA/QDA/**factor-analysis** (rides the shipped dense lstsq/pinv/eig_sym, SANITY 8). 58 asrt gated vs statsmodels/sklearn. ⭐ **CRUSH vs sklearn: Ridge 47×/PCA 23×/OLS 17×/Lasso 17×/GLM 2.7×.** | ~1000 | 58 | — |
| ↳ **v12-z** ✅ (DONE 2026-06-28) | **CLOSE.** CLI `hesap.{stats,special}.*` (describe/ttest + gamma/erf) + system docs (`hesap-special.md` created, `hesap-stats.md` rewritten) + **v11 spectral CIs back-wired** (multitaper χ² vs scipy chi2.ppf + AR Berk-normal analytic-gated) + ADR-0094. 17 asrt (7 CI + 10 CLI). | ~400 | 17 | — |
| **v13 — Numerical-Analysis + Motion cluster** ⏳ ACTIVE | **4 modules · DETAILED PLAN `docs/phases/phase-3.1.6-v13.md` · ADR-0095.** Certification-grade classical numerics + mission-critical motion (satellites/drones/robots/self-driving/games). **3 moat pillars:** determinism-by-construction · allocation-free bounded-recursion streaming · error-tier-exposing — the DO-178C/ISO-26262-ASIL-D moat GSL (mallocs) / Boost (throws) structurally lose. SANITY-8 reuse: Gauss nodes + eig_sym + hermite_eval + dense solvers + FFT/DCT + Fresnel + quaternion + Sibson-NNI + opt-QP. ★ = best-in-class pick. | ~13K | ~595 | ~6-10 wk |
| ↳ **v13-a** ✅ (DONE 2026-06-29) | **`crd-hesap-interp`: 1-D piecewise + interface + safety contract.** linear · nearest/prev/next · cubic Hermite (scalar; cousin = ode `hermite_eval`) · **★PCHIP** (Fritsch-Carlson, monotone, **no-overshoot** — the certifiable control-LUT default). The `Interpolator` build-once/eval-many object + `InterpStatus` + caller-workspace contract + Tier-2 `worst_case_error`. Gate: scipy `interp1d`/`PchipInterpolator` ≤1e-12 + **no-overshoot invariant on monotone data** + linear `h²/8·max\|f″\|` Tier-2 bound + run-twice bit-identity + status validation. **⭐ CRUSH (vs scipy+MATLAB): PCHIP build 29.6×/21.4× · eval-sorted 2.05×/10.4× · eval-random 2.64×/1.45× (branchless search fixed a MATLAB loss).** | ~600 | 53 | — |
| ↳ **v13-b** ✅ (DONE 2026-06-29) | **interp: cubic splines (all boundary conditions).** natural · clamped · **★not-a-knot (default)** · periodic (cyclic) — the tridiagonal/cyclic system via `dense`. Gate: scipy `CubicSpline(bc_type=…)` ≤1e-12 + C² continuity + the **`(5/384)h⁴‖f⁽⁴⁾‖` Tier-2 certified bound** (Hall-Meyer optimal constant) + an O(n) Thomas solve. **⭐ CRUSH (vs scipy+MATLAB): spline build 10.1×/12.8× · eval-sorted 1.98×/8.9×.** | ~500 | 78 | — |
| ↳ **v13-c** ✅ (DONE 2026-06-29) | **interp: local + stable-polynomial.** Akima · **★makima** (no flat-region overshoot) · **★barycentric Lagrange** (2nd form, backward-stable, O(n) eval) · Newton divided-differences (incremental) · **★Floater-Hormann** (barycentric rational, *pole-free on all real nodes* — the equispaced-data answer). Gate: scipy/MATLAB `makima`/`BarycentricInterpolator`/`FloaterHormannInterpolator` ≤1e-10 + barycentric-on-Chebyshev-beats-Runge (maxerr 0.018). **⭐ CRUSH vs scipy: makima build 11.5× · barycentric build 110× · Floater-Hormann build 1421× · evals 1.8–9.1× (makima 4.7×/~9× MATLAB).** | ~700 | 121 | — |
| ↳ **v13-d** ✅ (DONE 2026-06-29) | **interp: spectral + rational.** **★Chebyshev interpolation** (DCT-fit via `fft` + Clenshaw recurrence — near-minimax, exponential convergence) · trigonometric/Fourier (via `fft`) · rational/Padé (over the shipped `dense::lstsq`, scipy-exact, spurious-pole guard). Gate: exp-convergence (Chebyshev→exp ≤1e-12) · band-limited exactness (trig) · scipy.pade ≤1e-10. **⭐ CRUSH vs numpy/scipy: Chebyshev build 24.8× · Padé build 54.2× · Padé eval 2.8× · Chebyshev batch-eval 3.4× (vectorized-across-points Clenshaw fixed a numpy-vectorized loss, bit-identical to scalar).** | ~700 | 24 | — |
| ↳ **v13-e** ✅ (DONE 2026-06-29) | **interp: scattered N-D RBF.** multiquadric · inverse-multiquadric · Gaussian · **★thin-plate spline** · polyharmonic (cubic) · **★Wendland compact-support** · Shepard/IDW — augmented poly-tail system over `dense` (LDLᵀ Bunch-Kaufman, the symmetric-indefinite saddle-point solver). Gate: scipy `RBFInterpolator` ≤1e-8 (5 kernels) + interpolation property + thin-plate polynomial-reproduction + Wendland compact-support + Shepard boundedness. **⭐ CRUSH vs scipy (gauss N=100 2-D): build 1.29× · eval 1.12×** — both started as LOSSES (lstsq-QR + redundant sqrt = 0.41×/0.84×), SOLVED via LDLᵀ (saddle-point symmetry) + r²-kernel sqrt-elimination (faster + more accurate). | ~900 | 81 | — |
| ↳ **v13-f** ✅ (DONE 2026-06-29) | **interp: gridded N-D + uncertainty.** bi/trilinear · bi/tricubic · tensor-product B-splines · Clough-Tocher C¹ (over Delaunay) · **★kriging / Gaussian-process** (returns mean **+ predictive variance** — the safety-critical sensor-fusion interpolant). Gate: scipy `RegularGridInterpolator`/`RectBivariateSpline`/`CloughTocher2DInterpolator` + sklearn GP (mean+var). **▶ N-linear + bicubic + cubic-B-spline ✅ (2026-06-29): `RegularGridInterpolant` — `eval_linear` (bilinear/trilinear fast paths, branchless `locate` + precomputed reciprocal) · `eval_cubic` (Keys convolution) · `eval_bspline` (Unser prefilter mirror-boundary), all over a shared `eval_4tap` core. 115 asrt ≤1e-9/1e-11 vs scipy.RGI/MATLAB `interpn`/scipy.ndimage + interpolation-property + clamp-no-NaN. ⭐⭐ BENCH (all peers): linear 3.65 ns = 20×scipy/3.1×MATLAB/edges Boost · bicubic 9.9 ns = 13×scipy/1.14×Boost-1D (MATLAB batch-cubic 6.5 leads bulk only) · B-spline 8.45 ns = 5.1×scipy.ndimage/1.46×MATLAB-spline/1.23×Boost-cardinal (FULL crush). **kriging/GP ✅ (`GaussianProcessInterpolant` mean+predictive-variance over dense::Cholesky; 17 asrt ≤1e-8/1e-7 vs sklearn; FIT 10.5×sklearn/**237× MATLAB fitrgp** + predict mean+var 2.45×sklearn/1.06×MATLAB — forward-only variance solve 1−‖L⁻¹k*‖²; Boost N/A—no probabilistic interpolator).** **Clough-Tocher ✅ (`CloughTocher2DInterpolant` over `geometry-delaunay` — bit-exact transcription of scipy `_interpnd.pyx`: AFFINE-INVARIANT neighbor-centroid reduced direction + 19 Bézier ordinates precomputed per triangle + curvature-min Gauss-Seidel gradient; eval = jump-walk locate (fast non-adaptive orient2d on the hot path + an EXACT linear-scan fallback on any walk-failure ⇒ no interior query ever spuriously NaNs) + extended-barycentric cubic. 55 asrt: bit-close ≤1e-9 vs scipy `CloughTocher2DInterpolator` + interpolation-property + EXACT linear-reproduction + C¹-smoothness + outside-hull NaN + determinism + dense-40k-interior-grid no-NaN/order-independence (eval = pure function of the query). ⭐⭐ CRUSH: FIT 107µs = 2.14×scipy · EVAL 85 ns/pt = 1.70× scipy-batch / 35.7× scipy-per-point (lever: fast non-adaptive orient2d for the walk, C⁰-safe) · 2.90× MATLAB griddata('cubic') · Boost N/A (no scattered C¹). **v13-f COMPLETE — all 5 gridded/scattered methods crush every available peer.**)** | ~1150 | 187 | — |
| ↳ **v13-g** ✅ (DONE 2026-06-30) | **extend `crd-hesap-quadrature`: the integrate API + nodes + result contract.** Gauss-**Lobatto** + Gauss-**Radau** · the `integrate()`/`QuadResult`/`AdaptiveWorkspace`/`QuadStatus` surface · composite trapezoid/Simpson on samples · Newton-Cotes. Gate: exactness degree (Gauss-n ≤2n−1, Lobatto ≤2n−3) + **positive-weight invariant** + scipy `simpson`/`trapz`. **✅ gauss_lobatto/radau (Jacobi-root nodes + closed-form weights, exactness-verified) · QuadResult{value,error_estimate,eval_count,subdiv_count,status,tolerance_met} error-tier contract · integrate_gauss/lobatto/radau + integrate_with_nodes / integrate_symmetric (precomputed real-time paths) · newton_cotes (exact Lagrange-integral, beats the Vandermonde) · trapezoid (uniform+non-uniform) · simpson (scipy-exact BOTH parities). 170 asrt ≤1e-13 vs scipy + exactness-degree + positive-weight + endpoints + determinism; full quad suite 300. ⭐⭐ CRUSH per-call real-time: gauss-sym 66ns = 85× scipy fixed_quad / 1.02× Boost gauss<10> (symmetric-pair fix flipped Boost's initial 4% edge) · simpson 21.9×scipy · trapezoid 70×scipy / 32.7×MATLAB trapz · newton_cotes 2.97×scipy; Boost/MATLAB have no composite/Lobatto/Radau/Newton-Cotes peer. The AdaptiveWorkspace + Gauss-Kronrod adaptive engine land in v13-h.** | ~700 | 170 | — |
| ↳ **v13-h** ✅ (QNG+QAG+QAGS+QAGP+QAGI DONE 2026-06-30) | **quadrature: the adaptive engine (real-time-safe).** **★Gauss-Kronrod** + the **ITERATIVE bounded-depth work-stack driver** (not recursive): QNG · QAG · **★QAGS** (+Wynn-ε, endpoint singularities — the default) · QAGP (break-points) · QAGI (infinite). Gate: QUADPACK/`scipy.quad` + `\|G−K\|` estimate + **no-recursion/fixed-workspace structural guard** + the Lyness-Kaganove hidden-peak honesty test. **✅ CORE DONE: GK21 rule (QUADPACK dqk21 constants — scipy hard-codes the same; bit-exact value + error estimate incl. the roundoff floor; degree-31=3n+1 exactness) + adaptive QAG (iterative bounded-depth work-stack, NO recursion, hard WCET bound → MaxSubdivisions). 71 asrt: GK exactness + adaptive ≤1e-10 vs scipy.quad (5 fns) + the no-recursion/WCET structural guard + the Lyness-Kaganove honesty test (estimate ≠ bound) + determinism; full quad suite 371. **QAGS** (Wynn-ε extrapolation, scipy.quad's default — a faithful goto-preserving port of QUADPACK dqagse + dqelg + dqpsrt from scipy's __quadpack.c; converges the singular integrands ∫1/√x=2 / ∫ln x=−1 / ∫(1−x)^−½=2 / ∫x^−0.8=5 that plain QAG stalls on). **QAGI** (infinite ranges — dqagie + dqk15i transform, the SAME Wynn-ε via a shared `qags_driver` + a reusable `AdaptiveWorkspace`; ∫₀^∞e^−x²=√π/2, ∫_{−∞}^∞1/(1+x²)=π, etc. ≤1e-8). ⭐⭐ **FULL PEER BOARD — CRUSHES EVERY peer incl. GSL (scipy+MATLAB+Boost+GSL 2.7.1): QAGS 631ns = 54× scipy / 1.53× Boost-GK-adaptive / 1.29× GSL-qags · QAGI 1076ns = 23× scipy / 1.36× GSL-qagiu.** ⚠ the GSL "near-parity" was a Cerid inefficiency, NOT the algorithm: the per-panel error estimate used `crd::math::pow(·,1.5)` (the heavy double-double pow, ~290ns/call) → `x·√x` (one hardware sqrt) flipped 0.88×→1.29× / 0.89×→1.36× (value unaffected — pow is only in the abserr heuristic). PLUS the determinism + WCET-bound + error-tier MOAT GSL/scipy/Boost lack. Boost exp_sinh is a double-exponential method = v13-i, not the QAGI peer. **QNG** (`integrate_qng` — non-adaptive Patterson 10/21/43/87 ladder, savfun reuse, ≤87 evals, allocation-free) + **QAGP** (`integrate_qagp` — break-points: split at the sorted singularities → QAGS pieces summed). ⭐⭐⭐ **EVERY METHOD CRUSHES GSL: QNG 133ns=1.09× · QAGS 631ns=1.29× · QAGP 1200ns=1.24× · QAGI 1076ns=1.36×** (+ scipy 22-54×, Boost-GK 1.53×, + the determinism/WCET/error-tier MOAT). v13-h METHOD SET COMPLETE. ⏳ optional (not a row requirement): higher GK31/41/51/61 rules (alternative QAG panels, low value — GK21 is the universal default). NEXT crush = v13-i (DE methods, beats Boost exp_sinh).** | ~1100 | 110 | — |
| ↳ **v13-i** ✅ (DE + CC/Fejér + Romberg DONE 2026-06-30) | **quadrature: non-Gauss + double-exponential.** **★Clenshaw-Curtis**/Fejér (Chebyshev points, FFT weights, nested, positive) · **★tanh-sinh / exp-sinh / sinh-sinh** (DE — endpoint singularities & infinite ranges & arbitrary precision) · Romberg. Gate: Boost.Math `tanh_sinh`/`exp_sinh` + Clenshaw-Curtis accuracy-per-eval + singular endpoints. **✅ DE DONE (`de.hpp`): precomputed `DeRule` nodes + level-refinement; tanh-sinh (endpoint sing) / exp-sinh ([a,∞)) / sinh-sinh ((−∞,∞)). 13 asrt vs analytic + determinism; full quad suite 423. ⭐⭐⭐ CRUSHES BOOST ALL 3: exp_sinh 351ns=1.17× / sinh_sinh 482ns=1.21× / tanh_sinh 102ns=1.34× (levers: double-exp convergence estimate d²/dₘ₋₁ halves levels + per-run tail truncation). **✅ CC/Fejér/Romberg DONE (`nongauss.hpp`): Clenshaw-Curtis (clencurt weights, fixed-rule + nested-adaptive `CcAdaptiveRule`) + Fejér-1 + Romberg (function + samples=scipy.romb). +42 asrt (exactness + analytic + romberg_samples vs scipy.romb); full quad suite 465. ⭐⭐⭐ CRUSH: Romberg 207ns=1.19× GSL-romberg ≫scipy.romb · CC-adaptive 272ns=2.05× GSL-cquad (precomputing the per-call-recomputed clencurt weights flipped a 0.59× loss). Boost N/A (no CC/Fejér/Romberg).** ⇒ the WHOLE quadrature board (v13-g/h/i) has ZERO open comparisons — every method crushes every available peer (scipy/MATLAB/Boost/GSL). | ~1000 | 55 | — |
| ↳ **v13-j** | **quadrature: oscillatory + singular weights.** **★QAWO** (Filon-type modified Clenshaw-Curtis — ω-independent cost) · QAWF (Fourier tail + ε-accel) · QAWS (algebraico-log endpoints) · QAWC (Cauchy principal value) · **★Levin collocation** (general/Bessel oscillators). Gate: QUADPACK `qawo`/`qawf`/`qaws`/`qawc` + accuracy-grows-with-ω. | ~800 | ~35 | — |
| ↳ **v13-k** | **quadrature: multi-D cubature.** tensor-product Gauss · **★Genz-Malik** adaptive (d≈2–7) · **★Smolyak sparse grids** (breaks the curse, dimension-adaptive) · **★Lebedev** (sphere — exact Yₗₘ, minimal points) · simplex (Dunavant — FEM/CFD). Gate: scipy/Cubature/Tasmanian + spherical-harmonic + simplex exactness. | ~900 | ~40 | — |
| ↳ **v13-l** | **`crd-hesap-diff`: finite differences (done right).** **★Fornberg arbitrary-stencil weights** (any nodes/order, stable) · central/forward/backward **per-scheme + per-magnitude optimal step** · **★Richardson/Ridders** (error-killer + estimate) · matrix FD Jacobian + Hessian-vector. Gate: `numpy.gradient`/`findiff`/NR `dfridr` + the √ε/ε^⅓ accuracy floors. | ~600 | ~30 | — |
| ↳ **v13-m** | **diff: exact + spectral + noisy.** **★★complex-step** `Im[f(x+ih)]/h` (machine-exact, **zero subtractive cancellation**, no h-tuning — satellite/aero/MDO sensitivity) · **★Chebyshev/Fourier differentiation matrices** (spectral) · **★Savitzky-Golay** differentiation (noisy IMU/telemetry rates). Gate: complex-step = analytic-exact ≤machine-ε · spectral exp-convergence · scipy `savgol_filter(deriv=k)`. | ~700 | ~30 | — |
| ↳ **v13-n** | **`crd-hesap-motion`: attitude trajectories.** **★SQUAD** (C² spherical cubic via nested SLERP — smooth slews/camera rigs) · **★quaternion-B-spline / log-quat spline** (C², manifold-correct via exp/log) — reuse `crd-math` slerp + quat log/exp. Gate: const-angular-velocity (SLERP) + C¹/C² continuity + unit-norm preservation. | ~500 | ~25 | — |
| ↳ **v13-o** | **motion: continuous-curvature paths.** **★clothoid / Euler spiral** (curvature linear in arc length → bounded lateral jerk for self-driving steering; over `special::fresnel` + a G¹/G² fitting solve) · **★NURBS** (rational weights → exact arcs/conics; extends geometry-curves B-spline). Gate: curvature-linearity + exact-circle reproduction + G² continuity. | ~700 | ~30 | — |
| ↳ **v13-p** | **motion: optimal polynomial trajectories.** **★minimum-snap** (degree-7, C⁴, the `pᵀQp` QP over differentially-flat outputs — drones; via `crd-hesap-opt`) · minimum-jerk (quintic — robot arms/haptics) + the differential-flatness mapping. Gate: Mellinger reference + continuity-up-to-snap + unconstrained-QP stability at high segment count. | ~700 | ~30 | — |
| ↳ **v13-q** | **motion: jerk-limited real-time.** S-curve/trapezoidal velocity profiles · **★★Ruckig-class online time-optimal generator** (jerk-limited, multi-DoF time-synchronized, deterministic worst-case — modern best, supersedes Reflexxes) · **Kochanek-Bartels (TCB)** (extends the shipping Catmull-Rom). Gate: jerk/acc/vel-limit conformance + time-optimality vs Ruckig + the **bounded-WCET real-time guarantee**. | ~900 | ~35 | — |
| ↳ **v13-z** | **CLOSE.** CLI `hesap.{interp,quad,diff,motion}.*` (curated agent subset) + 4 system docs (`hesap-interp.md`/`hesap-diff.md`/`hesap-motion.md` + rewrite `hesap-quadrature.md`) + **ADR-0095** + the all-peers crush scoreboard + the `{1,4,16}` determinism-moat audit + the **safety-critical conformance audit** (builds `-fno-exceptions`; the iterative-not-recursive structural guard; no-heap-in-hot-path; status-not-exception). | ~800 | ~30 | — |
...
#### FFT generated-codelet substrate (Fork A) — BANKED in the engine + parity-assault CLOSED (2026-06-20)
> The Fork-A generated-codelet substrate is SHIPPED in the engine DEFAULT path: `build/gen_subfft_m3.py` emits the
> hierarchical sub-FFT codelets (1024=32×32, 2048=64×32, 4096=64×64, **256=16×16**) into `detail/hier_codelets.hpp`,
> and the four-step P1/P2 fusion (gather + scatter) consumes them by default — this carried f64/f32 large-N to
> ~0.84–0.96× MKL and **fixed the 256K Stockham trough to ~0.85× (3.6× CRD speedup)**. The MKL-parity assault is
> **CLOSED / deferred** per the v10 closure (2026-06-20): 8 measured attempts proved the four-step inter-stage twiddle
> is the irreducible ~15% gap MKL closes only via asm-integrated butterflies (out of scope — ADR-0082 portable-C++ +
> WASM-no-asm; no native/asm path for v10). Full history + the M0–M9 milestone record:
> `docs/design/hesap_fft_generated_codelets.md` + memory `project_v10_fft_plan`.
...
| **v12-l** ✅ (DONE 2026-06-27) | **Autodiff-ready log-densities:** analytic ∂logp/∂x and ∂logp/∂θ for every distribution (25 continuous + 12 discrete + 8 heavy-tail + MVN/MVt; the HMC/NUTS + MLE enabler; exp-family suff-stats yield algorithmic crush — Normal ~888,000×, Gamma ~8,700× vs JAX; StudentT O(N)-irreducible, near-parity 1.14× XLA). Gradient-checked vs FD (348 asrt). | Stan-math reference gradients · FD grad-check | ~600 | ~25 |
| **— INFERENCE (`crd-hesap-stats`) —** | | | | |
| **v12-m** ✅ (DONE 2026-06-27) | **Descriptive + estimators:** moments (mean/var/stddev/skew/kurt, scipy conventions, +robust: MAD/trimmed mean) · quantiles (all 9 R Hyndman-Fan types + Harrell-Davis beta-weighted) · ECDF · histogram bin-rule selectors (Sturges/Freedman-Diaconis/Scott) · weighted mean/variance · covariance/correlation matrices. Gate: scipy.stats/numpy/MATLAB <1e-9 (82 asrt). | scipy.stats · R `quantile(type=)` · MATLAB `quantile`/`prctile` | ~500 | ~20 |
| **v12-n** ✅ (DONE 2026-06-28) | **The full test suite — COMPLETE** (`hypothesis.hpp`; 121 asrt bit-for-bit vs scipy/statsmodels; full stats suite 318396/116; crushes scipy 37–1142× / R 104–1413× / MATLAB 20–834×; new acyclic stats→quadrature edge for the studentized range): t(1samp/ind/Welch/rel)/z/ANOVA(1-way·2-way·repeated)/Bartlett/Levene-BrownForsythe · MannWhitney/Wilcoxon/Kruskal-Wallis/Friedman/sign/Mood · KS(1+2)/Anderson-Darling/Cramér-von-Mises/Shapiro-Wilk(Royston)/Jarque-Bera/D'Agostino-K²/Lilliefors · χ²(Yates)/Fisher-exact/McNemar/G-test · Pearson/Spearman/Kendall-τb/distance-corr · Tukey-HSD/Games-Howell/Scheffé/Dunnett(all studentized-range or equicorrelated-MVt via Gauss-Hermite×Gauss-Laguerre)/Holm/BH-FDR/Bonferroni · Cohen's d/η²/Cramér's V. **Honest scope notes:** Mood = no-tie variance (scipy adds a Mielke tie correction for tied samples); Dunnett = balanced/equal-treatment-size (equicorrelation); CvM = asymptotic p (scipy adds a ~2e-4 finite-n correction). | scipy.stats · statsmodels · R · MATLAB `ttest*`/`anovan`/`ranksum`/`kstest`/`adtest`/`swtest`/`chi2gof`/`multcompare` | ~1100 | 121 |
| **v12-o** ✅ (DONE 2026-06-28) | **Resampling — COMPLETE** (`resampling.hpp` + `resampling_parallel.hpp`): bootstrap (basic/percentile/BCa/studentized) · block bootstrap (moving-block) · jackknife (+ delete-d) · exact+MC permutation · CV (`kfold_indices`, k=n→LOO) · **parallel over crd-jobs, bit-identical to serial = the determinism moat under threading** (new acyclic stats→jobs edge). 52 asrt: deterministic core (jackknife, CI math, exact permutation) gated bit-for-bit vs scipy/statsmodels; the RNG bootstrap CI MC-gated vs scipy.stats.bootstrap @B=300000; same-seed + parallel==serial bit-identity. ⭐⭐ **FULL CRUSH per-thread AND parallel** (B=100000 percentile-CI-of-mean): serial 22.8ms = **2.08× scipy / 21.4× R / 13.8× MATLAB**; parallel(32c) 9.4ms = 5.05× / 51.9× / 33.4×. The per-thread crush came from killing the `% n` modulo (Lemire `bounded`) then Threefry4x64→**Philox4x32 + u32 Lemire** (indices fit in 32 bits) — a 0.74× serial loss → 2.08× win, moat preserved. | scipy.stats.bootstrap · R `boot`/`bootci` · MATLAB `bootci` | ~700 | 52 |
| **— THE INSANE TAIL (`crd-hesap-stats`) —** | | | | |
| **v12-p** ✅ (DONE 2026-06-28) | **KDE + robust + streaming — COMPLETE** (`kde.hpp`/`robust.hpp`/`cov_robust.hpp`/`streaming.hpp`/`tdigest.hpp`): KDE Gaussian/Epanechnikov (Scott/Silverman/rule-of-thumb/LOO-CV bandwidth; Gaussian bit-matches scipy.stats.gaussian_kde) · robust (Huber/Tukey-biweight M-est + Huber proposal-2 · exact MCD covariance · Theil-Sen · Hodges-Lehmann) · covariance shrinkage (Ledoit-Wolf/OAS) · online (Welford · P² quantile · t-digest · HyperLogLog · count-min). 49 asrt: deterministic estimators bit-for-bit vs scipy/statsmodels/sklearn/numpy; probabilistic sketches within their accuracy guarantee vs the true value. ⭐⭐ **CRUSH (ns/call):** kde_eval **5872× scipy** · huber_loc **503× statsmodels** · oas **437× sklearn** · ledoit_wolf 257× · theil_sen 6.9× (O(n²)). 2 real fixes: RLM scale = center-0 MAD `median(\|resid\|)/0.6745`; proposal-2 uses N−1 df in the gamma term. | sklearn · statsmodels · scipy · MATLAB `ksdensity`/`robustcov` · R | ~1100 | 49 |
| **v12-q** ✅ (DONE 2026-06-28) | **MCMC / Bayesian — COMPLETE** (`mcmc_diagnostics.hpp` + `mcmc.hpp`): Metropolis-Hastings + adaptive (Haario) · Gibbs · **HMC** · **NUTS** (recursive tree-doubling + U-turn + **dual-averaging** = the full Stan algorithm) · slice (stepping-out+shrinkage) · SMC (tempered) + diagnostics (rank-normalized R-hat · Geyer bulk-ESS · autocorr · Geweke). 36 asrt: **diagnostics gated BIT-FOR-BIT vs ArviZ** (caught 4 formula bugs by reading their backend source); samplers recover known targets + the **determinism moat** (same-seed bit-identical) + a bit-exact leapfrog. ⭐⭐ **CRUSH (effective-samples/sec, the table metric): Cerid NUTS 907,142 ess/s vs PyMC NUTS 8,711 = 104×** (87× samples/s); HMC 12.7M ess/s. Honest scope: adaptive_metropolis = component-wise (= full Haario in 1-D). | Stan · PyMC · ArviZ · MATLAB `hmcSampler`/`mhsample`/`slicesample` | ~1100 | 36 |
| **v12-r** ✅ (DONE 2026-06-28) | **Regression / GLM / multivariate — COMPLETE** (`regression.hpp`, 9 groups): OLS/WLS/GLS (over the shipped `dense::lstsq` QR/COD/SVD; coef+R²+SE) · ridge (closed form) + lasso/elastic-net (cyclic coordinate descent) · GLM via IRLS (logistic/Poisson/gamma-log, Fisher scoring) · robust Huber (RLM IRLS, center-0 MAD) + quantile regression (pinball IRLS) + **RANSAC** (consensus + inlier refit, same-seed bit-identical = determinism moat) · PCA (covariance eigendecomp) + LDA/QDA (Bayes-rule classifiers, exact-integer prediction match) + factor analysis (SVD/EM, log-likelihood stop). **SANITY 8**: reuses `lstsq`/`pinv`/`eig_sym`/`Symmetric` from crd-hesap-dense — zero reimplementation. 58 asrt, **all gold-standard gated vs statsmodels/sklearn**. ⭐⭐ **CRUSH vs sklearn (fits/sec, native C++ over the shipped factors vs Python interpreter+numpy-dispatch overhead): Ridge 47.2× · PCA 23.2× · OLS 16.9× · Lasso 16.7× · GLM-logit 2.7×** (closed-form models crush hardest). | statsmodels · sklearn · MATLAB `fitlm`/`fitglm`/`lasso`/`pca`/`fitcdiscr` · R | ~1000 | 58 |
| **— CLOSE —** | | | | |
| **v12-z** ✅ (DONE 2026-06-28) | **CLOSE — COMPLETE.** CLI `hesap.stats.*` (describe / ttest_1samp) + `hesap.special.*` (gamma / erf), curated agent subset via `CRD_HESAP_CLI_REGISTER_MODULE` + `cli_anchor.hpp` · system docs `hesap-special.md` (created) + `hesap-stats.md` (rewritten for the v12 cluster) · **deferred v11 spectral CIs back-wired**: multitaper PSD **χ² CI** (`multitaper_psd_ci`, ν=2K/Thomson ν(f), `χ²_{ν,q}=2·gammainc_p_inv(ν/2,q)`, gated vs **independent scipy chi2.ppf** + the frequency-constant invariant) + AR-PSD **asymptotic-normal CI** (`ar_psd_ci`, Berk Var{log Ŝ}≈2p/N, `Ŝ·exp(±z·√(2p/N))`, z=ndtri — NO external peer, analytic-gated + honestly documented) · ADR-0094 created + indexed. 17 asrt. Suites GREEN: dsp 27076 / special 402091 / stats 318601. | scipy `chi2.ppf` · MATLAB `pmtm pxxc` · (AR = no clean peer) | ~400 | 17 |
...
## v12 — Statistics (Special Functions + RNG + Distributions + Tests)

**Status:** ✅ v12-a…v12-m SHIPPED; **v12-n ✅ DONE (2026-06-28)** — the FULL hypothesis-test suite, every test in the table (t/z/Welch/ANOVA[1-way·2-way·repeated]/Bartlett/Levene · MannWhitney/Wilcoxon/KW/Friedman/sign/Mood · KS/AD/CvM/Shapiro/JB/D'Agostino/Lilliefors · χ²/Fisher/McNemar/G · Pearson/Spearman/Kendall/dCor · Tukey/Games-Howell/Scheffé/Dunnett/Holm/BH/Bonferroni · Cohen's d/η²/Cramér's V), 121 assertions vs scipy/statsmodels, crushes scipy/R/MATLAB; new acyclic edge `stats→quadrature`. Honest scope: Mood no-tie variance · Dunnett balanced · CvM asymptotic p. **v12-o ✅ DONE (2026-06-28)** — the FULL resampling suite (bootstrap percentile/basic/BCa/studentized · block · jackknife+delete-d · exact+MC permutation · CV · parallel over crd-jobs = determinism moat), 52 assertions; per-thread AND parallel CRUSH (serial 2.08× scipy/21.4× R/13.8× MATLAB; parallel 5.05×/51.9×/33.4×) via Philox-u32 + Lemire. **v12-p ✅ DONE (2026-06-28)** — KDE (Gaussian/Epanechnikov + 4 bandwidths) · robust (Theil-Sen/Hodges-Lehmann/Huber-M/Tukey-M/proposal-2) · covariance (Ledoit-Wolf/OAS/exact-MCD) · streaming (Welford/P²/t-digest/HyperLogLog/count-min), 49 assertions vs scipy/statsmodels/sklearn/numpy; CRUSH kde 5872×/huber 503×/oas 437×. **v12-q ✅ DONE (2026-06-28)** — MCMC/Bayesian: Metropolis/adaptive-Haario/Gibbs/HMC/**NUTS**(+dual-averaging = full Stan)/slice/SMC + diagnostics (rank-R-hat/Geyer-ESS/autocorr/Geweke), 36 assertions; diagnostics bit-for-bit vs ArviZ + samplers recover targets + determinism moat; **NUTS 104× ess/s vs PyMC**. **v12-r ✅ DONE (2026-06-28)** — regression/GLM/multivariate (9 groups): OLS/WLS/GLS · ridge/lasso/elastic-net · GLM-IRLS (logistic/Poisson/gamma) · robust-Huber/quantile/RANSAC · PCA/LDA/QDA/factor-analysis; rides the shipped dense lstsq/pinv/eig_sym (SANITY 8), 58 assertions gated vs statsmodels/sklearn; **CRUSH Ridge 47×/PCA 23×/OLS 17×/Lasso 17× vs sklearn**. Full hesap-stats suite **318591/135** GREEN. **v12-z ✅ DONE (2026-06-28)** — CLOSE: CLI `hesap.{stats,special}.*` (describe/ttest + gamma/erf) + system docs (`hesap-special.md` created, `hesap-stats.md` rewritten) + **the deferred v11 spectral CIs back-wired** (multitaper χ² vs scipy chi2.ppf + AR Berk-normal analytic-gated, closing the ADR-0093 deferral) + ADR-0094 created+indexed; 17 asrt; dsp 27076 / special 402091 / stats 318601 GREEN. **THE v12 STATISTICS ARC a→z IS COMPLETE.** PENDING (user's process only): Windows 4-config DoD + 18-config CI + commit (v12-q/r/z uncommitted). **▶▶▶ NEXT (ACTIVE): v13 — the Numerical-Analysis + Motion cluster** (interpolation · adaptive quadrature · numerical differentiation · trajectory generation). RE-SCOPED to a MAJOR 4-module certification-grade cluster (`crd-hesap-interp` + extend `crd-hesap-quadrature` + `crd-hesap-diff` + `crd-hesap-motion`); the 3 moat pillars (determinism-by-construction · allocation-free bounded-recursion streaming · error-tier-exposing) = the DO-178C/ISO-26262-ASIL-D moat for satellites/drones/robots/self-driving/games. **DETAILED PLAN: `docs/phases/phase-3.1.6-v13.md`; ADR-0095 at v13-a.** ~12-16 KLOC / ~17 sub-slices / multi-session. Sequencing: interp(a-f) → quadrature-extend(g-k) → diff(l-m) → motion(n-q) → close(z).

---

### Original v12 stub (history only)

> ⛔ **NOTE:** v12 numbering was originally reserved for DSP, now reassigned to Statistics. The DSP work shipped as the maximal 3-module **v11** cluster (dsp a–t + wavelet a–e + comms a–g).
