...
| ↳ **v4j-2** | Inverse-based pivoting + dropping strategy (the ILUPACK numerical core — bounded inverse-factor norm pivot test + dual dropping). | ~600 | ~12 | — |
| ↳ **v4j-3** | Quality tuning + the head-to-head (vs ILUPACK/AMGCL convergence on hard matrices) + complex. | ~400 | ~11 | — |
| ↳ **v4k** | **AMG core (classical Ruge-Stüben).** Strength-of-connection + C/F coarsening + interpolation + **Galerkin coarse op `Pᵀ A P`** (via spgemm) + **V/W/F-cycles** + smoother interface (Jacobi/GS/Chebyshev). Ships AMG-as-solver AND AMG-as-`LinearOp`-preconditioner. Deterministic coarsening tie-breaks. `crd-hesap-amg` module. Real+complex. | ~1400 | ~35 | — |
| ↳ **v4l** | **SA-AMG + AGMG.** Smoothed aggregation (Vaněk 1996) + aggregation-based AGMG (Notay 2010). Head-to-head vs AMGCL/PETSc-GAMG/hypre-BoomerAMG (grid-complexity + convergence-factor + setup+solve wall). Real+complex. | ~1000 | ~28 | — |
| ↳ **v4m** | **Bootstrap / adaptive AMG** (αSA / bootstrap setup for anisotropic / unknown-near-nullspace problems). Real+complex. | ~700 | ~20 | — |
| ↳ **v4z** | **Close.** Complex-completeness audit (every solver/preconditioner/AMG has its complex/Hermitian variant) + CLI-completeness audit (every op a command) + **ADR-0065 §lock** (D(iter)-N: deterministic reductions, fixed coarsening tie-breaks, breakdown handling, recycling subspace selection) + vs-reference rollup (Eigen + AMGCL/PETSc/hypre) + 18-config sweep. | ~200 | — | — |
...
- **v4j** — **Multilevel ILU (ILUPACK-class)** — split: **j-1** multilevel scaffold +
  reordering/scaling, **j-2** inverse-based pivoting + dropping strategy, **j-3** quality
  tuning + the head-to-head. (Research-grade; sized as its own mini-cluster.)

**`crd-hesap-amg`:**
- **v4k** — **AMG core**: strength-of-connection + **classical Ruge-Stüben** coarsening
  (C/F splitting) + interpolation operators + **Galerkin coarse operator `Pᵀ A P`** (via
  spgemm) + **V/W/F-cycles** + the smoother interface (Jacobi/GS/Chebyshev from v4a/v4i).
  Ships AMG-as-solver AND AMG-as-`LinearOp`-preconditioner.
- **v4l** — **SA-AMG** (smoothed aggregation, Vaněk 1996) + **AGMG** (aggregation-based,
  Notay 2010).
- **v4m** — **bootstrap / adaptive AMG** (αSA / bootstrap setup for hard, anisotropic, or
  unknown-near-nullspace problems).
