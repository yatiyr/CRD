...
| ↳ **v2c** | **Full symbolic factorisation** (the v5 hand-off). Elimination tree (Liu, path-compressed) + column counts (Gilbert-Ng-Peyton skeleton) + L sparsity pattern (etree row-subtrees) + supernode detection (for v5 supernodal Cholesky). Gate: symbolic-L pattern == structure of a numeric Cholesky factor (cross-check); fast vs Eigen symbolic. | ~450 | ~18 |
| ↳ **v2d** | **Multilevel ND — scaffold.** Heavy-edge-matching coarsening (D(ord)-1 tie-break) + initial partitioning on the coarsest graph (BFS/region-grow bipartition) + uncoarsening recursive-descent framework (NO refinement yet — proves the multilevel scaffold correct with mediocre-but-valid partitions). | ~350 | ~12 |
| ↳ **v2e** | **Multilevel ND — refinement + ND driver + v2-close.** Fiduccia-Mattheyses refinement (gain buckets, equal-gain by D(ord)-1) during uncoarsening + vertex-separator extraction from the edge cut + recursive-bisection nested-dissection driver + AMD-hybrid threshold (METIS pattern: switch to AMD on small subgraphs). **Gate: beats AMD fill on the structured-mesh (FEM) subset.** v2-close: CLI completeness + ADR-0065 §16 (lock D(ord)-1..N) + 18-config sweep. | ~300 | ~12 |
