...
| ↳ **v7-f** (planned) | **First-order** — steepest descent · **nonlinear CG** (FR/PR+/HS/DY + restarts) · momentum/Nesterov. | — | — | — |
| ↳ **v7-g** (planned) | **Newton family** — full Newton (Hessian via hesap-direct) · **Newton-CG / truncated Newton** (Hessian-vector, large-scale) · modified-Newton (negative-curvature). | — | — | — |
| ↳ **v7-h** (planned) | **Trust-region** — framework (radius mgmt) + subproblems: Cauchy · **dogleg** · 2D-subspace · **Steihaug-CG** · **trust-Krylov (GLTR)** · **exact (Moré-Sorensen** via hesap-eig secular eqn). [gold: scipy trust-*, GALAHAD] | — | — | — |
| ↳ **v7-i** (planned) | **Stochastic / ML** — SGD(+momentum/Nesterov) · Adam · AdamW · Nadam · RAdam · RMSprop · Adagrad · Adadelta · Lion · LAMB + LR schedules + grad clipping + decoupled weight decay. **⚠ MOAT SPLITS (advisor):** optimizer-UPDATE determinism is in-scope now (parallel-reduction, like v6); FULL shuffled-minibatch reproducibility needs the Philox/Threefry counter-RNG from **v12-stats (not built)** — name the dep or build a minimal counter-RNG; do NOT claim "reproducible ML training" without it. [gold: PyTorch/JAX optimizers] | — | — | — |
| ↳ **v7-j** (planned) | **Constrained substrate** — equality/inequality constraints + their Jacobians + the **KKT system** + Lagrangian/merit functions + multipliers/dual variables + KKT-residual stopping. Extends `Objective` (v7-a). | — | — | — |
| ↳ **v7-k** ⭐ (planned) | **QP (quadratic programming)** — active-set (**Goldfarb-Idnani** dual) · interior-point (Mehrotra) · **OSQP-class ADMM** (operator-splitting, embedded MPC). [gold: OSQP, qpOASES, quadprog] — the MPC/contact/SVM workhorse. | — | — | — |
| ↳ **v7-l** (planned) | **LP (linear programming)** — revised simplex (Dantzig + Bland anti-cycling) · Mehrotra primal-dual interior-point. [gold: HiGHS, GLPK, scipy.linprog] | — | — | — |
| ↳ **v7-m** (planned) | **Conic** — **SCS-class** operator-splitting (SOCP/SDP via cone projection; O'Donoghue 2016). [gold: SCS, ECOS] | — | — | — |
| ↳ **v7-n** ⭐ (planned) | **NLP (nonlinear programming)** — **SQP** (sequential QP, the smooth-constrained workhorse) · **interior-point (IPOPT-class, Wächter-Biegler filter line-search)** · augmented Lagrangian (LANCELOT-class). **PROBE THE IPOPT/cyipopt INSTALL HERE** (heavy; can reuse our MUMPS as its linear solver). [gold: IPOPT, NLopt SLSQP, scipy trust-constr/SLSQP] — trajectory opt / FEA / topology / eylem constrained dynamics. | — | — | — |
| ↳ **v7-o** (planned) | **Algebraic modeling layer** — JuMP/CasADi pattern: declarative variables/objective/constraints → auto-derivatives (via autodiff) → solver dispatch (the ergonomic façade, opt-in like the MATLAB-class hesap façade). | — | — | — |
| ↳ **v7-p** (planned, slip-candidate) | **Derivative-free / black-box** — Nelder-Mead (simplex) · Powell · pattern-search (GPS/MADS) · BOBYQA/NEWUOA · COBYLA. [gold: NLopt, scipy] — no named consumer yet; elite completeness. | — | — | — |
| ↳ **v7-q** (planned, slip-candidate) | **Global / metaheuristic** — **CMA-ES** · differential evolution · PSO · simulated annealing · basin-hopping · dual-annealing · multi-start. [gold: pycma, scipy global] — non-convex (hyperparameter/design/balancing). | — | — | — |
| ↳ **v7-r** (planned, slip-candidate) | **MIP (mixed-integer)** — branch-and-bound + cutting planes (Gomory) + branch-and-cut over the LP relaxation. [gold: HiGHS, GLPK, SCIP] — heavy; few near-term Cerid consumers. | — | — | — |
| ↳ **v7-z** (planned) | **CLOSE** — CLI `hesap.opt.*` (unconstrained + QP + LP + NLP) + gold-standard bench (Ceres / scipy.optimize / NLopt / OSQP / SCS / HiGHS / IPOPT / CMA-ES / PyTorch, matched accuracy, honest, with the wall-clock-vs-Python-overhead caveat from v6-z) + end-to-end {1..16} determinism moat (bit-identical trajectory) + `docs/systems/hesap-opt.md` + ADR + 18-config full sweep. | — | — | — |
