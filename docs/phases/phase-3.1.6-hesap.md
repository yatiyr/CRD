...
| **v0b** ✅ 2026-05-19 | BLAS L1 × 4 type variants (axpy / dot / dotu / dotc / nrm2 / scal / copy / swap / asum / iamax) + Kahan-Babuška-Neumaier pairwise reduction + 28 working CLI commands + ArgValue typed-param substrate. **Sparse**: deferred to v1. **SIMD parity**: scalar shipping; canonical tree topology designed for 1:1 SIMD slot-in (filed as `v0b-simd-followon`). 39+60 cases / 151+318 assertions PASS. Session: `docs/sessions/2026-05-19-hesap-v0b-blas1.md`. | ~700 → ~2 200 actual | ~80 → 60 actual | ~3 d → 1 d |
| **v0c** ✅ 2026-05-19 | BLAS L2: all 17 ops × 4 type variants where applicable (gemv / gbmv / hemv / hbmv / symv / sbmv / ger / geru / gerc / syr / her / syr2 / her2 / trmv / trsv / tbmv / tbsv) + Matrix / MatrixView / Symmetric / Hermitian / Triangular / Banded bodies + 8 working CLI commands (gemv/symv/trsv) — additional CLI surface filed for v0c-cli-extend follow-on (non-blocking; engine surface complete). 94+39 cases / 521+151 assertions PASS. Session: `docs/sessions/2026-05-19-hesap-v0c-blas2.md`. | ~1000 → ~4 800 actual | ~80 → 60 actual | ~5 d → 1 d |
| **v0d** ◑ 2026-05-19 FOUNDATION + perf-chunk-1 + perf-f64-avx2 + parallelism shipped | **BLAS L3 via task-DAG** — Goto/BLIS 5-loop layered gemm with packing + scalar/AVX2 microkernel hot-swap (ADR-0082 intrinsics; ASM deferred behind locked CRD_HESAP_MICROKERNEL_BACKEND switch) + Vec4d substrate + f64 AVX2 microkernel + `gemm_parallel<T, L>(num_workers, …)` BLIS-style outer-loop parallelism + syrk / herk / syr2k / her2k / trmm / trsm + mixed-precision gemm_mixed<f32, f64> + `crd-hesap-sched` substrate + 4 working CLI commands + benchmark fixtures. **Single-core: f32 at 59% peak / f64 at 57% peak at N=1024** (intrinsics-tier; ~12× foundation→perf for f32, +3.3× for f64 via Vec4d). **Multi-core (win-release, N=1024): f32 = 4.71× at 32 workers (508 GFLOPS); f64 = 4.20× at 16 workers (227 GFLOPS)**. Remaining follow-ons: `v0d-microkernel-tune` (AVX-512+NEON+SVE2 + prefetch + block tune), `v0d-formal-dag`, `v0d-cli-extend`, `v0d-parallelism-auto-dispatch`, `v0d-asm-microkernel` (ADR-0082 §revisit gate). Sessions: `2026-05-19-hesap-v0d-blas3.md`, `2026-05-19-hesap-v0d-perf-chunk1.md`, `2026-05-19-hesap-v0d-perf-f64-avx2.md`, `2026-05-19-hesap-v0d-parallelism.md`. | ~1200 → ~6100 actual to date | ~100 → 33 actual | ~7 d → 1 d foundation + 3 perf sub-slices in 1 d |
