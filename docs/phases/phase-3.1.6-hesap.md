...
| ↳ **v6-c** 🔄 PARTIAL 2026-06-06 | **Arnoldi + Krylov-Schur** (Stewart 2001 ≡ IRAM) — nonsymmetric. **ARNOLDI DONE: `arnoldi.hpp` `eigs_nonsym` — full-GS Arnoldi → upper Hessenberg H → Rayleigh-Ritz via dense `eig(H)` (complex Ritz values + small Ritz vectors) → wanted COMPLEX eigenvalues + the cheap Arnoldi residual estimate β_m·|s[m-1]|; `more_wanted_c` complex `which`.** Tests `[eigen]`: 2×2 rotation-block matrix [[a,b],[−b,a]] ⇒ KNOWN eigenvalues a±b·i, full Arnoldi at m=n recovers them (8±0.5i,7±0.5i by LargestReal) + {1,2,4,8} MOAT bit-identical complex Ritz values. VERIFIED win-debug + win-tidy. **PENDING (v6-c completion): Krylov-Schur restart (deterministic reorder-Schur, large-n/bounded-m) + complex eigenVECTOR recovery (X=V·S) + true complex residuals.** | ~350 | ~12 | — |
...
| ↳ **v6-e** (planned) | **LOBPCG** (Knyazev 2001) — block + preconditioned, symmetric-generalized SPD (FEM modal/buckling); uses hesap-preconditioners (or a v5 factor). **⭐ algorithmic crush.** Block ⇒ subspace-identity moat test. | ~350 | ~12 | — |
| ↳ **v6-f** (planned, slip-candidate) | **Jacobi-Davidson** (JDQR/JDQZ) — Davidson subspace + the correction equation solved inexactly via GMRES (reuse hesap-iterative); interior/hard/clustered; PRIMME-class. **⭐ algorithmic crush.** | ~350 | ~12 | — |
| ↳ **v6-g** (planned, slip-candidate) | **FEAST** (Polizzi 2009) — contour-integration for ALL eigenvalues in [a,b]; quadrature points → shift-invert (v5) → subspace iteration; embarrassingly parallel over quadrature (moat-friendly). The interior specialist. | ~300 | ~10 | — |
| ↳ **v6-h** (planned) | **IRLBA** (Baglama-Reichel 2005) — implicitly-restarted Lanczos BIDIAGONALIZATION for largest/smallest SINGULAR triplets of a (rectangular) sparse matrix (the sparse SVD); reuses the Lanczos substrate + dense bidiag SVD. (Distinct from principle #10 randomized SVD / `randomized_range`.) parity+moat. | ~300 | ~10 | — |
| ↳ **v6-z** (planned) | **CLOSE** — complex completeness audit + CLI `hesap.eigen.*` + end-to-end {1..16} determinism moat ALL methods (well-separated spectra) + gold-standard bench (ARPACK/scipy eigsh + PRIMME + FEAST-lib + Spectra-C++, matched accuracy, honest) + `docs/systems/hesap-eigen.md` + ADR (the new module edges) + 18-config full sweep. | ~250 | ~10 | — |
