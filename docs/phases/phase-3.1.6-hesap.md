...
| ↳ **v9-a** (PLANNED) | **Substrate + fixed-step explicit.** **TWO API LAYERS from day 1** (the game-dev brainstorm 2026-06-11, memory `project_ode_in_games_layering`): (a) **stepper KERNEL HEADERS** — raw-span, allocation-free, inlineable (`step_euler/semi_implicit/midpoint/rk4`...) so eylem's fused SoA sweep / animation / DAW consume them WITHOUT the driver (no virtual RHS in hot loops — the anti-pattern); (b) the **general driver**: `OdeFunction` capability contract (rhs; optional dense/sparse Jacobian, jac-vec, mass matrix, events) · `OdeOptions` (rtol/atol scalar-or-vector, h0/hmax, max_steps) · `OdeResult` + **deterministic work counters** (nsteps/naccept/nreject/nfev/njev/nlu/nsol — the work-precision currency) · the **dense-output contract** (per-method interpolant) · the **step-controller substrate** (elementary + Gustafsson PI as pure functions) · `integrate_fixed` wiring proof over the (a) kernels. Gates: empirical h-refinement order slopes (oracle-free, sharp), run-twice bit-identity, n=0/1 edges. ADR-0091. | ~450 | ~15 | — |
| ↳ **v9-b** (PLANNED) | **Embedded explicit RK — the non-stiff spine.** RK23 (Bogacki-Shampine) + RK45 (Dormand-Prince) + DOP853 with **scipy's exact semantics** (error norm, controller constants, Hairer auto-h0 ⇒ trajectory-exact gates) + **Tsit5** (Tsitouras 2011) + Cash-Karp + Verner 6(5)/7(6). Per-method dense output (incl. DOP853's 3-extra-stage 7th-order). Gates: scipy step-sequence/solution matches · Arenstorf + Pleiades · order verification per pair · Hairer DOPRI5/DOP853 e2e oracle harness (the NLopt recipe). | ~700 | ~25 | — |
| ↳ **v9-c** (PLANNED) | **Events + continuous output.** g(t,y) events (directional/terminal, scipy semantics) root-refined on the interpolant (Brent) · `t_eval` sampling · the `OdeSolution` object. Gates: bouncing ball, scipy event-semantics match, event-at-step-boundary edges. | ~250 | ~10 | — |
| ↳ **v9-d** (PLANNED) | **BDF/NDF — the stiff spine.** scipy's BDF exactly (Shampine NDF κ-constants = ode15s-class, variable order 1–5, the D-difference array ⇒ trajectory-exact candidate) over the **`OdeLinearSolver` seam** (dense LU now; the seam is the slice's architectural deliverable) + Newton with Jacobian-reuse policy (scipy/CVODE rules) + FD-Jacobian fallback. Gates: Robertson + VdP μ=1000 + HIRES + OREGO scipy-matched · **IVP-Testset reference solutions** (absolute anchors) · first CVODE work-precision harness. Sensitivity hooks reserved here. | ~600 | ~20 | — |
| ↳ **v9-e** (PLANNED) | **Radau IIA 5 — the implicit-RK gold standard.** scipy Radau exact-semantics port (= RADAU5): 3-stage collocation, the real+complex 2×2 eigen-transformation (**complex LU from hesap-dense — the named consumer edge**), embedded error + Gustafsson predictive controller, collocation dense output. Gates: scipy trajectory match · Hairer RADAU5 oracle · the stiff corpus · accuracy curves. | ~500 | ~15 | — |
| ↳ **v9-f** (PLANNED) | **Rosenbrock-W + SDIRK — the real-time stiff family.** Rosenbrock-W 4(3) (RODAS-class; NO Newton iteration, one LU + k solves/step = bounded per-step cost ⇒ THE DAW/real-time + MOL workhorse) + L-stable SDIRK pairs (KenCarp implicit cores). Exact + FD Jacobian modes. Gates: Hairer RODAS oracle work-precision · odeint rosenbrock4 wall-clock (same-language peer) · Robertson/VdP. | ~450 | ~15 | — |
| ↳ **v9-g** (PLANNED) | **Symplectic/geometric — the eylem pull (independent; any time after v9-a).** Velocity/position Verlet + semi-implicit Euler + **Yoshida 4/6/8** + Forest-Ruth, fixed-step, separable-Hamiltonian form. Gates: **10⁶-step Kepler/outer-solar-system energy-drift bounds** (bounded oscillation vs RK45's secular drift, shown honestly) · **time-reversibility bit-checks** (forward-then-backward recovers y₀) · order verification · odeint symplectic cross-check. = ADR-0063 replay determinism landing in hesap. | ~350 | ~12 | — |
| ↳ **v9-h** (PLANNED) | **Mass matrices + index-1 DAE.** M·y′ = f (constant/time-dependent, possibly singular ⇒ semi-explicit index-1) through BDF + Radau + **consistent initialization** (the calc_ic problem). Decide explicit-vs-residual form HERE with measurements. Gates: transistor amplifier (Testset reference) · index-1 pendulum · IDA cross-check (installed ✓). | ~400 | ~12 | — |
| ↳ **v9-i** (PLANNED) | **IMEX additive RK — the MOL/CFD pull.** ARS + KenCarp ARK pairs (explicit advection ⊕ implicit diffusion). Gates: Brusselator-1D/2D advection-diffusion MOL vs **ARKODE** (installed ✓) · per-part order verification. | ~350 | ~10 | — |
| ↳ **v9-j** (PLANNED) | **Sparse + Krylov Newton — where v9 meets the whole stack.** The `OdeLinearSolver` seam grows sparse-direct mode (CSR Jacobian → hesap-direct multifrontal with `reuse_symbolic`/`refactorize`) + matrix-free Krylov mode (jac-vec → hesap-iterative GMRES + the v4 preconditioner seam = CVODE SPGMR). Gates: Brusselator-2D/heat-2D MOL at n~10⁴–10⁵ **work-precision + wall vs CVODE-KLU and CVODE-SPGMR** · the {1..16} moat on parallel RHS/Jacobian + the inherited factor moat. THE showcase slice: ode→direct→iterative→jobs, all deterministic. | ~400 | ~12 | — |
| ↳ **v9-k** (PLANNED) | **Sensitivities — the control/opt pull.** Forward (staggered, CVODES pattern: augmented system sharing step sequence + Jacobian) + **adjoint** (checkpointed backward integration over dense output, CVODES ASA pattern). Gates: vs **CVODES** forward/adjoint on Robertson-with-parameters (published values) · FD cross-validation via the v7 `gradient_check` · the v7-b Dual forward-mode cross-check (THREE independent derivative oracles). | ~500 | ~15 | — |
| ↳ **v9-l** (PLANNED — **SLIP CANDIDATE**) | **Higher-index DAE: Pantelides + dummy derivatives.** Structural index analysis (bipartite matching) + Mattsson-Söderlind dummy derivatives ⇒ index-1 ⇒ the v9-h machinery. Gates: index-3 pendulum · car-axis (Testset) · slider-crank. Slip rationale: research-grade; the multibody/Modelica-class consumer is real but downstream (eylem constraints use their own stabilized formulations first). | ~500 | ~12 | — |
| ↳ **v9-z** (PLANNED) | **CLOSE.** CLI `hesap.ode.*` (canned Testset problems + method/tolerance selection; callables = API-level, the v7-z pattern) · **THE WORK-PRECISION SCOREBOARD** vs CVODE/IDA/ARKODE + scipy + odeint at matched tolerances over the canonical corpus (Robertson/VdP/HIRES/OREGO/Arenstorf/Pleiades/Brusselator-MOL/transistor) — FULL BOARD, all metrics together · {1..16} moat sweep · `docs/systems/hesap-ode.md` + ADR-0091 finalized + session logs. | ~350 | ~12 | — |
