...
| ↳ **v13-f** | **interp: gridded N-D + uncertainty.** bi/trilinear · bi/tricubic · tensor-product B-splines · Clough-Tocher C¹ (over Delaunay) · **★kriging / Gaussian-process** (returns mean **+ predictive variance** — the safety-critical sensor-fusion interpolant). Gate: scipy `RegularGridInterpolator`/`RectBivariateSpline`/`CloughTocher2DInterpolator` + sklearn GP (mean+var). **▶ N-linear ✅ (2026-06-29): `RegularGridInterpolant` general-dim 2^d-blend + bilinear/trilinear fast paths (branchless `locate` + precomputed reciprocal); 16 asrt ≤1e-12 vs scipy.RGI (2-D + 3-D) + exact bilinear-repro + clamp-no-NaN. ⭐ BENCH 3.65 ns/pt = 20×scipy · 3.1×MATLAB `interpn` · edges Boost `bilinear_uniform` 3.69 (both at the memory-latency floor). cubic/B-spline/Clough-Tocher/kriging ⏳.** | ~900 | 16+ | — |
| ↳ **v13-g** | **extend `crd-hesap-quadrature`: the integrate API + nodes + result contract.** Gauss-**Lobatto** + Gauss-**Radau** · the `integrate()`/`QuadResult`/`AdaptiveWorkspace`/`QuadStatus` surface · composite trapezoid/Simpson on samples · Newton-Cotes. Gate: exactness degree (Gauss-n ≤2n−1, Lobatto ≤2n−3) + **positive-weight invariant** + scipy `simpson`/`trapz`. | ~600 | ~30 | — |
| ↳ **v13-h** | **quadrature: the adaptive engine (real-time-safe).** **★Gauss-Kronrod** + the **ITERATIVE bounded-depth work-stack driver** (not recursive): QNG · QAG · **★QAGS** (+Wynn-ε, endpoint singularities — the default) · QAGP (break-points) · QAGI (infinite). Gate: QUADPACK/`scipy.quad` + `\|G−K\|` estimate + **no-recursion/fixed-workspace structural guard** + the Lyness-Kaganove hidden-peak honesty test. | ~900 | ~40 | — |
| ↳ **v13-i** | **quadrature: non-Gauss + double-exponential.** **★Clenshaw-Curtis**/Fejér (Chebyshev points, FFT weights, nested, positive) · **★tanh-sinh / exp-sinh / sinh-sinh** (DE — endpoint singularities & infinite ranges & arbitrary precision) · Romberg. Gate: Boost.Math `tanh_sinh`/`exp_sinh` + Clenshaw-Curtis accuracy-per-eval + singular endpoints. | ~800 | ~35 | — |
