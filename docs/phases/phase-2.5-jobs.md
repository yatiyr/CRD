...
| v1k | Integration smoke + crd-app wiring | ✅ | `smoke_jobs` rewritten as full public API demo (init/shutdown, run+wait, parallel_for sum, H/N/L all-ran, frame_alloc/reset); `Application::run()` calls `jobs::init(m_desc.jobs_config)` / `jobs::shutdown()` with `!m_valid` guard; `crd-app` CMakeLists gains PUBLIC `crd-jobs` link; `ApplicationDesc` gains `jobs_config` field; `smoke_renderer` runs clean (exit 0) with job system wired into Application; all 6 configs green (355/355 debug, 352/352 release) |
