...
**Status:** ✅ **CLOSED 2026-05-10** — all 17 slices shipped (v1a–v1p). Phase 3.0 delivered the full 8-layer slot-shaped ECS foundation: SlotMap → ComponentRegistry → Archetype + SparseSet hybrid storage → mixed-backend visitor → relations (6 built-ins) → query DSL → 7-phase Schedule + Commands → IComponentIndex framework + ChangeDetect + AsyncAware (+ 5 reserved no-op shells) → Transform + propagation → SceneResource + cooker → Öbek system (12 sub-slices) → Preset + Profile (6 sub-slices) → async GPU upload + sandbox integration (3 sub-slices) → reserved-slot + API surface freeze. **856 unit tests across 12 build configs.** All 13 ADRs (0049–0061) realised.
**ADRs:** ADR-0049, ADR-0050, ADR-0051, ADR-0052, ADR-0053, ADR-0054, ADR-0055, ADR-0056, ADR-0057, **ADR-0058 (Öbek)** ✅ realised, **ADR-0059 (Preset), ADR-0060 (Profile)** ✅ realised v1n, **ADR-0061 (Async GPU upload contract)** ✅ realised v1o1+v1o2 (locked 2026-05-09)
...
| v1n | **Preset + Profile system** — `PresetResource` + per-type `PresetLoader` + `PresetRegistry`; first concrete types `QualityPreset` (`'PRQL'`) + `CameraPreset` (`'PRCM'`); five-layer resolution stack (default → extends → preset → instance → runtime); `ProfileResolver` with closed typed predicates (os / gpu_tier / domain / mode / target_fps / cpu_cores); additive profile composition (priority-sorted stack, Cerid-unique vs Unreal first-match-wins); runtime context detection. 6 sub-slices (v1n1..v1n6), 24 tests, 245 assertions. | ✅ shipped 2026-05-09 |
| v1n1 | **Preset substrate** — new `crd-preset` module: `PresetResource` (PINF/PDAT/PCHN chunk model) + `PresetLoader` (per-type, runtime-configured by FourCC + schema_version + payload_size) + `PresetRegistry::register_type<T>()` (closed-by-C++-types, idempotent re-registration) + `PresetArtifactBuilder` (test-only public; cooker promotes in v1n2+) + `IPresetTarget` empty base + `PresetApplyEvent`. No concrete types yet (those are v1n2 / v1n3). 5 tests, 38 assertions. | ✅ shipped 2026-05-09 |
| v1n2 | **`QualityPreset` first concrete type** — schema struct (FourCC `'PRQL'`, version 1, 144 B) with `shadow_resolution` / `msaa_samples` / `ssr_quality` / `ssao_quality` / `post_fx_count` / `post_fx[8]`; `IPresetTarget::apply(const QualityPreset&)` overload added to base; binary layout pinned via `static_assert(sizeof == 144 && alignof == 8)`. 4 tests, 38 assertions. | ✅ shipped 2026-05-09 |
| v1n3 | **`CameraPreset` second concrete type** — schema struct (FourCC `'PRCM'`, version 1, 40 B) with `fov_y_radians` / `near_plane` / `far_plane` / aperture/shutter/iso/EV-comp / `ev100_min` / `ev100_max` / `LensModel` enum / `ExposureMode` enum; `IPresetTarget::apply(const CameraPreset&)` overload added; partial-override convention pinned (`using IPresetTarget::apply;` required by GCC `-Woverloaded-virtual`, documented in `preset_target.hpp`). 3 tests, 39 assertions. | ✅ shipped 2026-05-09 |
| v1n4 | **Five-layer resolver primitive** — `resolve_preset<T>(resource, runtime_override)` + `apply_preset<T>(target, resource, runtime_override)` template helpers in `preset_resolver.hpp`. Captures ADR-0059 §2: L0 schema default, L1+L2 fold into the cooked PDAT bytes (cooker pre-resolves extends chain deepest-first, ADR-0059 §3 — same Öbek resolver as v1m3b), L3 reserved (Phase 4+), L4 caller-supplied full-T override. Compile-time-checked schema contract via static_assert on `T::fourcc` / `T::version`; runtime FourCC + size check via CRD_ASSERT. 4 tests, 36 assertions. | ✅ shipped 2026-05-09 |
| v1n5 | **Profile substrate (ADR-0060)** — new `crd-profile` module: `ProfileContext` + closed enums (`OperatingSystem` / `GpuTier` / `ProjectDomain` / `AppMode`) + closed predicate schema (`PredicateField` / `PredicateOp` / `PredicateRecord` 8 B layout pinned) + runtime `Profile` struct (priority + predicates + apply_bundle of `ResourceId`s) + `ProfileResource` payload + `ProfileLoader` (FourCC `'PROF'`) + `ProfileArtifactBuilder` test-public; CRDR layout FINF (16 B) + FRLE (rule entries) + FBND (bundle entries with rule_idx cross-link). v1n5 enforces 1 bundle per rule; resolver + additive composition + context detection land in v1n6. 4 tests, 79 assertions. | ✅ shipped 2026-05-09 |
| v1n6 | **Profile resolver — closes v1n** — `evaluate_predicate(record, ctx)` (canonical signed-i64 widening for sign-preserving integer + enum compare; Equal / GreaterEq / LessEq / InMask), `ProfileResolver::resolve(ctx, out)` (match → priority-stable-sort → concat bundles in priority-ascending order; ADR-0060 §3 additive composition), `detect_os()` + `detect_cpu_cores()` helpers (GPU tier deferred to consumers — RHI dep too heavy for the leaf module). NO `apply_profile_bundle` driver — that's the v1o3 sandbox bridge to `IPresetTarget*`. 4 tests, 50 assertions. | ✅ shipped 2026-05-09 |
| v1o | **Async GPU upload contract (ADR-0061) + Sandbox integration with Öbek + Preset + Profile** — 3 sub-slices: **v1o1** `crd::rhi::Fence` + non-waiting `Queue::submit(cmd, fence)`; **v1o2** `UploadHandle` + `GpuUploader::upload_*_async` + `PendingMeshUpload` component + `RenderUploadSystem` (RenderExtract); **v1o3** sandbox uses async upload + profile + öbek end-to-end with full cooker integration. | ✅ shipped 2026-05-09 (v1o1 + v1o2 + v1o3) |
| v1o1 | **`crd::rhi::Fence` + non-waiting submit** — new `Fence` interface (`is_signaled()` non-blocking + `wait()` blocking + `reset()`) added to `crd-rhi`; `Device::create_fence()` factory; `Queue::submit(CommandBuffer&, Fence&)` non-waiting variant. Vulkan backend wraps `VkFence` (vkCreateFence / vkWaitForFences / vkResetFences / vkDestroyFence + vkQueueSubmit with fence). Fake-backend Fence + extended Queue/Device for unit tests; smoke + renderer-test fakes updated to satisfy the new pure virtuals. 5 tests (4 fake + 1 real Vulkan), 17 assertions. | ✅ shipped 2026-05-09 |
| v1o2 | **`UploadHandle` + async GpuUploader + RenderUploadSystem (ADR-0061 Layer 2 + Layer 3)** — `UploadHandle` move-only handle in `crd-renderer` owning the in-flight Fence + staging buffers + recorded CommandBuffer + a pending `GpuMesh`/`GpuTexture`; `is_valid()` / `is_ready()` (non-blocking) / `wait()` / `take_mesh()` / `take_texture()`. `GpuUploader::upload_mesh_async` + `upload_texture_async` record-and-submit-with-fence path returning `UploadHandle`. ECS-side `PendingMeshUpload { UploadHandle handle }` component + `RenderUploadSystem` (RenderExtract phase): polls `(Renderable, PendingMeshUpload)` query, on `is_ready()` moves the GpuMesh into a per-system `HashMap<EntityId.raw, GpuMesh>`, patches the Renderable's vertex/index buffer pointers, calls `AsyncAwareIndex::mark_loaded`, and queues a `Commands::remove_component<PendingMeshUpload>`. The standard `query<...>().skip_pending<Renderable>()` filter automatically excludes pending entities until promotion. 6 tests, 26 assertions. | ✅ shipped 2026-05-09 |
| v1o3 | **Sandbox integration: drop-callback hook + ForwardRenderPath as IPresetTarget + cookers + content** — full end-to-end: (1) `RenderMeshIndex` (new `IComponentIndex`) takes ownership of `RenderUploadSystem`'s mesh store; observes `Renderable` lifecycle and evicts the resident `GpuMesh` automatically on `World::destroy(e)` — the proper drop-callback hook v1o2's session log promised. (2) `QualityPreset` bumped to schema v2 (one byte of `_reserved` repurposed → `enable_depth_prepass`; layout still 144 B). (3) `ForwardRenderPath` implements `crd::preset::IPresetTarget`; `apply(QualityPreset)` caches the preset and `enable_depth_prepass = 0` skips the depth-prepass DRAWS while keeping the pass declaration (FrameGraph topology invariant). (4) Three new cooker handlers — `.preset.toml`, `.profile.toml`, `.obek.toml` — registered with the asset cooker; profile cooker resolves bundle entries via sibling `.meta` sidecars. (5) Demo content: `quality_default.preset.toml` + `camera_default.preset.toml` + `default.profile.toml` + `obek_demo.obek.toml`. (6) Sandbox now owns a `crd::scene::World` driving a single ECS-based render path (procedurals + imports both spawn entities; imports go through async upload + `PendingMeshUpload`); profile resolves at boot and the preset bundle drives `ForwardRenderPath` + a sandbox-side `SandboxCameraTarget`; ImGui panels expose Quality + Camera L4 runtime overrides plus Öbek instantiate / override-translate / revert / unpack. (7) `wsl-build.ps1` redirects Linux build dirs to `~/cerid-build/<preset>` on native ext4 (drops 9P-bridge tax). 2 new tests (RenderMeshIndex drop-callback + manual release), QualityPreset round-trip updated for v2. **Closes Phase 3.0 v1o.** | ✅ shipped 2026-05-09 |
| v1p | **Reserved-slot freeze — closes Phase 3.0** — `ScriptComponent` + `ScriptHandle` types added under `crd/scene/script_component.hpp` (16 B / 8 B layouts pinned, ADR-0056 §2 — registers as a regular SparseSet component, not a trait). Reserved spatial DSL operators `Query<>::in_aabb(AABB)` + `Query<>::within_radius(center, radius)` shipped as passthrough no-ops behind `IRenderPath`/`SpatialBVHIndex` consumer (ADR-0053 §6) — chain-friendly today, start filtering when Phase 3.5 wires the BVH. New test file `test_phase_3_0_freeze.cpp` (5 cases): every reserved trait round-trips through `register_component`; auto-register fires every shell index in one call; `ScriptComponent` registers + spawns + reads back; spatial operators compile + chain + iterate; **API surface freeze** captured as compile-time `static_assert` pins on every public sizeof / alignof / FourCC / version + closed-enum value the cooker writes and the loader reads (Öbek info+descriptors / QualityPreset v2 / CameraPreset v1 / ProfileFileInfo / PredicateRecord / InheritPolicy / Replication / PredicateField / PredicateOp / ScriptHandle / ScriptComponent). | ✅ shipped 2026-05-10 |
