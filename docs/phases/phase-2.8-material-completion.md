...
**Scope:**
- `assets/source/BoxTextured.glb` (CC0, Khronos), `assets/source/Duck.glb` (Apache 2.0, Khronos),
  `assets/source/Suzanne.glb` (CC0, Blender).
- `assets/source/checker_512.png` (CC0, synthetic), `assets/source/bricks_512.png`
  (CC0, ambientCG.com).
- `assets/source/LICENSES.md` — license table for each file.
- CMake custom target `cook-demo-assets`: runs the cooker on each `.glb` and `.png` to produce
  `.crdr` packs under `assets/cooked/`.
- The cooked assets are loaded by `SandboxLayer` via the existing `ResourceManager`.

**Tests:**
- `cook-demo-assets` target produces valid `.crdr` files (cooker exits 0, manifest written).
- `ResourceManager` loads `BoxTextured.crdr` synchronously; `MeshResource` has at least one primitive.
...
**Scope:**
- Replace the Meshgen Browser ImGui panel with a unified **Asset Browser** panel:
  - **Procedural shapes** section: 8 meshgen shapes (same as current Meshgen Browser).
  - **Loaded assets** section: BoxTextured, Duck, Suzanne (from `assets/cooked/`).
- Click any item (procedural or asset): upload/load the mesh, swap the rendered object.
- Per-selection metadata: vertex count, index count, triangle count, source (procedural / glTF).
- Existing panel positions and sizes adjusted to accommodate the wider list.

**Tests:**
- Switching between a meshgen shape and a glTF asset does not leak GPU memory (VMA report checked
  in the GPU smoke).
- Asset browser panel renders all 11 entries (8 procedural + 3 glTF) without crash.
