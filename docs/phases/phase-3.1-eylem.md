...
| **v1a-material** | Materials substrate per ADR-0069 (✅ Accepted 2026-05-11). 64-byte cache-line `Material` struct (locked at v1a freeze): 6-value `FrictionModel` (Coulomb / Stribeck / LuGre / Karnopp / Anisotropic / FrictionTriple) + 3-value `RestitutionModel` (Constant / Newton / Hunt-Crossley) + surface velocity + density + per-collider material via `MaterialId` handle (PhysX / Jolt pattern; FNV-1a-64 content-addressed) + 5 combine modes (Average / Min / Max / Multiply / **GeometricMean** new — Box2D v3 / Jolt / Unity DOTS / AGX consensus default for friction). LuGre per-contact bristle state folded into ADR-0068 §8 contact warm-start cache (16B lazy alloc); Tustin discretisation with cooker-precomputed division-free constants. **Must close before v1a-b interface freeze** — gates downstream API surface (vehicles need anisotropic friction, robotics need LuGre, cinematic need Hunt-Crossley). 8 deferred sub-slices fill formula impls inside the frozen surface (v5 vehicles ship Stribeck/LuGre/Karnopp/Anisotropic/FrictionTriple; v7 FEM ships Hunt-Crossley; v8d MPM ships Newton; post-v1 destruction consumes `yield_stress` reservation). Per ADR-0069. Research dossier: `docs/research/cerid-eylem-materials.md`. | ~150 | ~5 |
...
| **v1c-sensor** | ✅ shipped 2026-05-11 (collider.hpp surface) — `ColliderFlags::is_sensor` per-collider flag (NOT per-body, per ADR-0068 §10.1 / §10.2 — Jolt's per-body model rejected for multi-domain mandate). Scene-component round-trip + öbek serialisation. Per ADR-0068 §2. | ~50 | ~3 |
