...
**Status:** 🚧 v1b in flight (started 2026-05-10; v0 ✅ + v1a ✅ + v1a-draw d0..d4 ✅ + v1b-a/b/c/d ✅ + v1a-material a/b/c/d ✅ + v1b-e 🚧 sweep running 2026-05-11). **⏸ PAUSED at v1b close — Phase 3.1.7 `crd-geometry` executes next (ADR-0076 §12 amendment 2026-05-11), then v1c resumes consuming geometry from day 1 with no deferred-refactor.**
...
| **v1d** | **Narrow-phase substrate + ConvexHull + Plane.** GJK distance + EPA penetration over a `support<T>(d)` template family (specialise per shape); Sutherland-Hodgman manifold reduction; SAT fast path for box-box. Regression battery of pre-baked contact pairs. Locks the narrow-phase architecture; v1d-mesh + v1d-hf fold their per-shape contact gen into the same dispatch. **Consumes `crd-geometry-convex` (GJK + EPA + SAT) directly** (post-§12 amendment); the original "ships own narrow GJK+EPA, refactor later" reservation is OBSOLETE. | ~900 | ~25 |
