...
**Status:** 🚧 active (started 2026-05-10; v0 ✅ + v1a ✅ + v1a-draw d0..d4 ✅ + v1b-a/b ✅ + v1a-material a/b/c/d ✅ — material substrate cluster CLOSED 2026-05-11; v1b-c next)
...
| **v1b-c** | `RigidBodyComponent` + `ColliderComponent` declared in `crd-eylem` interface (SparseSet storage hint per ADR-0050) + on_add hook allocates pool slot + on_remove hook frees pool slot. `EylemSystem : ISystem` registered in `SchedulePhase::Physics`, `fixed_step() = true`. Step body: `linvel += gravity·dt; pos += linvel·dt; rot integrated from angvel via small-angle quaternion update` → write back to `Transform` via `set_translation` / `set_rotation_quat` (so TransformPropagation marks dirty + propagates by PreRender). **First eylem entity integrating motion in the ECS world.** **Next slice.** | ~150 | ~3 |
| **v1b-d** | `crd-eylem-viz` companion module (deferred from v1a-draw d3 because needs RigidBodyComponent / ColliderComponent which land in v1b-c — registering visualizers for non-existent components would be a stub target, violates quality bar). Depends on `crd-eylem` + `crd-eylem-rigid3d` + `crd-draw`. `register_eylem_visualizers(VisualizerRegistry&)` registers: RigidBodyComponent → axis triad (always) + velocity arrow (when ShowVelocity flag); ColliderComponent → wireframe matching the shape kind (sphere_wire / box_wire / capsule_wire from crd-draw). Joint visualizer registered as no-op shell (joints land v1f). Keeps `crd-eylem` itself free of any rendering dep via dependency-inverted plug-in. | ~150 | ~2 |
| **v1b-e** | Sandbox demo: spawn 3-5 rigid bodies (1 sphere, 1 box, 1 capsule) with `Transform + RigidBodyComponent + ColliderComponent + DebugVizComponent{AxisTriad+Wireframe+ShowVelocity}` at +y=5. Gravity wired. Smoke verifies bodies' Transform.translation.y descends frame-over-frame. **First eye-test of eylem doing physics in the renderer.** Bodies fall through the world because v1d collision hasn't shipped yet — that's the honest framing. | ~100 | smokes |
...
| **v1d** | **Narrow-phase substrate + ConvexHull + Plane.** GJK distance + EPA penetration over a `support<T>(d)` template family (specialise per shape); Sutherland-Hodgman manifold reduction; SAT fast path for box-box. Regression battery of pre-baked contact pairs. Locks the narrow-phase architecture; v1d-mesh + v1d-hf fold their per-shape contact gen into the same dispatch. | ~900 | ~25 |
