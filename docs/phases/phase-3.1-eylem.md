...
|---|---|---|---|
| **v0a** | SIMD wrapper types: `Vec4f` / `Vec8f` / `Mat4f` / `Quatf` with backend selection (SSE2 / AVX2 / NEON / scalar) at compile time. Round-trip + correctness tests vs scalar reference. | ~600 | ~25 |
| **v0b** | AoSoA storage helpers: `Soa<T, N>` template, gather / scatter, `soa_for_each` lambda iterator that lane-utilises automatically. | ~400 | ~15 |
| **v0c** | `crd::math::deterministic` — Cephes-style polynomial sin/cos/tan/atan2/asin/acos/exp/log/pow. Bit-exact CI test across MSVC/clang/gcc × x64/ARM. (ADR-0063 §2.) | ~700 | ~30 |
| **v0d** | `crd::containers::sort` / `stable_sort` / `nth_element` / `push_heap` / `pop_heap` / `make_heap` — pdqsort-derived with pinned tie-breaker (compare by `(key, original_index)`). Lint check banning `std::sort` etc. in `engine/eylem/**`. (ADR-0063 §3.) | ~500 | ~20 |
| **v0e** | Math + containers benchmark harness: micro-bench AoSoA-8 vs scalar on Vec3 dot / cross / Mat4 multiply / quaternion compose. Goal-line numbers logged for regression detection. | ~400 | ~10 |
