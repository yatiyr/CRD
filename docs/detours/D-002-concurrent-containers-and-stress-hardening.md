...
- `tests/foundation/stress/` — a reusable stress harness (v0): jobs-driven runner over real fibers + a
...
|---|---|---|
| v0 | Stress harness | **implemented 2026-05-12; pending full-sweep.** `tests/foundation/stress/` — `stress_harness.hpp` (jobs-fiber mode + `std::thread`-mirror mode, splitmix64 seeded per (base,worker,round), `FailSink` thread-safe failure recorder + `CRD_STRESS_FAIL_IF` / `CRD_STRESS_ORACLE_OK`, round-based quiescent oracle, `bounded()` / `soak()` configs), `main_stress.cpp` (crash dumps + one-shot `jobs::init`), `test_containers_stress.cpp` (disjoint-write Array, many-readers, isolated-TlsfAllocator+HashMap churn — each ×{fibers,threads}), `test_allocators_stress.cpp` (isolated TlsfAllocator alloc/free/realloc churn with per-block pattern verification; `[.soak]` variant), `tests/foundation/stress/CMakeLists.txt` + wired into `tests/CMakeLists.txt`. Clean build + 4/4 pass on **win-debug and win-asan** (incl. a negative-test pass — a deliberately out-of-slice write was caught by the disjoint-write oracle). **TODO before close: win-clang-cl (verify `make_job` SBO size holds under a different lambda layout) + the full 14-config `scripts/full-sweep.ps1`.** |
...
- `crd::jobs::detail::MpmcQueue<T>` (`engine/foundation/jobs/src/mpmc_queue.hpp`) is already mostly generic
