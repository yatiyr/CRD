...
| ✔ | Slice | Deliverable | → §§ |
|---|---|---|---|
| ◧ | CEIR-1a | ✅ **CORE LANDED + GATED (win-debug, 2026-08-08):** new module `engine/ceir` (`crd-ceir`, host-only, deps core/log/memory/containers/units per ADR-0109) — `Context`/`Module`/`Operation`/`Value`/`Block`/`Region` + **intrusive in-arena def-use** (MLIR-shape `Use`; RAUW = splice, zero-alloc) + O(1) intrusive op/block edit + `SourceLoc` field from day one. The growable bump **arena is `crd::memory::GrowableLinearAllocator`** — it lives in **crd-memory** (its proper home; the growable sibling of `LinearAllocator`), NOT a crd-ceir-private `detail/Arena`, with its own **7/7 boundary-adversary gate** in `tests/memory` (chunk growth · oversized-own-chunk · exact fill-to-tail · reset-reuse · no-per-alloc-malloc-within-a-chunk · dealloc-noop · reallocate-copies · destructor-frees-every-chunk). `tests/ceir` **7/7 green** incl. the ⭐ **no-per-op-malloc gate** (32 ops, 0 parent allocs) + def-use/RAUW/set_operand/erase/insert_before/boundaries; all files **tidy-clean**. **I3/I5 grep-gates LANDED** — `crd-ceir-invariants` ctest (`scripts/check_ceir_invariants.{ps1,sh}`, registered both OS blocks): I5 = link-edge whitelist (host-only), I3 = no forbidden-module include; **proven to bite** on a negative fixture (both violations → exit 1). REMAINING for 1a close: the 4-config per-slice sweep (win-debug ✓; asan/shipping/tidy in flight). | §12 §13 |
