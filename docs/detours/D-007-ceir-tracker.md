...
> | **Landed history** — [`D-007-gpu-program-system.md`](D-007-gpu-program-system.md) | The CKIR/RAF/RET ledger. Its live-tracking role moves HERE at CEIR-0f. | For "how did we get here." |
>
...
**Naming note:** C++ type/op names below are the working proposal; the **CEIR-0c** ownership ADR finalizes them.
Everything else in a row is binding.
...
|---|---|---|---|---|
| ⬜ | CEIR-0a | **Execution-path inventory FROM CODE** (never from docs). Sweep ≥: every `ExecutorTypeId` registrant + `AuthoredPass`/`run_authored_cb` path in `engine/render-pass`/`render-graph`; `crd::rendergraph::execute_frame` + the frame-cook Cooked→Template bridge (ADR-0106); `scene_renderer.cpp` orchestration + its hand-list (RAH-7 target); the five cookers' outputs (`.frame.toml`→blob · `.crdt` · `.crdv` · `.crdl` · `.crdm`) + `.crdr` 'SHDR'/VART program classes (ADR-0104); `IComputeContext` dispatch consumers (`kir-cuda`, bvh-gpu Morton, FFT/sort/NRC harnesses); `crd-draw` submit_overlay; media decode sites. Each: file:line, currency in/out, classified **composite algorithm** (→ CEIR asset @ CEIR-13) vs **atomic native capability** (→ intrinsic @ CEIR-0d) | table w/ file:line evidence; zero "per the docs" entries | §127 §128 |
| ⬜ | CEIR-0b | **Scripting-direction ADR draft**: supersedes the ADR-0081 "C++-only scripting" cornerstone. States: C++ stays first-class native + hot-reload; Cerid gains an OWNED textual/visual language stack; no Lua/Python/JS runtime; CLI/RPC/MCP stays source-of-truth; program assets are agent-authorable. Pins the language non-negotiables NOW: no mandatory GC in hot paths, deterministic time/RNG, capability security, C++ FFI. ⛔ PRINCIPLES/AGENTS/README untouched until accepted AND a vertical slice exists | ADR drafted, PENDING USER REVIEW | §5 §98 §99 |
| ⬜ | CEIR-0c | **CEIR/CHIR/CKIR ownership ADR draft**: layer contract (CEIR=execution/orchestration; CKIR=per-invocation kernels, unchanged; CHIR=reserved language layer → CEIR-29); lowering directions (CHIR→CEIR→CKIR/providers, never sideways); module placement — **`engine/ceir` (crd-ceir), host-only foundational module, deps `crd-core/log/memory/containers/units` ONLY** (GPU contact only via providers in later bands ⇒ acyclic one-way graph); finalized C++ names for the band-1 types; the semantic-identity model (stable semantic IDs · source spans · layout-metadata separation) | ADR drafted, PENDING USER REVIEW; acyclicity argued | §2 §4 §85 §10 |
| ⬜ | CEIR-0d | **Native-intrinsic ADR draft**: the registration schema (stable ID · version · typed I/O · effects · domain · determinism · thread safety · lifetime · capabilities · provider · cost hints · hot-reload compat · debug metadata); the legitimacy rule with examples (swapchain acquire/present = legitimate; `ForwardPlusExecutor` = forbidden); the three plugin extension levels (A subgraph-only / B custom op + lowering / C new native capability) | ADR drafted, PENDING USER REVIEW | §100 §101 |
| ⬜ | CEIR-0e | **CHIR-0 language design note** (design-only): the §98 feature set; ownership-model options weighed (value semantics · borrows · arenas · handles · state stores); syntax direction sketches; the text/visual projection + semantic-identity model; explicit statement that implementation waits for the CEIR corpus (CEIR-29) | design note, PENDING USER REVIEW; ZERO implementation | §4 §98 §19 §10 |
| ⬜ | CEIR-0f | **D-007 restructure**: CEIR spine section into `D-007-gpu-program-system.md`; old 122-subslice table re-hung (RPL→CEIR-15 · MLR→CEIR-21 · frame/executor→CEIR-12/13 · I2D effects→CEIR-28 · hesap-GPU→CEIR-19 · D7E→CEIR-30); THIS file wired as the one live tracker (context.md + ROADMAP pointers); the **ADR-0106 supersession plan** written (strike-in-place executes @ CEIR-12f) | exactly one live tracker; no row tracked twice | §175 §159 |
| ⬜ | CEIR-0g | Maturity merge: post-RAF L0–L7 (§PR-3) ↔ CEIR L0–L8 → ONE model; `docs/capabilities/gpu-platform-capabilities.toml` → the §174 manifest shape (feature · maturity · assets · tests · providers · fallbacks · determinism tier · board) | one maturity model; registry migration plan | §173 §174 |
| ⬜ | CEIR-0h | Migration + explicit **DELETION** tables: frame paths (§126 steps 1–8), executors (from 0a), privileged programmatic paths, stale hand-lists. Every deletion names its parity gate FIRST (the-deletion-is-the-proof) | tables complete; each deletion has a named gate | §126 §127 §178 |
| ⬜ | CEIR-0z | Close: the **§184 fifteen-item report** + honest per-band KLOC/duration sizing for CEIR-1…13 (the mission doc deliberately doesn't estimate; this is where estimates become real) | all 15 items answered from evidence; user sign-off | §184 |

## CEIR-1 — Core IR substrate — ⬜  → §175 band 1 · core: §12 §7
...
| ✔ | Slice | Deliverable | → §§ |
|---|---|---|---|
| ⬜ | CEIR-1a | `Context`/`Module`/`Op`/`Value`/`Block`/`Region` with arena storage + def-use chains; operands/results as spans into arena arrays (no per-op heap) | §12 §13 |
| ⬜ | CEIR-1b | `SymbolTable` + `ceir.func` core: `func.func`/`func.call`/`func.return`, visibility, cross-module symbol refs | §34 |
| ⬜ | CEIR-1c | Interned typed attributes/properties + `SourceLoc` provenance on EVERY op from day one (retrofitting provenance is how source maps die) | §111 |
| ⬜ | CEIR-1d | Dialect registry + op interfaces/traits: dialects register `(name → op set, verifier, printer/parser hooks)`; analyses dispatch via interfaces. ⛔ grep-gate: no central `switch` over cross-dialect ops; unknown-dialect ops survive load/print opaquely | §6 §7 §101 |
| ⬜ | CEIR-1e | Deterministic textual printer + parser; `print(parse(print(x)))==print(x)` byte-exact; graph layout metadata separated from semantics | §10 §166 |
| ⬜ | CEIR-1f | Binary serialization: FourCC chunks, explicit version word, field-by-field encode/decode, forward-skip of unknown chunks | §104 §123 |
| ⬜ | CEIR-1g | `ModuleBuilder` fluent C++ API — emits ordinary canonical IR through the same verifier (⛔ no privileged bypass) | §121 |
| ⬜ | CEIR-1h | **Harness seed, built NOW:** round-trip fuzz (random valid modules → print→parse→print byte-compare; binary→load→save byte-compare), malformed-input rejection corpus, stable-hash test | §119 §167 |
| ⬜ | CEIR-1z | **Gate:** typed hello-world (func + const + call + return) round-trips text ⇄ binary ⇄ builder byte-identically; fuzz corpus green in ctest; guard greps registered | §167 |
...
| ✔ | Slice | Deliverable | → §§ |
|---|---|---|---|
| ⬜ | CEIR-2a | The `*.ceirop.toml` schema format + its validator (a schema error is a cook-time diagnostic pointing at the TOML line — the declared-contract rule) | §8 |
...
| ⬜ | CEIR-6b | `ceir.task` ops + the crd-jobs provider (the first `IExecutionProvider`, host-only) | §38 §31 §69 |
...
(`gpu-context` `command_model.hpp` / `detail/command_lowering.hpp`) and `IComputeContext` — CEIR orchestrates the
proven layer, it does not invent GPU plumbing (§158). CKIR by identity (§85): `compute.dispatch @kernel` references
...
| ⬜ | CEIR-10z | **Proof (§129):** `add`·`reduce`·`scan`·FFT as CEIR program ASSETS — authored text AND builder, dispatched Vulkan AND DX12, ValidationCapture-silent, bit-exact vs the existing CPU oracles, hot-reload live-swaps, reference executor validates host-visible semantics | §129 §170 |
