...
TEST_CASE("v12-q: MCMC diagnostics (R-hat / ESS / autocorr / Geweke) vs ArviZ", "[v12-q][stats][mcmc]")
{
    ...
    constexpr crd::usize k_m = 4;
    constexpr crd::usize k_n = 500;
    crd::containers::Array<double> ch(&alloc);
    ch.resize(k_m * k_n);
    for (crd::usize c = 0; c < k_m; ++c)
    {
        ch[c * k_n] = static_cast<double>(c);
        for (crd::usize t = 1; t < k_n; ++t)
        {
            const double r = (static_cast<double>(lcg(c * 100000 + t) % 2000) - 1000.0) / 1000.0;
            ch[c * k_n + t] = 0.7 * ch[c * k_n + t - 1] + r;
        }
    }
    const auto chains = ConstSpan<double>{ch.data(), k_m * k_n};
    const auto c0 = ConstSpan<double>{ch.data(), k_n};
    ...
    CHECK(close(rhat(chains, k_m, k_n, &alloc), 0.999780540060195, 1e-6));    // ArviZ az.rhat (rank)
    CHECK(close(ess_bulk(chains, k_m, k_n, &alloc), 808.485534731799, 1e-2)); // ArviZ az.ess (bulk)
}
...
TEST_CASE("v12-q: samplers recover N(0,1) + leapfrog + determinism moat", "[v12-q][stats][mcmc]")
{
    ...
    const auto logp = [](ConstSpan<double> x)
    {
        return -0.5 * x[0] * x[0];
    };
    const auto grad = [](ConstSpan<double> x, crd::containers::Span<double> g)
    {
        g[0] = -x[0];
    };
    ...
    {
        constexpr crd::usize k_k = 4;
        constexpr crd::usize k_nn = 5000;
        crd::containers::Array<double> pooled(&alloc);
        pooled.resize(k_k * k_nn);
        for (crd::usize c = 0; c < k_k; ++c)
        {
            const auto ch = run_chain(static_cast<crd::u64>(100 + c));
            for (crd::usize i = 0; i < k_nn; ++i)
            {
                pooled[c * k_nn + i] = ch[i];
            }
        }
        const auto ps = ConstSpan<double>{pooled.data(), k_k * k_nn};
        CHECK(within(mean(ps), 0.0, 0.07));
        CHECK(within(variance(ps, 1), 1.0, 0.12));
        CHECK(rhat(ps, k_k, k_nn, &alloc) < 1.05);
    };
...
TEST_CASE("v12-q: Gibbs recovers a correlated 2D Gaussian", "[v12-q][stats][mcmc]")
{
    ...
    constexpr crd::usize k_nn = 20000;
    constexpr crd::usize k_b = 1000; // burn-in
    const auto ch = gibbs(cond, x0, k_nn, 42, &alloc);
    const crd::usize ns = k_nn - k_b;
    double m0 = 0;
    double m1 = 0;
    for (crd::usize s = k_b; s < k_nn; ++s)
    {
        m0 += ch[s * 2];
        m1 += ch[s * 2 + 1];
    }
...
{
    crd::memory::TlsfAllocator alloc(static_cast<crd::usize>(1) << 24);
    const auto log_prior = [](ConstSpan<double> x)
    {
