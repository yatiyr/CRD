...
template <typename T> SparseMatrix<T, SparseFormat::Csr> tall(crd::memory::IAllocator* a, crd::u32 m, crd::u32 n)
{
...
TEST_CASE("LSQR solves a complex least-squares problem (c64)", "[hesap-iterative][lsqr][complex]")
{
    ...
    for (crd::u32 i = 0; i < n; ++i)
    {
        ...
        if (i + 1 < n)
        {
            bld.add(i, i + 1, C{-1.0, 0.2});
        }
    }
    for (crd::u32 k = 0; n + k < m && k < n; ++k)
    {
        bld.add(n + k, k, C{1.0, -0.3});
    }
    auto a = bld.compress();
...
TEST_CASE("LSMR is bit-exact over serial vs parallel spmv (determinism moat)", "[hesap-iterative][lsmr][determinism]")
{
    crd::jobs::init();
    {
...
template <typename T>
SparseMatrix<T, SparseFormat::Csr> column_scaled_tall(crd::memory::IAllocator* a, crd::u32 m, crd::u32 n,
                                                      double decades)
{
    TripletBuilder<T> b(a, m, n);
    for (crd::u32 i = 0; i < n; ++i)
    {
        ...
        {
            const T s1 =
                static_cast<T>(std::pow(10.0, decades * static_cast<double>(i + 1) / static_cast<double>(n - 1)));
            b.add(i, i + 1, T(-1) * s1);
        }
        if (i > 0)
        {
...
TEST_CASE("LSQR is bit-exact over serial vs parallel spmv (determinism moat)", "[hesap-iterative][lsqr][determinism]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator alloc{64U << 20};
