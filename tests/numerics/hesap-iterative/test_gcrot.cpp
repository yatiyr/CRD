...
template <typename T>
SparseMatrix<T, SparseFormat::Csr> cluster_shifted(crd::memory::IAllocator* a, crd::u32 n, crd::u32 nsmall,
                                                   double shift)
{
    TripletBuilder<T> b(a, n, n);
    for (crd::u32 i = 0; i < n; ++i)
...
TEST_CASE("GCROT cross-solve recycling saves iterations across a shift sequence", "[hesap-iterative][gcrot][recycle]")
{
    crd::jobs::init();
    {
