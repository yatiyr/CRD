...
template <typename T> SparseMatrix<T, SparseFormat::Csr> laplacian2d(crd::memory::IAllocator* a, crd::u32 g)
{
    const crd::u32 n = g * g;
...
TEST_CASE("IC(0) on a tridiagonal SPD system is exact Cholesky (PCG converges in 1 step)", "[hesap-iterative][ic0]")
{
    crd::jobs::init();
    {
...
TEST_CASE("IC(0)-PCG converges on the 2D Laplacian and cuts iterations vs plain CG", "[hesap-iterative][ic0]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i)
        {
            REQUIRE(std::abs(x0(i) - x1(i)) < 1e-5);
        }
    }
    crd::jobs::shutdown();
}
...
TEST_CASE("IC(0)-PCG solves a complex HPD system", "[hesap-iterative][ic0][complex]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i)
        {
            b(i) = C{1.0 + 0.01 * static_cast<crd::f64>(i % 5), 0.1};
        }
        IterativeOptions<crd::f64> opts;
        opts.rel_tol = 1e-10;
        opts.max_iter = 50;
