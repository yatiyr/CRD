...
namespace
{
template <typename T> SparseMatrix<T, SparseFormat::Csr> spd_tridiag(crd::memory::IAllocator* a, crd::u32 n)
{
...
template <typename T> SparseMatrix<T, SparseFormat::Csr> hpd_tridiag(crd::memory::IAllocator* a, crd::u32 n)
{
    TripletBuilder<T> b(a, n, n);
    for (crd::u32 i = 0; i < n; ++i)
    {
...
template <typename T> SparseMatrix<T, SparseFormat::Csr> spd_varying_diag(crd::memory::IAllocator* a, crd::u32 n)
{
    TripletBuilder<T> b(a, n, n);
    for (crd::u32 i = 0; i < n; ++i)
    {
...
TEST_CASE("Block-CG solves a multi-RHS SPD system (f64, s=4)", "[hesap-iterative][block-cg]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 k = 0; k < n; ++k)
        {
            for (crd::u32 j = 0; j < s; ++j)
            {
                b(k * s + j) = 1.0 + 0.1 * static_cast<crd::f64>(j) + 0.01 * static_cast<crd::f64>(k % 7);
            }
        }
...
TEST_CASE("Block-CG is breakdown-free on a rank-deficient block (duplicate RHS columns)", "[hesap-iterative][block-cg]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator alloc{64U << 20};
...
TEST_CASE("Block-PCG with complex Jacobi block preconditioner solves a multi-RHS HPD system "
          "(c64; complex completeness)",
          "[hesap-iterative][block-cg][block-pcg][complex]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator alloc{64U << 20};
        ...
        auto a = hpd_tridiag<C>(&alloc, n);
        ParallelSpmmLinearOp<C> op(a);
        crd::hesap::preconditioners::JacobiBlockPreconditioner<C> jac(a, &alloc);
        dense::Vector<C> b(&alloc, n * s);
        for (crd::u32 k = 0; k < n; ++k)
        {
            for (crd::u32 j = 0; j < s; ++j)
        ...
        auto res = block_pcg<C>(op, jac, b.span(), x.span(), opts, ws, &alloc);
        REQUIRE(res.converged);
        REQUIRE(block_residual<C>(op, x.span(), b.span(), s, &alloc) < 1e-7);
    }
    crd::jobs::shutdown();
          ...
          "[hesap-iterative][block-cg][block-pcg]")
{
    crd::jobs::init();
    {
        ...
        for (crd::usize i = 0; i < n * s; ++i)
        {
            b(i) = 1.0 + 0.003 * static_cast<crd::f64>(i % 13);
        }
        ...
        for (crd::usize i = 0; i < n * s; ++i)
        {
            REQUIRE(x_native(i) == x_adapted(i));
        }
    }
    crd::jobs::shutdown();
}
