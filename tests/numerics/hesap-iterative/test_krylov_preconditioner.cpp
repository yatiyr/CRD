...
TEST_CASE("KrylovPreconditioner: inner-GMRES preconditioner accelerates outer FGMRES", "[hesap-iterative][nested]")
{
    crd::jobs::init();
    {
        ...
        auto res_precond = fgmres<crd::f64>(op, &p, b.span(), xn.span(), opts, wn, &alloc);

        REQUIRE(rp.converged);
        REQUIRE(res_precond.converged);
        ...
        REQUIRE(res_precond.iterations < rp.iterations);
    }
    crd::jobs::shutdown();
}
