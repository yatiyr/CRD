...
// device-free plan is proven in tests/execution/ceir-gpu/test_tensor_pipeline.cpp; here the plan DRIVES a real chained GPU run.
...
#include "../../gpu/gpu-shared/autodiff_fd_functors.hpp" // CEIR-25c-2: SumGemmAA + MlpLoss FD witnesses (shared with the DX12 pipeline TU)
...
TEST_CASE("ceir35 Q7: executor overhead vs hand-rolled dispatch (Vulkan)", "[ceir][ceir35][perf][executor][gpu]")
{
    ...
    REQUIRE(median_a_gpu > 0.0); // a 0 median ⇒ the device timestamps resolved nothing (measuring NOTHING) — hard fail, not a pass
