// CEIR-20b STEP 4 (Vulkan): the DEVICE PROOF of `execute_work_lowered` (the ceir.work fallback executor). An AUTHORED
// ceir.work program (queue_alloc → produce → consume) runs on a real Vulkan device through the WorkHooks: produce (a
// DIRECT dispatch of the authored work_smoke_produce.ckir) writes the queue's (count=5,1,1) DEVICE header; consume (an
// INDIRECT dispatch of work_smoke_consume.ckir) is SIZED by that device-written count via vkCmdDispatchIndirect (the
// queue header IS the args), so out[0] == 5 proves the grid came from the DEVICE, never host-sized. ⛔ This is the
// migrated-executor scar's device leg — a new execution path must run on a real device. Soft-skips with no adapter (the
// [ceir20b] host-only cases carry the seam coverage).
//
// ⛔ SINGLE-SUBMIT + REPLAYED BARRIERS (the device-resident model, advisor-locked — the 6-dispatch FFT precedent): the
// caller owns begin()/submit_and_wait() + the boundary transfers, the executor owns the inter-stage barriers (via the
// barrier hook).
...
namespace
{
// Load a .ckir kernel from disk → compile to a ComputePipeline (`nbind` storage bindings, no push). Also VALIDATES the
// authored .ckir format (ckir_read → emit GLSL → SPIR-V — the first thing that fails if the node/stmt/cval format is
// wrong).
std::unique_ptr<gpu::ComputePipeline> load_kernel_pipe(const char* path, const char* name, int nbind,
                                                       gpu::VulkanComputeContext& compute,
                                                       crd::memory::IAllocator* alloc)
{
    ...
    REQUIRE(crd::kir::emit_compute_kernel_glsl(kg, ke, alloc, kern));
    const auto cres =
        gpu::compile_glsl_to_spirv(gpu::ShaderStage::Compute, crd::containers::to_view(kern.source), name, alloc);
    INFO(cres.error_message.c_str());
    REQUIRE(cres.ok);
    auto pipe = compute.create_pipeline_from_spirv(
        crd::containers::ConstSpan<crd::u8>(cres.spirv.data(), cres.spirv.size()), nbind, 0U);
...
crd::containers::ConstSpan<crd::u8> smoke_kernel_bytes(const ce::Operation* /*op*/, void* user)
{
    ...
    return crd::containers::ConstSpan<crd::u8>(&c->sentinel,
                                               1U); // sentinel: this smoke resolves pipelines by symbol, not bytes
}
// The SwapResolve precedent: the reloaded/authored `kernel` symbol selects the pipeline, not the test.
                             ...
                             crd::containers::ConstSpan<ceg::WorkBufferHandle> handles, void* user)
{
    ...
    // ⭐ the DEVICE-count-sized dispatch: the (count,1,1) header at offset 0 of the queue buffer IS the indirect args.
...
TEST_CASE(
    "ceir 20b: the ceir.work executor runs on Vulkan -- produce writes the device count, an INDIRECT consume reads it "
    "(device-generated work)",
    "[ceir][ceir-gpu][vulkan][gpu][work][ceir20b]")
{
    ...
    // (a) compile the 2 AUTHORED .ckir smoke kernels → pipelines (validates the .ckir format).
    auto pipe_produce =
        load_kernel_pipe(CRD_REPO_DIR "/assets/ckir/work_smoke_produce.ckir", "work_smoke_produce", 1, compute, &alloc);
    auto pipe_consume =
        load_kernel_pipe(CRD_REPO_DIR "/assets/ckir/work_smoke_consume.ckir", "work_smoke_consume", 2, compute, &alloc);
    ...
    { // host-init to ZERO (produce writes the real header; out starts at 0 for the atomic counter)
    ...
    // (c) the AUTHORED ceir.work program: queue_alloc → produce → consume.
    ...
    // the WorkResolvedBinding table: each exposed Value → its NAMED device buffer (by source_param).
    ...
    // (d) hooks + (e) DRIVE — one submit, caller owns begin/submit + the boundary transfers; executor owns the
    ...
    rec.barrier(*q_dev, gpu::ComputeAccess::TransferDst, gpu::ComputeAccess::ShaderWrite); // produce WRITES the header
    rec.barrier(*o_dev, gpu::ComputeAccess::TransferDst, gpu::ComputeAccess::ShaderWrite); // consume writes out
    ...
    // ⭐ the DEVICE PROOF. ⛔ NON-OVERWRITE: queue[0] (count), out[0] (atomic counter) and out[1] (count readback) are
    // DISTINCT slots nothing later overwrites, so this SINGLE end-of-run readback equals what per-stage readbacks would
    // see — the property that makes single-submit valid for the straight-line body (per-stage submits return only for
