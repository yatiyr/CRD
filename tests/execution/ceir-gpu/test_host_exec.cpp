...
#include "../../gpu/gpu-shared/ckir_asset_resolve.hpp" // CEIR-30b-2b-1: the shared HostKernelResolveFn (loads relu.ckir for VizDispatch)
...
TEST_CASE("ceir 30b-2b-1: the two-class sandwich runs all-Host bit-exact", "[ceir][host-exec]")
{
    ...
    // `crd::tests::fill_two_class_sandwich` (tests/gpu/gpu-shared/two_class_fixture.hpp) at dd=4 — byte-identical to the CUDA three-way
