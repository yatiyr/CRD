          ...
          "[hesap-iterative][ilut][tri-solve]")
{
    crd::jobs::init();
    {
        ...
        crd::containers::Array<crd::u32> ptr(&alloc);
        crd::containers::Array<crd::u32> col(&alloc);
        crd::containers::Array<crd::f64> val(&alloc);
        crd::containers::Array<crd::f64> inv_diag(&alloc);
...
TEST_CASE("ILUT with no dropping on a tridiagonal is exact LU (FGMRES converges in 1 step)", "[hesap-iterative][ilut]")
{
    crd::jobs::init();
    {
          ...
          "[hesap-iterative][ilut]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i)
        {
            REQUIRE(std::abs(x0(i) - xt(i)) < 1e-5);
        }
    }
    crd::jobs::shutdown();
}
