...
SparseMatrix<T, SparseFormat::Csr> laplace2d(crd::memory::IAllocator* a, crd::u32 g)
{
    ...
    for (crd::u32 y = 0; y < g; ++y)
    {
        for (crd::u32 x = 0; x < g; ++x)
        {
            ...
            b.add(i, i, T(4));
            if (x + 1 < g) { b.add(i, i + 1, T(-1)); }
            if (x > 0) { b.add(i, i - 1, T(-1)); }
            if (y + 1 < g) { b.add(i, i + g, T(-1)); }
            if (y > 0) { b.add(i, i - g, T(-1)); }
        }
    }
    return b.compress();
}
...
TEST_CASE("SPAI (static) accelerates FGMRES on a nonsymmetric system", "[hesap-iterative][spai]")
{
    crd::jobs::init();
    {
        ...
        IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-9; opts.max_iter = 500;
...
TEST_CASE("SPAI adaptive yields a stronger preconditioner than static (fewer iters)", "[hesap-iterative][spai]")
{
    crd::jobs::init();
    {
        ...
        auto iters = [&](SpaiPreconditioner<crd::f64>& m) {
            ...
            IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-8; opts.max_iter = 5000;
...
TEST_CASE("SPAI-BiCGSTAB solves a complex nonsymmetric system", "[hesap-iterative][spai][complex]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i)
        {
            ...
            if (i + 1 < n) { tb.add(i, i + 1, C{-1.0, 0.2}); }
            if (i > 0) { tb.add(i, i - 1, C{-1.3, -0.1}); }
        }
        auto              a = tb.compress();
        ...
        IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-9; opts.max_iter = 400;
...
TEST_CASE("FSPAI adaptive converges and is at least as strong as static", "[hesap-iterative][fspai]")
{
    crd::jobs::init();
    {
        ...
        SparseLinearOp<crd::f64>     op(a);
        dense::Vector<crd::f64>      b(&alloc, n);
        for (crd::u32 i = 0; i < n; ++i) { b(i) = 1.0 + 0.01 * static_cast<crd::f64>(i % 5); }
...
TEST_CASE("FSPAI build is bit-identical across thread counts (determinism moat)", "[hesap-iterative][fspai][determinism]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator   alloc{256U << 20};
