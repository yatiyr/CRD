          ...
          "[hesap-iterative][block-gmres][block-qr]")
{
    ...
    dense::Vector<crd::f64> W0(&alloc, n * s), W(&alloc, n * s), cm(&alloc, n * s);
    crd::containers::Array<crd::f64> R(&alloc);
    R.resize(static_cast<crd::usize>(s) * s);
    crd::u32 seed = 12345U;
    auto     rng  = [&]() { seed = seed * 1664525U + 1013904223U; return static_cast<crd::f64>(seed >> 8) / 16777216.0 - 0.5; };
    for (crd::usize i = 0; i < n * s; ++i) { W0(i) = rng(); W(i) = W0(i); }
    ...
    for (crd::u32 i = 0; i < s; ++i)
    {
        ...
        {
            crd::f64 dot = 0.0;
            for (crd::usize k = 0; k < n; ++k) { dot += W(k * s + i) * W(k * s + j); }
    ...
    for (crd::usize k = 0; k < n; ++k)
    {
        ...
        {
            crd::f64 acc = 0.0;
            for (crd::u32 i = 0; i < s; ++i) { acc += W(k * s + i) * R[static_cast<crd::usize>(i) * s + j]; }
          ...
          "[hesap-iterative][block-gmres]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 k = 0; k < n; ++k)
        {
            ...
            for (crd::u32 j = 0; j < s; ++j)
            {
                b(k * s + j) = std::sin(0.21 * static_cast<crd::f64>(k) * static_cast<crd::f64>(j + 1) + 0.4 * static_cast<crd::f64>(j))
...
TEST_CASE("Block-GMRES solves a complex nonsymmetric multi-RHS system (c64)",
          "[hesap-iterative][block-gmres][complex]")
{
    crd::jobs::init();
    {
