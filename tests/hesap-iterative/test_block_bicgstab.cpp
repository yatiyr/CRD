...
template <typename T> void fill_independent_rhs(dense::Vector<T>& b, crd::u32 n, crd::u32 s)
{
    for (crd::u32 k = 0; k < n; ++k)
    {
        for (crd::u32 j = 0; j < s; ++j)
        {
            const crd::f64 v = std::sin(0.21 * static_cast<crd::f64>(k) * static_cast<crd::f64>(j + 1) +
