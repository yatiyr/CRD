...
#include <crd/hesap/preconditioners/reordered.hpp>
...
using crd::hesap::preconditioners::ReorderedPreconditioner;
...
{
    return conv_diff2d<T>(a, g, 0.0); // beta=0 ⇒ symmetric SPD 5-point Laplacian
}
} // namespace

TEST_CASE("AMD-reordered ILUT-BiCGSTAB solves the original system (nonsym)", "[hesap-iterative][reordered]")
{
    crd::jobs::init();
    {
        ...
        const crd::u32             g = 24;
        const crd::u32             n = g * g;
        auto                       a = conv_diff2d<crd::f64>(&alloc, g, 0.35);
        SparseLinearOp<crd::f64>   op(a);
        ReorderedPreconditioner<crd::f64, IlutPreconditioner<crd::f64>> m(a, &alloc, 20U, 1e-4);
        ...
        // TRUE residual on the ORIGINAL (unreordered) system.
...
TEST_CASE("AMD-reordered IC(0)-PCG solves an SPD system", "[hesap-iterative][reordered]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator alloc{96U << 20};
        ...
        SparseLinearOp<crd::f64>   op(a);
        ReorderedPreconditioner<crd::f64, Ic0Preconditioner<crd::f64>> m(a, &alloc);
        dense::Vector<crd::f64> b(&alloc, n), x(&alloc, n);
...
TEST_CASE("AMD-reordered ILUT handles a complex nonsymmetric system", "[hesap-iterative][reordered][complex]")
{
    crd::jobs::init();
    {
        crd::memory::TlsfAllocator alloc{48U << 20};
        using C        = Complex<crd::f64>;
        const crd::u32 n = 169; // 13×13
        auto           a = conv_diff2d<C>(&alloc, 13, 0.3);
        SparseLinearOp<C> op(a);
        ReorderedPreconditioner<C, IlutPreconditioner<C>> m(a, &alloc, 20U, 1e-4);
...
TEST_CASE("AMD-reordered ILUT is bit-exact over serial vs parallel spmv (determinism moat)",
          "[hesap-iterative][reordered][determinism]")
{
    crd::jobs::init();
    {
        ...
        auto solve = [&](const crd::hesap::LinearOp<crd::f64>& op, dense::Vector<crd::f64>& x) {
            ReorderedPreconditioner<crd::f64, IlutPreconditioner<crd::f64>> m(a, &alloc, 25U, 1e-4);
