...
TEST_CASE("ILU(0) on a tridiagonal nonsym system is exact LU (FGMRES converges in 1 step)",
          "[hesap-iterative][ilu0]")
{
    crd::jobs::init();
    {
...
TEST_CASE("ILU(0) accelerates FGMRES + BiCGSTAB on 2D convection-diffusion",
          "[hesap-iterative][ilu0]")
{
    crd::jobs::init();
    {
        ...
        dense::Vector<crd::f64> xp(&alloc, n), xi(&alloc, n);
        GmresWorkspace<crd::f64> wsp(&alloc, n, 40), wsi(&alloc, n, 40);
        ...
        if (plain.converged) { REQUIRE(pre.iterations < plain.iterations); }
        ...
        for (crd::u32 i = 0; i < n; ++i) { REQUIRE(std::abs(xi(i) - xb(i)) < 1e-5); }
    }
    crd::jobs::shutdown();
}
