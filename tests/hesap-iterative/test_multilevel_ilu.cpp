...
template <typename T>
SparseMatrix<T, SparseFormat::Csr> shifted_tridiag(crd::memory::IAllocator* a, crd::u32 n)
{
                  ...
                  crd::memory::IAllocator* alloc, crd::u32 n)
{
    ...
    {
        const auto bi = b(i);
        if constexpr (crd::hesap::dense::is_complex_v<T>) { nb += double(bi.re) * bi.re + double(bi.im) * bi.im; }
        else { nb += double(bi) * bi; }
        const auto d = b(i) - ax(i);
        if constexpr (crd::hesap::dense::is_complex_v<T>) { nr += double(d.re) * d.re + double(d.im) * d.im; }
        else { nr += double(d) * d; }
    }
