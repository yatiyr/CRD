...
TEST_CASE("ILU(p) on a tridiagonal is exact LU at any p (FGMRES converges in 1 step)",
          "[hesap-iterative][ilup]")
{
    crd::jobs::init();
    {
...
TEST_CASE("ILU(p) fill and convergence improve monotonically with p (and large p = full LU)",
          "[hesap-iterative][ilup]")
{
    crd::jobs::init();
    {
        ...
        IlupPreconditioner<crd::f64> p0(a, &alloc, 0), p1(a, &alloc, 1), p2(a, &alloc, 2);
        ...
        const auto i0 = iters(p0), i1 = iters(p1), i2 = iters(p2);
        ...
        {
            for (crd::u32 j = 0; j < nd; ++j) { tb.add(i, j, (i == j) ? 10.0 + i : (0.5 + 0.1 * static_cast<crd::f64>((i + 3 * j) % 5))); }
        }
        auto                         ad = tb.compress();
        SparseLinearOp<crd::f64>     opd(ad);
        IlupPreconditioner<crd::f64> pdense(ad, &alloc, 0); // dense ⇒ ILU(0) = full LU
        dense::Vector<crd::f64>      bd(&alloc, nd), xd(&alloc, nd);
        for (crd::u32 i = 0; i < nd; ++i) { bd(i) = 1.0 + 0.1 * static_cast<crd::f64>(i); }
        IterativeOptions<crd::f64>   opts; opts.rel_tol = 1e-10; opts.max_iter = 50;
