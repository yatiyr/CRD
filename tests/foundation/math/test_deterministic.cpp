          ...
          "[deterministic][accuracy]")
{
    ...
    for (int i = 0; i < k_n; ++i)
    {
        const f32 t = -2.0F * det::kPi + (4.0F * det::kPi) * static_cast<f32>(i) / static_cast<f32>(k_n - 1);
          ...
          "[deterministic][accuracy]")
{
    ...
    for (int i = 0; i < k_n; ++i)
    {
        const f32 t = -det::kPi / 3.0F + (2.0F * det::kPi / 3.0F) * static_cast<f32>(i) / static_cast<f32>(k_n - 1);
...
{
    REQUIRE(std::abs(det::sin(det::kPi)) < 1.0e-6F);
}

TEST_CASE("deterministic cos(pi) is -1 within ulps", "[deterministic]")
{
    REQUIRE(std::abs(det::cos(det::kPi) + 1.0F) < 1.0e-6F);
}
...
TEST_CASE("deterministic atan2 at axes", "[deterministic]")
{
    ...
    REQUIRE(std::abs(det::atan2( 1.0F,  0.0F) - det::kHalfPi)  < 1.0e-6F);
    REQUIRE(std::abs(det::atan2(-1.0F,  0.0F) + det::kHalfPi)  < 1.0e-6F);
    REQUIRE(std::abs(det::atan2( 0.0F, -1.0F) - det::kPi)    < 1.0e-6F);
}
...
TEST_CASE("deterministic GOLDEN sin", "[deterministic][golden]")
{
    ...
    const u32 g_pi4  = bits(det::sin( det::kQuarterPi));
    const u32 g_pi2  = bits(det::sin( det::kHalfPi));
    const u32 g_pi   = bits(det::sin( det::kPi));
    const u32 g_n10  = bits(det::sin(-1.0F));
    const u32 g_neg  = bits(det::sin(-det::kHalfPi));
    ...
    REQUIRE((bits(det::sin( det::kQuarterPi)) ^ 0x80000000U) == bits(det::sin(-det::kQuarterPi)));
}
...
TEST_CASE("deterministic GOLDEN cos", "[deterministic][golden]")
{
    ...
    REQUIRE(bits(det::cos( det::kQuarterPi)) == bits(det::cos(-det::kQuarterPi)));
}
