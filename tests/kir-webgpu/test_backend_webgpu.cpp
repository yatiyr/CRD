// test_backend_webgpu.cpp — Phase 3.1.6 v17-d: the CKIR WebGPU backend (wgpu-native locally; the same path runs in the
// browser via WASM). The FOURTH backend from one CKIR IR. Runs elementwise/matmul/reduce through KirBackendWebGpu and
// gates them CLOSE to the CPU oracle. NOTE: WGSL has no `precise` ⇒ ULP-tolerant, not bit-exact (an implementation may
// fuse FMAs). ADR-0098.
...
TEST_CASE("v17-d: WebGPU elementwise matches the CPU oracle (ULP-tolerant)", "[kir][webgpu][gpu]")
{
    ...
    float        cpu_out[kN];
    float        d2[kN];
    REQUIRE(wg.run(g, out, inputs, 2, gpu_out));
    REQUIRE(cpu.run(g, out, inputs, 2, cpu_out));
    REQUIRE(wg.run(g, out, inputs, 2, d2));
    for (int i = 0; i < kN; ++i)
    {
        CHECK(absf(gpu_out[i] - cpu_out[i]) <= 1e-4F * (1.0F + absf(cpu_out[i]))); // ULP-tolerant (WGSL no precise)
        CHECK(gpu_out[i] == d2[i]);                                               // run-to-run deterministic
    }
}


TEST_CASE("v17-g: WebGPU FUSES GEMM+bias+SiLU into one kernel, correct vs the oracle", "[kir][webgpu][gpu]")
{
    crd::memory::TlsfAllocator alloc(64 << 20);
...
TEST_CASE("v17-breadth: WebGPU floor/ceil/sign/cmpeq/cmple bit-match the CPU oracle", "[kir][webgpu][gpu]")
{
    crd::memory::TlsfAllocator alloc(64 << 20);
    kir::KirBackendWebGpu        be(&alloc);
    if (!be.valid()) { WARN("no WebGPU device available; skipping"); return; }
...
TEST_CASE("v17-breadth: WebGPU round ties-to-even bit-matches the CPU oracle", "[kir][webgpu][gpu]")
{
    crd::memory::TlsfAllocator alloc(64 << 20);
    kir::KirBackendWebGpu        be(&alloc);
    if (!be.valid()) { WARN("no WebGPU device available; skipping"); return; }
...
TEST_CASE("v17-perf: WebGPU T2 fast parallel scan matches T1 oracle + deterministic", "[kir][webgpu][gpu]")
{
    crd::memory::TlsfAllocator alloc(64 << 20);
    kir::KirBackendWebGpu        be(&alloc);
    if (!be.valid()) { WARN("no WebGPU device available; skipping"); return; }
