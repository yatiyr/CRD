...
TEST_CASE("TextureResource cooked from TGA round-trip", "[resources][texture][cook]")
{
    ...
    tga_name.append(".tga");
    const ScopedTestFile tga_file(fs::temp_directory() / crd::containers::StringView(tga_name.data(), tga_name.size()));
    const fs::Path& tga_path = tga_file.path();
    ...
    cook_ctx.source_path = tga_path.generic();
    cook_ctx.id          = txtr_id;
