...
TEST_CASE("MeshResource cooked from GLB round-trip", "[resources][mesh][cook]")
{
    ...
    const ScopedTestFile glb_file(fs::temp_directory() / crd::containers::StringView(glb_name.data(), glb_name.size()));
    const fs::Path& glb_path = glb_file.path();
    ...
    cook_ctx.source_path = glb_path.generic();
    cook_ctx.id          = mesh_id;
