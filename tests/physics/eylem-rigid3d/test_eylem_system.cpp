...
// a Catch listener pattern like tests/assets/resources/test_resource_manager.cpp.
          ...
          "[eylem][v1b-c][system][integration]")
{
    ...
    constexpr crd::f32 k_dt    = 1.0F / 60.0F;
    constexpr crd::u32 nstep = 60U;
    constexpr crd::f32 k_g     = -9.81F;

    BodyPool      pool{crd::memory::default_allocator(), 64U};
    PhysicsConfig cfg{};
    cfg.fixed_dt = crd::units::Duration32{k_dt};
    cfg.gravity = crd::math::from_raw_vec<crd::units::dim::Acceleration>(crd::math::Vec3f{0.0F, k_g, 0.0F});
    EylemSystem  system{pool, cfg};
    ...
    //                    = g·dt² · k·(k+1)/2
    const crd::f32 expected_y = k_g * k_dt * k_dt * static_cast<crd::f32>(nstep)
    ...
    REQUIRE(std::fabs(read.linear_velocity.y.value - k_g * k_dt * static_cast<crd::f32>(nstep)) < 1e-3F);
}
          ...
          "[eylem][v1b-c][system][schedule]")
{
    ...
    constexpr crd::f32 k_dt         = 1.0F / 64.0F;
    constexpr crd::f32 k_g          = -9.81F;
    constexpr crd::u32 nstep_seed  = 10U;
    constexpr crd::f32 k_frame_dt    = static_cast<crd::f32>(nstep_seed) * k_dt; // 10/64 exact
    ...
    world.step_fixed(static_cast<crd::f64>(k_frame_dt),
                     static_cast<crd::f64>(k_dt),
                     /*max_substeps=*/32U);

    // Discrete-Euler closed form for 10 substeps under gravity from rest.
    const crd::f32 expected_y = k_g * k_dt * k_dt * static_cast<crd::f32>(nstep_seed)
