...
{
    constexpr double kSorted[] = {1.9, 2.1, 2.2, 2.8, 3.0, 3.4, 3.7, 4.1, 4.7, 5.2, 5.9, 6.3};
    constexpr double kP[] = {0.1, 0.25, 0.5, 0.75, 0.9};
    constexpr double kRef[9][5] = {
    ...
    const auto sorted = crd::containers::Span<const double>{kSorted, 12};
    for (int t = 1; t <= 9; ++t)
    {
        for (int pi = 0; pi < 5; ++pi)
        {
            CHECK(close(quantile_sorted(sorted, kP[pi], t), kRef[t - 1][pi], 1e-9));
        }
    }
}
...
{
    constexpr double kSorted[] = {1.9, 2.1, 2.2, 2.8, 3.0, 3.4, 3.7, 4.1, 4.7, 5.2, 5.9, 6.3};
    const auto s = crd::containers::Span<const double>{kSorted, 12};
...
TEST_CASE("v12-m: covariance/correlation matrices vs numpy", "[v12-m][stats][descriptive]")
{
    ...
    constexpr double kMat[24] = {2.1, 3.4, 1.9, 5.2, 4.1, 2.8, 6.3, 3.0, 1.0, 2.0, 1.5, 3.0,
                                 2.5, 2.0, 3.5, 1.8, 5.0, 4.0, 6.0, 2.0, 3.0, 4.5, 1.0, 4.2};
    constexpr double kCov[9] = {2.32571428571429, 1.19571428571429,  -2.46,
                                1.19571428571429, 0.654107142857143, -1.23803571428571,
                                -2.46,            -1.23803571428571, 2.66125};
    constexpr double kCor[9] = {1,                  0.969448609096053,  -0.988812822693879,
                                0.969448609096053,  1,                  -0.938352229356669,
                                -0.988812822693879, -0.938352229356669, 1};
    double cov[9];
    double cor[9];
    covariance_matrix(crd::containers::Span<const double>{kMat, 24}, 3, 8, crd::containers::Span<double>{cov, 9}, 1);
    correlation_matrix(crd::containers::Span<const double>{kMat, 24}, 3, 8, crd::containers::Span<double>{cor, 9});
    for (int i = 0; i < 9; ++i)
    {
        CHECK(close(cov[i], kCov[i], 1e-9));
        CHECK(close(cor[i], kCor[i], 1e-9));
    }
}
...
{
    constexpr double kSorted[] = {1.9, 2.1, 2.2, 2.8, 3.0, 3.4, 3.7, 4.1, 4.7, 5.2, 5.9, 6.3};
    constexpr double kRaw[] = {2.1, 3.4, 1.9, 5.2, 4.1, 2.8, 6.3, 3.0, 4.7, 2.2, 5.9, 3.7};
    constexpr double kW[] = {1.0, 2.0, 1.0, 3.0, 1.0, 2.0, 1.0, 1.0, 2.0, 1.0, 3.0, 1.0};
    const auto s = crd::containers::Span<const double>{kSorted, 12};
    const auto raw = crd::containers::Span<const double>{kRaw, 12};
    const auto w = crd::containers::Span<const double>{kW, 12};
