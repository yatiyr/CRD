...
{
    AllocFixture f{};
    constexpr u32 n = 200U;
    auto base = make_random_cloud(n, 42U, &f.alloc);
    ...
    for (u32 seed = 1; seed <= 5U; ++seed)
    {
        ...
        shuffled.reserve(n);
        for (u32 i = 0; i < n; ++i) { shuffled.push_back(base[i]); }
        ...
        for (usize i = 0; i < n; ++i) { REQUIRE(base_pos[i] == shuf_pos[i]); }
    }
}
