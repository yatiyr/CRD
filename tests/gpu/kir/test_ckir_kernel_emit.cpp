...
// runs it bit-exact-to-ULP (tests/gpu/gpu-context-vulkan). This is the structural cross-backend proof they lower everywhere.
...
TEST_CASE("B15-b: Perlin-Worley cloud density emits on ALL backends (worley3_loop portability)", "[kir][kernel][clouds]")
{
    ...
    kir::GlslKernel            km(&alloc);
    kir::GlslKernel            kw(&alloc);
    REQUIRE(kir::emit_compute_kernel_glsl(g, e, &alloc, kg)); // Vulkan — GPU-verified bit-exact in tests/gpu/gpu-context-vulkan
