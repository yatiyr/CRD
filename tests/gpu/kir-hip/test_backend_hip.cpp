// test_backend_hip.cpp — Phase 3.1.6 v17-d: the CKIR HIP/ROCm backend. Guarded on the HIP SDK; on this NVIDIA box the
// module is skipped, so this suite is built + run on real AMD at Part C (RunPod). Same graphs as the other backends,
// gated BIT-EXACT vs the CPU oracle (HIP `-ffp-contract=off` ⇒ no FMA fusion). ADR-0098.
