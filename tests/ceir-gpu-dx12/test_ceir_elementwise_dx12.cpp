// CEIR-25b-2a (DX12) -- the DirectX-12 twin of the CEIR->CKIR native-provider ELEMENTWISE device gate. A declare-only
// ceir.tensor.elementwise op is SYNTHESIZED (synth_elementwise) into a graph-tier CKIR binary node, run on a REAL D3D12 device
// (KirBackendDx12) via the EXISTING fused-elementwise emitter (emit_elementwise_hlsl, reached by run()'s fall-through -- is_fusable
// covers the vocab), proven vs BOTH eval_cpu AND an INDEPENDENT scalar reference for the FULL binary vocab {add,sub,mul,div,max,min,pow}
// (a fn->KOp mis-wiring bit-matches its own oracle but NOT the independent ref -- the 20c-2 guard; also the HLSL/GLSL emitter agreement).
// PER-OP contract: add/sub/mul/div/max/min BIT-EXACT (correctly-rounded single f32 op, no FMA; div exact for b in {1,2}); pow ULP tol.
...
TEST_CASE("ceir 25b-2a: synth_elementwise runs the full binary vocab on a D3D12 device (add/sub/mul/div/max/min exact, pow within tol)",
          "[ceir][ckir-synth][gpu]")
{
