...
TEST_CASE("v13-i: tanh-sinh DE — finite intervals + endpoint singularities", "[v13-i][quadrature]")
{
    ...
    CHECK(close(q::integrate_tanh_sinh<f64>(rule, [](f64 x) { return std::log(x); }, 0.0, 1.0, 1e-12, 1e-12).value, -1.0,
                1e-10, 1e-11)); // ∫_0^1 ln x = -1
    // smooth — matches scipy.quad's value
    CHECK(close(
        q::integrate_tanh_sinh<f64>(rule, [](f64 x) { return std::exp(x) * std::cos(2 * x); }, 0.0, 1.0, 1e-12, 1e-12)
            .value,
        0.562449792050565, 1e-10, 1e-11));
    // right-endpoint singularity — resolved to ~1e-8 (the single-argument f(x) interface limit)
    CHECK(close(q::integrate_tanh_sinh<f64>(rule, [](f64 x) { return 1.0 / std::sqrt(1.0 - x); }, 0.0, 1.0, 1e-7, 1e-7)
                    .value,
                2.0, 1e-6, 1e-7));
}
...
TEST_CASE("v13-i: exp-sinh DE — semi-infinite [a,inf)", "[v13-i][quadrature]")
{
    ...
    CHECK(close(q::integrate_exp_sinh<f64>(rule, [](f64 x) { return std::exp(-x * x); }, 0.0, 1e-12, 1e-12).value,
                0.8862269254527580, 1e-10, 1e-11)); // ∫_0^∞ e^-x² = √π/2
    CHECK(close(q::integrate_exp_sinh<f64>(rule, [](f64 x) { return std::exp(-x); }, 0.0, 1e-12, 1e-12).value, 1.0,
                1e-10, 1e-11)); // ∫_0^∞ e^-x = 1
    CHECK(close(q::integrate_exp_sinh<f64>(rule, [](f64 x) { return 1.0 / (1.0 + x * x); }, 0.0, 1e-12, 1e-12).value,
                1.5707963267948966, 1e-10, 1e-11)); // ∫_0^∞ 1/(1+x²) = π/2
    CHECK(close(q::integrate_exp_sinh<f64>(rule, [](f64 x) { return 1.0 / (x * x); }, 1.0, 1e-12, 1e-12).value, 1.0,
                1e-9, 1e-10)); // ∫_1^∞ x^-2 = 1 (a-shifted)
}
...
TEST_CASE("v13-i: sinh-sinh DE — doubly-infinite (-inf,inf) + determinism", "[v13-i][quadrature]")
{
    ...
    CHECK(close(q::integrate_sinh_sinh<f64>(rule, [](f64 x) { return 1.0 / (1.0 + x * x); }, 1e-12, 1e-12).value,
...
TEST_CASE("v13-i: Clenshaw-Curtis — fixed rule (degree-n exactness) + nested adaptive", "[v13-i][quadrature]")
{
    ...
    CHECK(close(q::integrate_chebyshev<f64>(cc, [](f64 x) { return std::exp(x); }, 0.0, 1.0).value, 1.7182818284590452,
                1e-12, 1e-13));
...
TEST_CASE("v13-i: Fejér rule — degree-(n−1) exactness", "[v13-i][quadrature]")
{
    crd::memory::TlsfAllocator alloc(1U << 22);
    ...
    CHECK(close(q::integrate_chebyshev<f64>(fr, [](f64 x) { return std::exp(x); }, 0.0, 1.0).value, 1.7182818284590452,
                1e-10, 1e-11));
}
...
TEST_CASE("v13-i: Romberg — function (vs analytic) + samples (vs scipy.romb)", "[v13-i][quadrature]")
{
    ...
    // determinism
    auto rb = [] { return q::integrate_romberg<f64>([](f64 x) { return std::exp(x); }, 0.0, 1.0, 1e-12, 1e-12).value; };
    CHECK(rb() == rb());
}
