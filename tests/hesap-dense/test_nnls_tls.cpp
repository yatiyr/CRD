...
TEST_CASE("nnls: recovers a non-negative ground truth exactly", "[hesap][nnls][real]")
{
    ...
    constexpr crd::usize kM = 10;
    constexpr crd::usize kN = 4;
    // Full-rank Vandermonde in [0,1] (well-conditioned, rank kN) so the
    ...
    {
        const double t = static_cast<double>(i) / static_cast<double>(kM - 1);  // 0..1
        double pw = 1.0;
...
{
    crd::memory::TlsfAllocator alloc(static_cast<crd::usize>(4U * 1024U * 1024U));
    constexpr crd::usize kM = 9;
    constexpr crd::usize kN = 3;
    constexpr crd::usize kD = 2;
    Matrix<double, Layout::RowMajor> a(&alloc, kM, kN);
    for (crd::usize i = 0; i < kM; ++i)
    {
        for (crd::usize j = 0; j < kN; ++j)
    ...
    Matrix<double, Layout::RowMajor> xt(&alloc, kN, kD);
    xt.at(0, 0) = 1.0; xt.at(0, 1) = -0.5;
    xt.at(1, 0) = 2.0; xt.at(1, 1) = 1.5;
    xt.at(2, 0) = -1.0; xt.at(2, 1) = 0.25;
    Matrix<double, Layout::RowMajor> b(&alloc, kM, kD);
    for (crd::usize i = 0; i < kM; ++i)
    {
        for (crd::usize l = 0; l < kD; ++l)
        {
            double s = 0.0;
            for (crd::usize j = 0; j < kN; ++j)
            {
...
TEST_CASE("tls: complex consistent system recovers x", "[hesap][tls][complex]")
{
    ...
    constexpr crd::usize kN = 2;
    Matrix<Cx, Layout::RowMajor> a(&alloc, kM, kN);
    for (crd::usize i = 0; i < kM; ++i)
    {
        for (crd::usize j = 0; j < kN; ++j)
        {
