          ...
          "[hesap][colpiv][real][blocked]")
{
    ...
    constexpr crd::usize kM = 70;
    constexpr crd::usize kN = 40;
    ...
    // Left modes: C is m × kCols; column j transforms as op(Q)·col_j.
    for (bool trans : {true, false})
    {
        Matrix<double, Layout::RowMajor> c(&alloc, kM, kCols);
        fill(c);
        Matrix<double, Layout::RowMajor> ref(&alloc, kM, kCols);
        // Scalar reference: per-column apply.
        for (crd::usize j = 0; j < kCols; ++j)
        {
            crd::containers::Array<double> col(&alloc);
            col.resize(kM);
            for (crd::usize i = 0; i < kM; ++i) col[i] = c.at(i, j);
            if (trans)
                apply_q_transpose(qr, crd::containers::Span<double>{col.data(), kM});
            else
                crd::hesap::dense::apply_q(qr, crd::containers::Span<double>{col.data(), kM});
            for (crd::usize i = 0; i < kM; ++i) ref.at(i, j) = col[i];
        }
        crd::hesap::dense::detail::apply_q_block<double>(qp, ld, kM, kk, taus, c.data(), c.ld(), kM,
                                                         kCols, /*right=*/false, trans, &alloc);
        double e = 0.0;
        for (crd::usize i = 0; i < kM; ++i)
            for (crd::usize j = 0; j < kCols; ++j)
    ...
    // Right modes: C is kCols × m; row i transforms as row_i·op(Q).
    //   C·Qᵀ row i = (Q·row_iᵀ)ᵀ → scalar apply_q on the row.
    //   C·Q  row i = (Qᵀ·row_iᵀ)ᵀ → scalar apply_q_transpose on the row.
    for (bool trans : {true, false})
    {
        Matrix<double, Layout::RowMajor> c(&alloc, kCols, kM);
        fill(c);
        Matrix<double, Layout::RowMajor> ref(&alloc, kCols, kM);
        for (crd::usize i = 0; i < kCols; ++i)
        {
            crd::containers::Array<double> row(&alloc);
            row.resize(kM);
            for (crd::usize j = 0; j < kM; ++j) row[j] = c.at(i, j);
            if (trans)
                crd::hesap::dense::apply_q(qr, crd::containers::Span<double>{row.data(), kM});
            else
                apply_q_transpose(qr, crd::containers::Span<double>{row.data(), kM});
            for (crd::usize j = 0; j < kM; ++j) ref.at(i, j) = row[j];
        }
        crd::hesap::dense::detail::apply_q_block<double>(qp, ld, kM, kk, taus, c.data(), c.ld(),
                                                         kCols, kM, /*right=*/true, trans, &alloc);
        double e = 0.0;
        for (crd::usize i = 0; i < kCols; ++i)
            for (crd::usize j = 0; j < kM; ++j)
                e = std::max(e, std::abs(c.at(i, j) - ref.at(i, j)));
...
TEST_CASE("col-piv QR: A*P = Q*R reconstruction at 8x6", "[hesap][colpiv][real]")
{
    ...
    Matrix<double, Layout::RowMajor> q(&alloc, kM, kM);
...
TEST_CASE("COD: min-norm solve matches QR on full-rank over-determined", "[hesap][cod][real]")
{
    ...
    x.resize(kN);
    solve_cod<double, Layout::RowMajor>(cod, crd::containers::ConstSpan<double>{b.data(), kM},
...
{
    crd::memory::TlsfAllocator alloc(static_cast<crd::usize>(4U * 1024U * 1024U));
    constexpr crd::usize kM = 5;
    constexpr crd::usize kN = 3;
    // Column 2 := 2 * column 0  → rank 2, null vector z = [2, 0, -1].
    Matrix<float, Layout::RowMajor> a(&alloc, kM, kN);
    for (crd::usize i = 0; i < kM; ++i)
    {
    ...
    x.resize(kN);
    solve_cod<float, Layout::RowMajor>(cod, crd::containers::ConstSpan<float>{b.data(), kM},
                                       crd::containers::Span<float>{x.data(), kN});
