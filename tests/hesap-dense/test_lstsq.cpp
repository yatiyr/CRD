...
TEST_CASE("lstsq: multi-RHS solves each column independently", "[hesap][lstsq][real][multirhs]")
{
    ...
    Matrix<double, Layout::RowMajor> b(&alloc, kM, kRhs);
    for (crd::usize i = 0; i < kM; ++i)
    {
        b.at(i, 0) = b0[i];
        b.at(i, 1) = b1[i];
