...
void drive_growable_pool(crd::stress::RunMode mode)
{
    ...
    const auto work = [&sink](u32 w, u64 iters, crd::stress::Rng& rng)
    {
        constexpr usize slot_size = 64U;
        constexpr usize slot_align = 16U;
        constexpr usize slots_per_page = 8U;
        constexpr u32 max_live = 200U; // >> slots_per_page → forces growth
        crd::memory::MallocAllocator parent("v5-gpool-parent");
        crd::memory::GrowablePoolAllocator gp(slot_size, slot_align, slots_per_page, &parent, "v5-gpool");
...
TEST_CASE("allocators stress — TlsfAllocator adversarial sequential", "[stress][memory]")
{
    ...
    SECTION("coalescing — free-every-other then alloc-larger fits")
    {
        ...
        constexpr usize s = 256U;
        u8* blocks[n];
        for (usize i = 0; i < n; ++i)
        {
            blocks[i] = static_cast<u8*>(tlsf.try_allocate(s, 16U));
            REQUIRE(blocks[i] != nullptr);
            std::memset(blocks[i], static_cast<int>(i & 0xFFU), s);
        }
        for (usize i = 0; i < n; i += 2U) // free every other → adjacent holes
        {
        ...
        {
            u8* p = static_cast<u8*>(tlsf.try_allocate(s + s / 2U, 16U));
            if (p == nullptr)
            {
                break;
            }
            std::memset(p, 0xAB, s + s / 2U);
            ++got;
        }
        ...
        for (usize i = 1; i < n; i += 2U)
        {
            ...
            CHECK(blocks[i][s - 1] == static_cast<u8>(i & 0xFFU));
        }
    }
