// CEIR-22b (Vulkan) — the CEIR→CKIR native-provider REDUCE device gate. A declare-only `ceir.tensor.reduce` op is SYNTHESIZED
// (synth_reduce) into a graph-tier CKIR reduce (mask = 1<<axis), run on a REAL Vulkan device (KirBackendVulkan), and proven
// bit-exact vs BOTH eval_cpu AND an INDEPENDENT serial reference — for ALL FOUR fns {sum,prod,max,min} (a fn→KOp mis-wiring,
// e.g. max→ReduceMin, would bit-match its own oracle but NOT the independent ref; the 20c-2 guard). ⛔ data ((r+c)%3)+1 ∈
// {1,2,3} makes the four reductions DISTINCT + exact in f32. CKIR reduce KEEPDIMS: [6,8] reduce axis 1 → [6,1] (numel 6).
...
namespace
{
constexpr int kR = 6; // rows (kept)
constexpr int kC = 8; // cols (reduced, axis 1)
...
// reduce(input[kR,kC]){axis=1, fn} -> [kR], in a FRESH module (one per fn).
...
TEST_CASE("ceir 22b: synth_reduce runs sum/prod/max/min bit-exact on a Vulkan device (== eval_cpu AND independent serial refs)",
          "[ceir][ckir-synth][gpu]")
{
    ...
    // data ∈ {1,2,3}: the four reductions are DISTINCT + exact in f32.
    ...
    for (const Case& cs : cases)
    {
        ...
        for (int i = 0; i < kR; ++i)
        {
            ...
            CHECK(gpu_out[i] == cs.ref[i]);     // device == the independent serial reduction (SYNTHESIS — the fn→KOp guard)
        }
    }
