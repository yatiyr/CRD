...
SparseMatrix<T, SparseFormat::Csr> laplace2d(crd::memory::IAllocator* a, crd::u32 g)
{
    ...
    auto idx = [g](crd::u32 i, crd::u32 j) { return i * g + j; };
    for (crd::u32 i = 0; i < g; ++i)
        for (crd::u32 j = 0; j < g; ++j)
        {
            const crd::u32 r = idx(i, j);
            b.add(r, r, T(4));
            if (j > 0)     { b.add(r, idx(i, j - 1), T(static_cast<crd::f64>(-1.0))); }
            if (j + 1 < g) { b.add(r, idx(i, j + 1), T(static_cast<crd::f64>(-1.0))); }
            if (i > 0)     { b.add(r, idx(i - 1, j), T(static_cast<crd::f64>(-1.0))); }
            if (i + 1 < g) { b.add(r, idx(i + 1, j), T(static_cast<crd::f64>(-1.0))); }
        }
    return b.compress();
}
...
SparseMatrix<T, SparseFormat::Csr> cd2d(crd::memory::IAllocator* a, crd::u32 g, double eps, double beta, double c = 0.0)
{
    ...
    for (crd::u32 i = 0; i < g; ++i)
        for (crd::u32 j = 0; j < g; ++j)
        {
            ...
            if (j > 0)     { b.add(r, idx(i, j - 1), T(static_cast<crd::f64>(-eps - beta))); } // west (upwind)
            if (j + 1 < g) { b.add(r, idx(i, j + 1), T(static_cast<crd::f64>(-eps))); }         // east
            if (i > 0)     { b.add(r, idx(i - 1, j), T(static_cast<crd::f64>(-1.0))); }         // south
            if (i + 1 < g) { b.add(r, idx(i + 1, j), T(static_cast<crd::f64>(-1.0))); }         // north
...
TEST_CASE("AMG strength: a strong tridiagonal connection survives, a weak one is dropped",
          "[hesap-amg][strength]")
{
    ...
    b.add(1, 0, -1.0); b.add(1, 1, 2.0); b.add(1, 2, -0.01);
    b.add(2, 1, -0.01); b.add(2, 2, 2.0);
    auto a = b.compress();
    ...
    for (crd::u32 q = o[1]; q < o[2]; ++q) { if (c[q] == 0) { has01 = true; } if (c[q] == 2) { has12 = true; } }
    REQUIRE(has01);
    REQUIRE_FALSE(has12);
}
...
TEST_CASE("RS C/F splitting: valid coarsening + every F-point depends on a C-point (v4k-d)", "[hesap-amg][rs]")
{
    ...
    {
        if (cf[i] != amg::CfTag::Fine) { continue; }
        ++f_count;
        bool has_c_dep = false;
        for (crd::u32 q = so[i]; q < so[i + 1]; ++q) { if (cf[si[q]] == amg::CfTag::Coarse) { has_c_dep = true; break; } }
        ...
        if (has_any_dep) { REQUIRE(has_c_dep); }
    }
    REQUIRE(f_count > 0);
...
TEST_CASE("AMG aggregation partitions every node into a coarser set", "[hesap-amg][aggregation]")
{
    ...
    for (crd::u32 i = 0; i < a.rows(); ++i) { REQUIRE(agg[i] < n_agg); } // every node aggregated
}

TEST_CASE("AMG builds a multilevel hierarchy with coarsening factor >= 3 per level",
          "[hesap-amg]")
{
...
TEST_CASE("AMG-CG converges fast on a 2D Poisson problem (mesh-independence direction)",
          "[hesap-amg]")
{
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i) { b(i) = 1.0; x(i) = 0.0; }
        IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-8; opts.max_iter = 500;
                              ...
                              amg::SaAmg<crd::f64>::Cycle cycle, bool& converged)
{
    const crd::u32           n = a.rows();
    SparseLinearOp<crd::f64> op(a);
    amg::SaAmg<crd::f64>::Options o; o.cycle = cycle;
    amg::SaAmg<crd::f64>          m(a, alloc, o);
    dense::Vector<crd::f64>       b(alloc, n), x(alloc, n);
    for (crd::u32 i = 0; i < n; ++i) { b(i) = 1.0; x(i) = 0.0; }
    IterativeOptions<crd::f64>  opts; opts.rel_tol = 1e-8; opts.max_iter = 2000;
...
TEST_CASE("CLI: hesap.amg.f64 registers + solves a 2D Poisson (AMG-as-solver)", "[hesap-amg][cli]")
{
    ...
    crd::jobs::init();
    {
        ...
        crd::containers::Array<crd::i64> tr(&alloc), tc(&alloc);
        crd::containers::Array<crd::f64> vals(&alloc), b(&alloc);
        for (crd::u32 i = 0; i < n; ++i)
        {
            for (crd::u32 q = o[i]; q < o[i + 1]; ++q) { tr.push_back(i); tc.push_back(c[q]); vals.push_back(v[q]); }
            b.push_back(1.0);
        }
        ...
        const auto out = crd::containers::ConstSpan<crd::f64>{
            reinterpret_cast<const crd::f64*>(blob->bytes.data()), blob->bytes.size() / sizeof(crd::f64)};
...
TEST_CASE("AMG adaptive-candidate (alphaSA) builds + converges without regressing diffusion", "[hesap-amg]")
{
    crd::jobs::init();
    {
        ...
        amg::SaAmg<crd::f64>::Options o; o.adaptive_candidate = true;
        amg::SaAmg<crd::f64>          m(a, &alloc, o);
        REQUIRE(m.num_levels() >= 3); // builds a genuine hierarchy with the adaptive tentative
        dense::Vector<crd::f64> b(&alloc, n), x(&alloc, n);
        for (crd::u32 i = 0; i < n; ++i) { b(i) = 1.0; x(i) = 0.0; }
        IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-8; opts.max_iter = 200;
        KrylovWorkspace<crd::f64>  ws(&alloc, n);
        auto res = pcg<crd::f64>(op, m, b.span(), x.span(), opts, ws, &alloc);
        REQUIRE(res.converged);
...
TEST_CASE("AMG builds + solves a complex Hermitian SPD system (complex completeness)", "[hesap-amg][complex]")
{
    ...
    crd::jobs::init();
    {
        ...
        for (crd::u32 i = 0; i < n; ++i) { b(i) = C{1.0, 0.25}; x(i) = C{}; }
        IterativeOptions<crd::f64> opts; opts.rel_tol = 1e-8; opts.max_iter = 200;
