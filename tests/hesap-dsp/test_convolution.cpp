...
TEST_CASE("dsp conv: direct convolve == fftconvolve == scipy", "[v11-j][dsp][conv]")
{
    ...
    const usize na = sizeof(ref_conv_a)/sizeof(double), nb = sizeof(ref_conv_b)/sizeof(double);
...
{
    crd::memory::TlsfAllocator alloc(1U << 22);
    const usize na = sizeof(ref_oa_a)/sizeof(double), nb = sizeof(ref_oa_b)/sizeof(double);
    ...
    const usize ns = sizeof(ref_deconv_sig)/sizeof(double), nd = sizeof(ref_deconv_div)/sizeof(double);
