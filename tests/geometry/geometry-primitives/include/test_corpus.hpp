...
inline constexpr f32 kNan = std::numeric_limits<f32>::quiet_NaN();
inline constexpr f32 kInf = std::numeric_limits<f32>::infinity();
...
[[nodiscard]] inline crd::containers::Array<AABB3<f32>> degenerate_aabbs()
{
    return {
        ...
        AABB3<f32>(Vec3<f32>(+kInf, +kInf, +kInf), Vec3<f32>(-kInf, -kInf, -kInf)),
        // NaN min — never-hit semantics.
        AABB3<f32>(Vec3<f32>(kNan, 0, 0), Vec3<f32>(1, 1, 1)),
        // NaN max.
        AABB3<f32>(Vec3<f32>(0, 0, 0), Vec3<f32>(1, 1, kNan)),
        // Half-infinite — slab needs to handle ∞ in one axis without poisoning.
        AABB3<f32>(Vec3<f32>(-kInf, 0, 0), Vec3<f32>(1, 1, 1)),
        AABB3<f32>(Vec3<f32>(0, 0, 0), Vec3<f32>(+kInf, 1, 1)),
    };
}
...
[[nodiscard]] inline crd::containers::Array<Triangle3<f32>> degenerate_triangles()
{
    return {
        ...
        // NaN vertex.
        Triangle3<f32>(Vec3<f32>(kNan, 0, 0), Vec3<f32>(1, 0, 0), Vec3<f32>(0, 1, 0)),
        // ∞ vertex.
        Triangle3<f32>(Vec3<f32>(+kInf, 0, 0), Vec3<f32>(1, 0, 0), Vec3<f32>(0, 1, 0)),
    };
}
...
[[nodiscard]] inline crd::containers::Array<Sphere<f32>> degenerate_spheres()
{
    return {
        ...
        Sphere<f32>(Vec3<f32>(kNan, 0, 0), 1.0F),              // NaN center
        Sphere<f32>(Vec3<f32>(0, 0, 0), kNan),                 // NaN radius
        Sphere<f32>(Vec3<f32>(0, 0, 0), +kInf),                // ∞ radius
        Sphere<f32>(Vec3<f32>(+kInf, 0, 0), 1.0F),             // ∞ center
    };
}
...
[[nodiscard]] inline crd::containers::Array<Ray3<f32>> degenerate_rays()
{
    return {
        ...
        Ray3<f32>(Vec3<f32>(kNan, 0, 0), Vec3<f32>(1, 0, 0)),       // NaN origin
        Ray3<f32>(Vec3<f32>(0, 0, 0), Vec3<f32>(kNan, 0, 0)),       // NaN direction
        Ray3<f32>(Vec3<f32>(0, 0, 0), Vec3<f32>(+kInf, 0, 0)),      // ∞ direction
        Ray3<f32>(Vec3<f32>(+kInf, 0, 0), Vec3<f32>(1, 0, 0)),      // ∞ origin
    };
}
...
inline constexpr f32 kFarOriginModest = 1.0e6F;  // safe — every f32 op stays in the integer-exact regime for ints up to ~16M
inline constexpr f32 kFarOriginStress = 1.0e7F;  // stress — single-precision ULP at this magnitude is ~1.0
