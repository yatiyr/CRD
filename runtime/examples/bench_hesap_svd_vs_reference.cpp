...
extern "C" void dgesdd_(const char* jobz, const int* m, const int* n, double* a, const int* lda, double* s, double* u,
                        const int* ldu, double* vt, const int* ldvt, double* work, const int* lwork, int* iwork,
                        int* info);
...
int main()
{
    ...
    std::fprintf(stdout, "%-6s | %-9s | %-9s | %-9s | %-9s | %-9s | %-7s | %-7s | %-7s | %-7s | %-9s | %-9s\n", "N",
                 "Cerid", "Jacobi", "BDC", "dgesvd", "dgesdd", "C/Jac", "C/BDC", "C/svd", "C/sdd", "val|err|", "recon");
    ...
    for (crd::usize n : {crd::usize{128}, crd::usize{256}, crd::usize{512}, crd::usize{1024}})
    {
        ...
        crd::f64 svdt = 0.0;
        {
            ...
            int lwork = -1;
            crd::f64 wq = 0.0;
            dgesvd_("S", "S", &ni, &ni, la.data(), &ni, ls.data(), lu.data(), &ni, lvt.data(), &ni, &wq, &lwork, &info);
            ...
            svdt = time_loop(
                [&]()
                {
                    ...
                    dgesvd_("S", "S", &ni, &ni, la.data(), &ni, ls.data(), lu.data(), &ni, lvt.data(), &ni, work.data(),
                            &lwk, &inf);
                });
        }
        crd::f64 sddt = 0.0;
        ...
        std::fprintf(stdout,
                     ...
                     "%-9.2e | %-9.2e\n",
                     static_cast<size_t>(n), ct * 1e3, jt * 1e3, bt * 1e3, svdt * 1e3, sddt * 1e3, jt / ct, bt / ct,
                     svdt / ct, sddt / ct, verr, recon);
    }
    ...
    std::fprintf(stdout, "%-6s | %-9s | %-9s | %-9s | %-9s | %-9s | %-7s | %-7s | %-7s | %-7s | %-9s\n", "N", "Cerid",
                 "Jacobi", "BDC", "dgesvd", "dgesdd", "C/Jac", "C/BDC", "C/svd", "C/sdd", "val|err|");
    std::fprintf(stdout, "%s\n",
                 "----------------------------------------------------------------------------------------"
                 "--------------------");
    for (crd::usize n : {crd::usize{64}, crd::usize{128}, crd::usize{256}, crd::usize{512}, crd::usize{1024}})
    {
        ...
        crd::f64 svdt = 0.0;
        {
            ...
            int lwork = -1;
            crd::f64 wq = 0.0;
            dgesvd_("N", "N", &ni, &ni, la.data(), &ni, ls.data(), nullptr, &ni, nullptr, &ni, &wq, &lwork, &info);
            lwork = static_cast<int>(wq);
            crd::containers::Array<crd::f64> work(&alloc);
            work.resize(static_cast<crd::usize>(lwork));
            svdt = time_loop(
        ...
        crd::f64 sddt = 0.0;
        {
            ...
            int lwork = -1;
            crd::f64 wq = 0.0;
            dgesdd_("N", &ni, &ni, la.data(), &ni, ls.data(), nullptr, &ni, nullptr, &ni, &wq, &lwork, iwork.data(),
                    &info);
            lwork = static_cast<int>(wq);
            crd::containers::Array<crd::f64> work(&alloc);
            work.resize(static_cast<crd::usize>(lwork));
            sddt = time_loop(
                [&]()
                {
                    ...
                    int inf = 0;
                    int lwk = static_cast<int>(work.size());
                    dgesdd_("N", &ni, &ni, la.data(), &ni, ls.data(), nullptr, &ni, nullptr, &ni, work.data(), &lwk,
                            iwork.data(), &inf);
                });
        }
