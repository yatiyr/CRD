...
template <MathScalar T> struct EpaVertex
{
    ...
    crd::u32 vidx_a{kInvalidVertex};
    crd::u32 vidx_b{kInvalidVertex};
};
...
inline constexpr crd::usize kEpaMaxVertices = 64;
inline constexpr crd::usize kEpaMaxFaces = 128;

template <MathScalar T> struct EpaPolytope
{
    EpaVertex<T> verts[kEpaMaxVertices]{};
    crd::usize vert_count = 0;
    EpaFace<T> faces[kEpaMaxFaces]{};
    ...
    {
        if (vert_count >= kEpaMaxVertices)
        {
            failed_overflow = true;
            return kEpaMaxVertices;
        }
        verts[vert_count] = v;
        return vert_count++;
    }
    ...
    [[nodiscard]] bool vertex_already_present(crd::u32 vidx_a_q, crd::u32 vidx_b_q) const noexcept
    {
        ...
        if (vidx_a_q == kInvalidVertex || vidx_b_q == kInvalidVertex)
        {
            return false;
        }
    ...
    [[nodiscard]] bool expand_silhouette(crd::u8 new_v_idx) noexcept
    {
        ...
        constexpr crd::usize k_max_silhouette = 3 * kEpaMaxFaces; // upper bound
