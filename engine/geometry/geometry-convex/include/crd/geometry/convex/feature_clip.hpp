...
//     existing `tests/geometry/geometry-convex/test_hill_climb.cpp` convention —
...
inline constexpr crd::u8 kInvalidFaceU8 = static_cast<crd::u8>(0xFFU);
inline constexpr crd::u32 kInvalidFaceU32 = ~crd::u32{0};
...
template <MathScalar T> struct ObbFaceFeature
{
    ...
    crd::u8 face_index = kInvalidFaceU8;
};
...
template <MathScalar T> struct HullFaceFeature
{
    ...
    crd::u32 face_index = kInvalidFaceU32;
};
...
struct EdgeFeature
{
    ...
    crd::u32 face_a = kInvalidFaceU32;
    crd::u32 face_b = kInvalidFaceU32;
};
...
inline constexpr crd::u32 kObbFaceCornerTable[6][4] = {
    {4U, 5U, 7U, 6U}, // 0: +X
    {0U, 2U, 3U, 1U}, // 1: -X
    {2U, 6U, 7U, 3U}, // 2: +Y
    {0U, 1U, 5U, 4U}, // 3: -Y
...
inline constexpr crd::u32 kObbEdgeTable[12][4] = {
...
enumerate_faces(const OBB3<T>& obb) noexcept
{
    ...
    for (crd::u8 f = 0; f < 6U; ++f)
    {
        ...
        {
            face.vertices[k] = feature_detail::obb_corner(obb, feature_detail::kObbFaceCornerTable[f][k]);
        }
    }
    return out;
}
...
[[nodiscard]] inline crd::containers::StaticArray<EdgeFeature, 12> enumerate_edges_obb() noexcept
{
    ...
    {
        out[e].v0 = feature_detail::kObbEdgeTable[e][0];
        out[e].v1 = feature_detail::kObbEdgeTable[e][1];
        out[e].face_a = feature_detail::kObbEdgeTable[e][2];
        out[e].face_b = feature_detail::kObbEdgeTable[e][3];
    }
    return out;
}
...
inline void enumerate_edges(const ConvexHullView<T>& hull, crd::containers::Array<EdgeFeature>& out) noexcept
{
    ...
    {
        if (directed[i].face == kInvalidFaceU32)
        {
        ...
        for (crd::usize j = 0U; j < dn; ++j)
        {
            ...
            if (directed[j].face == kInvalidFaceU32)
            {
                continue;
            }
        ...
        {
            directed[i].face = kInvalidFaceU32;
            continue;
        }
        ...
        out.push_back(ef);
        directed[i].face = kInvalidFaceU32;
        directed[match].face = kInvalidFaceU32;
    }
}
