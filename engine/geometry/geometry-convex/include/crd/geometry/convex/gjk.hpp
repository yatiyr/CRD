...
template <MathScalar T> struct GjkSimplex
{
    ...
    crd::u32 vidx_a[4]{kInvalidVertex, kInvalidVertex, kInvalidVertex, kInvalidVertex};
    crd::u32 vidx_b[4]{kInvalidVertex, kInvalidVertex, kInvalidVertex, kInvalidVertex};
    crd::u8 size = 0;
};
                                               ...
                                               const Transform<T>& xform_b) noexcept
{
    ...
    crd::u32 last_vidx_a = kInvalidVertex;
    crd::u32 last_vidx_b = kInvalidVertex;
    ...
    for (crd::u8 iter = 0; iter < k_max_iter; ++iter)
    {
        ...
        if (sa.vertex_idx != kInvalidVertex && sb_b.vertex_idx != kInvalidVertex)
        {
