                         ...
                         crd::f64 detsum) noexcept
{
    ...
    crd::f64 errbound = kCcwerrboundB * detsum;
    ...
    errbound = kCcwerrboundC * detsum + predicate_detail::kEpsilon * std::fabs(det);
                         ...
                         const Vec3<crd::f64>& d, crd::f64 permanent) noexcept
{
    ...
    const crd::f64 errbound = kO3derrboundB * permanent;
    if (det >= errbound || -det >= errbound)
    {
        return det;
    }
    ...
    const crd::f64 errbound_c = kO3derrboundC * permanent + predicate_detail::kEpsilon * std::fabs(det);
    det +=
                         ...
                         const Vec2<crd::f64>& d, crd::f64 permanent) noexcept
{
    ...
    const crd::f64 errbound = kIccerrboundB * permanent;
    if (det >= errbound || -det >= errbound)
    {
        return det;
    }
                         ...
                         const Vec3<crd::f64>& d, const Vec3<crd::f64>& e, crd::f64 permanent) noexcept
{
    ...
    const crd::f64 errbound = kIsperrboundB * permanent;
    if (det >= errbound || -det >= errbound)
    {
        return det;
    }
