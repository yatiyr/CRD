                                                 ...
                                                 T epsilon = crd::math::default_epsilon<T>()) noexcept
{
    ...
    {
        safe_det = det < -det_floor ? det : -det_floor;
    }
    else
    {
        safe_det = det > det_floor ? det : det_floor;
...
inline constexpr crd::u32 kInvalidVertex = ~crd::u32{0};
...
{
    Vec3<T> point{};
    crd::u32 vertex_idx = kInvalidVertex;
};
...
// `engine/geometry/geometry-primitives/src/hull_support_simd.cpp` so the AVX2 `Vec8f`
...
inline constexpr crd::usize kSimdSupportThreshold = 32;
                                                       ...
                                                       crd::u32 hint_vertex_idx) noexcept
{
    ...
    {
        if (have_soa && hull.vertices.size() <= kSimdSupportThreshold)
        {
