...
// `engine/geometry/geometry-primitives/src/predicates.cpp` — large stack arrays for
...
inline constexpr crd::f64 kEpsilon = 1.1102230246251565e-16; // 2^-53

// Stage-A relative-error bounds, per Shewchuk Table 1.
inline constexpr crd::f64 kCcwerrboundA = (3.0 + 16.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kCcwerrboundB = (2.0 + 12.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kCcwerrboundC = (9.0 + 64.0 * kEpsilon) * kEpsilon * kEpsilon;

inline constexpr crd::f64 kO3derrboundA = (7.0 + 56.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kO3derrboundB = (3.0 + 28.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kO3derrboundC = (26.0 + 288.0 * kEpsilon) * kEpsilon * kEpsilon;

inline constexpr crd::f64 kIccerrboundA = (10.0 + 96.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kIccerrboundB = (4.0 + 48.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kIccerrboundC = (44.0 + 576.0 * kEpsilon) * kEpsilon * kEpsilon;

inline constexpr crd::f64 kIsperrboundA = (16.0 + 224.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kIsperrboundB = (5.0 + 72.0 * kEpsilon) * kEpsilon;
inline constexpr crd::f64 kIsperrboundC = (71.0 + 1408.0 * kEpsilon) * kEpsilon * kEpsilon;
                                        ...
                                        const Vec2<crd::f64>& c) noexcept
{
    ...
    const crd::f64 errbound = predicate_detail::kCcwerrboundA * detsum;
                                        ...
                                        const Vec3<crd::f64>& c, const Vec3<crd::f64>& d) noexcept
{
    ...
    const crd::f64 errbound = predicate_detail::kO3derrboundA * permanent;
    if (det > errbound || -det > errbound)
    {
        return det; // Stage A reliable
    }
                                        ...
                                        const Vec2<crd::f64>& c, const Vec2<crd::f64>& d) noexcept
{
    ...
    const crd::f64 errbound = predicate_detail::kIccerrboundA * permanent;
    if (det > errbound || -det > errbound)
    {
        return det;
    }
                                        ...
                                        const Vec3<crd::f64>& e) noexcept
{
    ...
    const crd::f64 errbound = predicate_detail::kIsperrboundA * permanent;
    if (det > errbound || -det > errbound)
    {
        return det;
    }
