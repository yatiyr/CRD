...
template <typename T> void fill_builder_real(TripletBuilder<T>& b, const TripletInputs& in)
{
...
template <typename U> void fill_builder_complex(TripletBuilder<Complex<U>>& b, const TripletInputs& in)
{
    for (crd::usize k = 0; k < in.tr.size(); ++k)
    {
...
template <typename T> CommandResult impl_from_triplets_real(const CommandArgs& args)
{
    TripletInputs in;
...
template <typename U> CommandResult impl_from_triplets_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
...
template <typename T> CommandResult impl_to_csr_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_csr_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_build_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_build_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_to_csc_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_csc_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
CommandResult impl_structural_query(const CommandArgs& args)
{
    ...
    const crd::f64 packed[8] = {static_cast<crd::f64>(s.rows),          static_cast<crd::f64>(s.cols),
                                static_cast<crd::f64>(s.nnz),           s.density,
                                static_cast<crd::f64>(s.n_outer),       static_cast<crd::f64>(s.min_inner_nnz),
                                static_cast<crd::f64>(s.max_inner_nnz), s.is_compressed ? 1.0 : 0.0};
    return blob_result(args.alloc, crd::containers::ConstSpan<crd::f64>{packed, 8});
}

// ---- spmv : y = alpha*op(A)*x + beta*y -> returns y (f64 blob) ----------

template <typename T, Trans Tr> CommandResult impl_spmv_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U, Trans Tr> CommandResult impl_spmv_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
    ...
    const Complex<U> alpha =
        af.size() >= 2 ? Complex<U>{static_cast<U>(af[0]), static_cast<U>(af[1])} : Complex<U>{U(1), U(0)};
    const Complex<U> beta =
        bf.size() >= 2 ? Complex<U>{static_cast<U>(bf[0]), static_cast<U>(bf[1])} : Complex<U>{U(0), U(0)};
...
template <typename T> CommandResult impl_spmv_sell_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_spmv_sell_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_transpose_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_transpose_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_to_coo_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_coo_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_scale_mat_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename T, int Op> CommandResult impl_binary_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_scale_mat_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename U, int Op> CommandResult impl_binary_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_diag_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_diag_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T, bool Upper> CommandResult impl_tri_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U, bool Upper> CommandResult impl_tri_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename U> bool read_b_complex(const CommandArgs& args, TripletBuilder<Complex<U>>& b, const char*& err)
{
    const auto br = args.get_i64_array("b_triplet_rows");
    const auto bc = args.get_i64_array("b_triplet_cols");
    const auto bv = args.get_f64_array("b_values");
...
template <typename T> CommandResult impl_spgemm_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_spgemm_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_spgemm_ata_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_spgemm_ata_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_spmm_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_spmm_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_sddmm_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_sddmm_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_to_bsr_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_bsr_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_from_bsr_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_from_bsr_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_bsr_spmv_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_bsr_spmv_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_to_ell_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_ell_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_from_ell_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_from_ell_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_ell_spmv_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename T> CommandResult impl_to_dia_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_dia_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_from_dia_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_from_dia_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_dia_spmv_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_ell_spmv_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename U> CommandResult impl_dia_spmv_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_mtx_read_real(const CommandArgs& args)
{
...
template <typename U> CommandResult impl_mtx_read_complex(const CommandArgs& args)
{
    MatrixMarketError err(args.alloc);
...
template <typename T> CommandResult impl_mtx_write_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_mtx_write_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_from_csc_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_from_csc_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_scale_rows_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_scale_rows_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_submatrix_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
    ...
    fill_builder_real<T>(tb, in);
    return scalar_result(args.alloc,
                         static_cast<crd::f64>(submatrix<T>(tb.compress(), r0, r1, c0, c1, args.alloc).nnz()));
}

template <typename U> CommandResult impl_submatrix_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
template <typename T> CommandResult impl_to_sell_real(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/false, in, err))
...
template <typename U> CommandResult impl_to_sell_complex(const CommandArgs& args)
{
    TripletInputs in;
    const char* err = nullptr;
    if (!read_inputs(args, /*complex=*/true, in, err))
...
CRD_HESAP_CLI_REGISTER_MODULE(
    [](CommandRegistry& reg)
    {
        ...
        // from_triplets.<T> -> nnz
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.from_triplets.f32",
                                                 "Assemble COO triplets into CSR (f32); returns nnz.",
                                                 OutputKind::Scalar, false),
                             &impl_from_triplets_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.from_triplets.f64",
                                                 "Assemble COO triplets into CSR (f64); returns nnz.",
                                                 OutputKind::Scalar, false),
                             &impl_from_triplets_real<crd::f64>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.from_triplets.c32",
                                                 "Assemble COO triplets into CSR (Complex<f32>); returns nnz.",
                                                 OutputKind::Scalar, false),
                             &impl_from_triplets_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.from_triplets.c64",
                                                 "Assemble COO triplets into CSR (Complex<f64>); returns nnz.",
                                                 OutputKind::Scalar, false),
                             &impl_from_triplets_complex<crd::f64>);

        // to_csr.<T> -> compressed value array (f64 blob)
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csr.f32",
                                                 "Assemble triplets, return CSR values (f32 blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csr_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csr.f64",
                                                 "Assemble triplets, return CSR values (f64 blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csr_real<crd::f64>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csr.c32",
                                                 "Assemble triplets, return CSR values (Complex<f32> flattened blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csr_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csr.c64",
        ...
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.build.c32",
                                                 "Build CSR from triplets (Complex<f32>); returns nnz.",
                                                 OutputKind::Scalar, true),
                             &impl_build_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.build.c64",
                                                 "Build CSR from triplets (Complex<f64>); returns nnz.",
                                                 OutputKind::Scalar, true),
                             &impl_build_complex<crd::f64>);

        // to_csc.<T> -> compressed CSC value array (f64 blob)
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csc.f32",
                                                 "Assemble triplets, return CSC values (f32 blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csc_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csc.f64",
                                                 "Assemble triplets, return CSC values (f64 blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csc_real<crd::f64>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csc.c32",
                                                 "Assemble triplets, return CSC values (Complex<f32> flattened blob).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_csc_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_csc.c64",
        ...
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.nnz",
                                                 "Number of stored entries after dedup; returns nnz.",
                                                 OutputKind::Scalar, false),
                             &impl_nnz);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.density",
                                                 "Fraction of dense cells stored; returns density.", OutputKind::Scalar,
                                                 false),
                             &impl_density);
        reg.register_command(
            make_triplet_schema(
                alloc, "hesap.sparse.structural_query",
        ...
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.spmv.c32", "y = alpha*A*x + beta*y (CSR, Complex<f32>)."),
            &impl_spmv_complex<crd::f32, Trans::None>);
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.spmv.c64", "y = alpha*A*x + beta*y (CSR, Complex<f64>)."),
        ...
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.spmv_sell.f32", "y = alpha*A*x + beta*y via SELL (f32)."),
            &impl_spmv_sell_real<crd::f32>);
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.spmv_sell.f64", "y = alpha*A*x + beta*y via SELL (f64)."),
        ...
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.transpose.f32",
                                                 "Transpose A; returns A^T values (f32).", OutputKind::BinaryBlob,
                                                 false),
                             &impl_transpose_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.transpose.f64",
                                                 "Transpose A; returns A^T values (f64).", OutputKind::BinaryBlob,
                                                 false),
        ...
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_coo.f32",
                                                 "CSR->COO; returns [row,col,val] triples (f32).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_coo_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_coo.f64",
                                                 "CSR->COO; returns [row,col,val] triples (f64).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_coo_real<crd::f64>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_coo.c32",
                                                 "CSR->COO; returns [row,col,re,im] (Complex<f32>).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_coo_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_coo.c64",
                                                 "CSR->COO; returns [row,col,re,im] (Complex<f64>).",
                                                 OutputKind::BinaryBlob, false),
                             &impl_to_coo_complex<crd::f64>);

        // scale.<T> : alpha*A -> scaled values blob.
        reg.register_command(
            make_scale_schema(alloc, "hesap.sparse.scale.f32", "alpha*A; returns scaled values (f32)."),
            &impl_scale_mat_real<crd::f32>);
        reg.register_command(
            make_scale_schema(alloc, "hesap.sparse.scale.f64", "alpha*A; returns scaled values (f64)."),
            &impl_scale_mat_real<crd::f64>);
        reg.register_command(
            make_scale_schema(alloc, "hesap.sparse.scale.c32", "alpha*A; returns scaled values (Complex<f32>)."),
            &impl_scale_mat_complex<crd::f32>);
        reg.register_command(
            make_scale_schema(alloc, "hesap.sparse.scale.c64", "alpha*A; returns scaled values (Complex<f64>)."),
        ...
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.add.c32", "C = A + B; returns nnz (Complex<f32>)."),
            &impl_binary_complex<crd::f32, 0>);
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.add.c64", "C = A + B; returns nnz (Complex<f64>)."),
        ...
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.sub.c32", "C = A - B; returns nnz (Complex<f32>)."),
            &impl_binary_complex<crd::f32, 1>);
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.sub.c64", "C = A - B; returns nnz (Complex<f64>)."),
        ...
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.hadamard.c32", "C = A .* B; returns nnz (Complex<f32>)."),
            &impl_binary_complex<crd::f32, 2>);
        reg.register_command(
            make_binary_schema(alloc, "hesap.sparse.hadamard.c64", "C = A .* B; returns nnz (Complex<f64>)."),
        ...
        auto spmm_schema = [&](const char* name, const char* desc)
        {
            ...
            add_param(s, alloc, "beta", "Scalar on prior C (complex flattened [re,im]; default 0)", ParamKind::F64,
                      false);
            return s;
        };
        ...
        auto sddmm_schema = [&](const char* name, const char* desc)
        {
            ...
            add_param(s, alloc, "alpha", "Scalar on each sampled dot (complex flattened [re,im]; default 1)",
                      ParamKind::F64, false);
            return s;
        };
        ...
        {
            CommandSchema s = make_triplet_schema(alloc, name, desc, OutputKind::Scalar, false);
            add_param(s, alloc, "b", "Block size (square b x b blocks; rows/cols must be multiples of b)",
                      ParamKind::U64, true);
            return s;
        };
        ...
        reg.register_command(
            bsr_conv_schema("hesap.sparse.to_bsr.c32", "CSR -> BSR; returns nnz_blocks (Complex<f32>)."),
            &impl_to_bsr_complex<crd::f32>);
        reg.register_command(
            bsr_conv_schema("hesap.sparse.to_bsr.c64", "CSR -> BSR; returns nnz_blocks (Complex<f64>)."),
        ...
        reg.register_command(
            bsr_conv_schema("hesap.sparse.from_bsr.c32", "CSR->BSR->CSR round-trip nnz (Complex<f32>)."),
            &impl_from_bsr_complex<crd::f32>);
        reg.register_command(
            bsr_conv_schema("hesap.sparse.from_bsr.c64", "CSR->BSR->CSR round-trip nnz (Complex<f64>)."),
        ...
        reg.register_command(
            bsr_spmv_schema("hesap.sparse.bsr_spmv.c32", "y = alpha*A*x + beta*y via BSR (Complex<f32>)."),
            &impl_bsr_spmv_complex<crd::f32>);
        reg.register_command(
            bsr_spmv_schema("hesap.sparse.bsr_spmv.c64", "y = alpha*A*x + beta*y via BSR (Complex<f64>)."),
            &impl_bsr_spmv_complex<crd::f64>);

        // to_ell.<T> (-> max_row_len) / from_ell.<T> (-> round-trip CSR nnz).
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_ell.f32",
                                                 "CSR -> ELL; returns max_row_len (f32).", OutputKind::Scalar, false),
                             &impl_to_ell_real<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_ell.f64",
                                                 "CSR -> ELL; returns max_row_len (f64).", OutputKind::Scalar, false),
        ...
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.ell_spmv.f32", "y = alpha*A*x + beta*y via ELL (f32)."),
            &impl_ell_spmv_real<crd::f32>);
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.ell_spmv.f64", "y = alpha*A*x + beta*y via ELL (f64)."),
        ...
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_dia.c32",
                                                 "CSR -> DIA; returns ndiag (Complex<f32>).", OutputKind::Scalar,
                                                 false),
                             &impl_to_dia_complex<crd::f32>);
        reg.register_command(make_triplet_schema(alloc, "hesap.sparse.to_dia.c64",
                                                 "CSR -> DIA; returns ndiag (Complex<f64>).", OutputKind::Scalar,
                                                 false),
        ...
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.dia_spmv.f32", "y = alpha*A*x + beta*y via DIA (f32)."),
            &impl_dia_spmv_real<crd::f32>);
        reg.register_command(
            make_spmv_schema(alloc, "hesap.sparse.dia_spmv.f64", "y = alpha*A*x + beta*y via DIA (f64)."),
        ...
        reg.register_command(
            submatrix_schema("hesap.sparse.submatrix.c32", "A[r0:r1, c0:c1]; returns nnz (Complex<f32>)."),
            &impl_submatrix_complex<crd::f32>);
        reg.register_command(
            submatrix_schema("hesap.sparse.submatrix.c64", "A[r0:r1, c0:c1]; returns nnz (Complex<f64>)."),
        ...
        reg.register_command(
            to_sell_schema("hesap.sparse.to_sell.c32", "CSR -> SELL; returns num_slices (Complex<f32>)."),
            &impl_to_sell_complex<crd::f32>);
        reg.register_command(
            to_sell_schema("hesap.sparse.to_sell.c64", "CSR -> SELL; returns num_slices (Complex<f64>)."),
