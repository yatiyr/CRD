...
static VKAPI_ATTR VkBool32 VKAPI_CALL capture_callback(
    ...
    void* user_data)
{
    ...
    // Whitelist check (whitelisted messages still recorded, just don't count).
    bool whitelisted = false;
    {
        std::lock_guard<std::mutex> lk(impl->records_mu);
        for (const auto& w : impl->whitelisted)
        {
            if (w == msg_id) { whitelisted = true; break; }
        }
    }
...

    {
        ...
        if (impl->records.size() < kMaxRecordedMessages)
        {
            ...
            rec.message_text      = crd::containers::String(
                callback_data->pMessage != nullptr ? callback_data->pMessage : "(null)");
