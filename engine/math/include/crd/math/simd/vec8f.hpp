...
struct alignas(32) Vec8f
{
    ...
    CRD_FORCEINLINE Vec8f(f32 e0, f32 e1, f32 e2, f32 e3,
                          f32 e4, f32 e5, f32 e6, f32 e7) noexcept
    {
#if CRD_SIMD_HAS_AVX2
    ...
    {
#if CRD_SIMD_HAS_AVX2
        Vec8f r; r.v = _mm256_loadu_ps(p); return r;
#else
    ...
    {
#if CRD_SIMD_HAS_AVX2
        Vec8f r; r.v = _mm256_load_ps(p); return r;
#else
    ...
    #define CRD_SIMD_VEC8_BIN_AVX2(OP, INTRIN)                            \
        Vec8f r; r.v = INTRIN(a.v, b.v); return r;
#else
    #define CRD_SIMD_VEC8_BIN_FALLBACK(OP)                                \
        return Vec8f(a.lo OP b.lo, a.hi OP b.hi);
#endif
...
CRD_FORCEINLINE Vec8f operator-(Vec8f a) noexcept { return Vec8f::zero() - a; }
CRD_FORCEINLINE Vec8f operator*(Vec8f a, f32 s) noexcept { return a * Vec8f(s); }
CRD_FORCEINLINE Vec8f operator*(f32 s, Vec8f a) noexcept { return Vec8f(s) * a; }
CRD_FORCEINLINE Vec8f operator/(Vec8f a, f32 s) noexcept { return a / Vec8f(s); }
...
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_min_ps(a.v, b.v); return r;
#else
...
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_max_ps(a.v, b.v); return r;
#else
...
CRD_FORCEINLINE Vec8f abs(Vec8f a) noexcept
{
    ...
    Vec8f r; r.v = _mm256_and_ps(a.v, sign_mask); return r;
#else
    return Vec8f(abs(a.lo), abs(a.hi));
#endif
}

CRD_FORCEINLINE Vec8f clamp(Vec8f a, Vec8f lo, Vec8f hi) noexcept { return min(max(a, lo), hi); }

CRD_FORCEINLINE Vec8f sqrt(Vec8f a) noexcept
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_sqrt_ps(a.v); return r;
#else
...
CRD_FORCEINLINE f32 dot(Vec8f a, Vec8f b) noexcept { return horizontal_sum(a * b); }
...
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_cmp_ps(a.v, b.v, _CMP_LT_OQ); return r;
#else
...
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_cmp_ps(a.v, b.v, _CMP_LE_OQ); return r;
#else
...
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r; r.v = _mm256_cmp_ps(a.v, b.v, _CMP_EQ_OQ); return r;
#else
    return Vec8f(cmp_eq(a.lo, b.lo), cmp_eq(a.hi, b.hi));
#endif
}

CRD_FORCEINLINE Vec8f cmp_gt(Vec8f a, Vec8f b) noexcept { return cmp_lt(b, a); }
CRD_FORCEINLINE Vec8f cmp_ge(Vec8f a, Vec8f b) noexcept { return cmp_le(b, a); }

CRD_FORCEINLINE Vec8f select(Vec8f mask, Vec8f true_v, Vec8f false_v) noexcept
{
#if CRD_SIMD_HAS_AVX2
    Vec8f r;
    r.v = _mm256_or_ps(_mm256_and_ps(mask.v, true_v.v),
                       _mm256_andnot_ps(mask.v, false_v.v));
    return r;
#else
    return Vec8f(select(mask.lo, true_v.lo, false_v.lo),
                 select(mask.hi, true_v.hi, false_v.hi));
#endif
}
