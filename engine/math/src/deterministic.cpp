...
inline constexpr f32 k_4_over_pi = 1.27323954473516F;

// π/4 split into three single-precision parts whose sum rounds to the
// nearest representable π/4. Cephes sinf.c.
inline constexpr f32 k_dp1 = 0.78515625F;
inline constexpr f32 k_dp2 = 2.4187564849853515625e-4F;
inline constexpr f32 k_dp3 = 3.77489497744594108e-8F;

// sin polynomial for |z| ≤ π/4:  sin(z) ≈ z + p2*z^3 + p1*z^5 + p0*z^7
inline constexpr f32 k_sincof_p0 = -1.9515295891E-4F;
inline constexpr f32 k_sincof_p1 =  8.3321608736E-3F;
inline constexpr f32 k_sincof_p2 = -1.6666654611E-1F;

// cos polynomial for |z| ≤ π/4:  cos(z) ≈ 1 - z^2/2 + p2*z^4 + p1*z^6 + p0*z^8
inline constexpr f32 k_coscof_p0 =  2.443315711809948E-005F;
inline constexpr f32 k_coscof_p1 = -1.388731625493765E-003F;
inline constexpr f32 k_coscof_p2 =  4.166664568298827E-002F;

// tan polynomial for |z| ≤ π/4:
inline constexpr f32 k_tancof_p0 = 9.38540185543E-3F;
inline constexpr f32 k_tancof_p1 = 3.11992232697E-3F;
inline constexpr f32 k_tancof_p2 = 2.44301354525E-2F;
inline constexpr f32 k_tancof_p3 = 5.34112807005E-2F;
inline constexpr f32 k_tancof_p4 = 1.33387994085E-1F;
inline constexpr f32 k_tancof_p5 = 3.33331568548E-1F;
}  // namespace
...
f32 sin(f32 xx) noexcept
{
    ...
    const f32 z  = ((x - j * k_dp1) - j * k_dp2) - j * k_dp3;
    const f32 zz = z * z;
    ...
    if (jq == 1U || jq == 2U)
    {
        ...
        y = ((k_coscof_p0 * zz + k_coscof_p1) * zz + k_coscof_p2) * zz * zz
          - 0.5F * zz + 1.0F;
    }
    else
    {
        // sin polynomial.
        y = ((k_sincof_p0 * zz + k_sincof_p1) * zz + k_sincof_p2) * zz * z + z;
    }
...
f32 tan(f32 xx) noexcept
{
    ...
    if (zz > 1.0e-4F)
    {
        const f32 num = ((((k_tancof_p0 * zz + k_tancof_p1) * zz + k_tancof_p2) * zz
                       + k_tancof_p3) * zz + k_tancof_p4) * zz + k_tancof_p5;
        y = z + z * (zz * num);
    }
...
inline constexpr f32 k_atancof_p0 =  8.05374449538E-2F;
inline constexpr f32 k_atancof_p1 = -1.38776856032E-1F;
inline constexpr f32 k_atancof_p2 =  1.99777106478E-1F;
inline constexpr f32 k_atancof_p3 = -3.33329491539E-1F;

inline constexpr f32 k_tan_3pi_8 = 2.41421356237309504880F;  // tan(3π/8)
inline constexpr f32 k_tan_pi_8  = 0.41421356237309504880F;  // tan(π/8)
}  // namespace
...
f32 atan(f32 xx) noexcept
{
    ...
    const f32 z = x * x;
    y += (((k_atancof_p0 * z + k_atancof_p1) * z + k_atancof_p2) * z
        + k_atancof_p3) * z * x + x;
...
namespace
{
inline constexpr f32 k_log2e = 1.44269504088896341F;   // 1/ln(2)
inline constexpr f32 k_ln2_hi = 0.693359375F;          // high part of ln(2)
inline constexpr f32 k_ln2_lo = -2.12194440e-4F;       // low part of ln(2)

// exp polynomial for |r| ≤ ln(2)/2 ≈ 0.3466
inline constexpr f32 k_expcof_p0 = 1.9875691500E-4F;
inline constexpr f32 k_expcof_p1 = 1.3981999507E-3F;
inline constexpr f32 k_expcof_p2 = 8.3334519073E-3F;
inline constexpr f32 k_expcof_p3 = 4.1665795894E-2F;
inline constexpr f32 k_expcof_p4 = 1.6666665459E-1F;
inline constexpr f32 k_expcof_p5 = 5.0000001201E-1F;

inline constexpr f32 k_exp_max =  88.722832F;   // input above → +inf
inline constexpr f32 k_exp_min = -87.336544F;   // input below → +0
...
f32 exp(f32 x) noexcept
{
    ...
    if (x > k_exp_max) return std::numeric_limits<f32>::infinity();
    if (x < k_exp_min) return 0.0F;

    // k = round(x * log2(e)); r = x - k*ln(2) (using high-precision split)
    const f32 fk = round(x * k_log2e);
    const crd::i32 k = static_cast<crd::i32>(fk);
    const f32 r = (x - fk * k_ln2_hi) - fk * k_ln2_lo;

    // Polynomial approximation of exp(r) - 1 - r:  exp(r) = 1 + r + r*r*P(r)
    const f32 z = r * r;
    const f32 p = ((((k_expcof_p0 * r + k_expcof_p1) * r + k_expcof_p2) * r
                  + k_expcof_p3) * r + k_expcof_p4) * r + k_expcof_p5;
...
inline constexpr f32 k_logcof_p0 =  7.0376836292E-2F;
inline constexpr f32 k_logcof_p1 = -1.1514610310E-1F;
inline constexpr f32 k_logcof_p2 =  1.1676998740E-1F;
inline constexpr f32 k_logcof_p3 = -1.2420140846E-1F;
inline constexpr f32 k_logcof_p4 =  1.4249322787E-1F;
inline constexpr f32 k_logcof_p5 = -1.6668057665E-1F;
inline constexpr f32 k_logcof_p6 =  2.0000714765E-1F;
inline constexpr f32 k_logcof_p7 = -2.4999993993E-1F;
inline constexpr f32 k_logcof_p8 =  3.3333331174E-1F;

// ln(2) split for high-precision compensation in the exponent reconstruction.
inline constexpr f32 k_log_q1 = -2.12194440e-4F;
inline constexpr f32 k_log_q2 =  0.693359375F;
...
f32 log(f32 x) noexcept
{
    ...
    f32 y = ((((((((k_logcof_p0 * m + k_logcof_p1) * m + k_logcof_p2) * m
                  + k_logcof_p3) * m + k_logcof_p4) * m + k_logcof_p5) * m
              + k_logcof_p6) * m + k_logcof_p7) * m + k_logcof_p8) * m * z;
...
namespace
{
inline constexpr f64 k64_4_over_pi = 1.27323954473516268615;

// pi/4 split into three f64 components for high-precision argument reduction.
inline constexpr f64 k64_dp1 = 7.85398125648498535156E-1;
inline constexpr f64 k64_dp2 = 3.77489470793079817668E-8;
inline constexpr f64 k64_dp3 = 2.69515142907905952645E-15;

// sin polynomial degree 6 (zz powers):
//   sin(z) ≈ z + z*z*z * polevl(zz, sincof)
inline constexpr f64 k64_sincof[6] = {
...
inline constexpr f64 k64_coscof[6] = {
...
f64 sin(f64 xx) noexcept
{
    ...
    const f64 z  = ((x - j * k64_dp1) - j * k64_dp2) - j * k64_dp3;
    const f64 zz = z * z;

    f64 y;
    if (jq == 1ULL || jq == 2ULL)
    {
        y = 1.0 - 0.5 * zz + zz * zz * polevl(zz, k64_coscof);
    }
    else
    {
        y = z + z * z * z * polevl(zz, k64_sincof);
    }
...
f64 tan(f64 xx) noexcept
{
    ...
    if (zz > 1.0e-14)
    {
        y = z + z * (zz * polevl(zz, k64_tanp) / p1evl(zz, k64_tanq));
    }
...
inline constexpr f64 k64_atan_p[5] = {
...
inline constexpr f64 k64_atan_q[5] = {
...
inline constexpr f64 k64_tan_3pi_8 = 2.41421356237309504880;  // tan(3π/8)
...
inline constexpr f64 k64_atan_mid  = 0.66;
// High-precision π/2 low part for compensation in the long-x reduction path.
inline constexpr f64 k64_morebits  = 6.123233995736765886130E-17;
}  // namespace
...
f64 atan(f64 xx) noexcept
{
    ...
    {
        y = pi_2_64;
        morebits = k64_morebits;
        x = -1.0 / x;
    }
    else if (x > k64_atan_mid)
    {
        y = pi_4_64;
        morebits = 0.5 * k64_morebits;
        x = (x - 1.0) / (x + 1.0);
    }
    else
    {
    ...
    const f64 z = x * x;
    f64 r = z * polevl(z, k64_atan_p) / p1evl(z, k64_atan_q);
    r = x * r + x + morebits;
    y += r;
...
namespace
{
inline constexpr f64 k64_log2e   = 1.44269504088896340736;
inline constexpr f64 k64_ln2_hi  = 0.693145751953125;             // ln(2) high
inline constexpr f64 k64_ln2_lo  = 1.42860682030941723212e-6;     // ln(2) low

inline constexpr f64 k64_exp_p[3] = {
    1.26177193074810590878E-4,
    3.02994407707441961300E-2,
    9.99999999999999999910E-1
};
inline constexpr f64 k64_exp_q[4] = {
...
inline constexpr f64 k64_exp_max =  709.782712893383996843;
inline constexpr f64 k64_exp_min = -708.396418532264106224;
...
f64 exp(f64 x) noexcept
{
    ...
    if (x > k64_exp_max) return std::numeric_limits<f64>::infinity();
    if (x < k64_exp_min) return 0.0;

    const f64       fk = round(x * k64_log2e);
    const crd::i32  k  = static_cast<crd::i32>(fk);
    f64 r = x;
    r -= fk * k64_ln2_hi;
    r -= fk * k64_ln2_lo;
    ...
    const f64 rr = r * r;
    const f64 px = r * polevl(rr, k64_exp_p);
    const f64 y  = 1.0 + 2.0 * px / (polevl(rr, k64_exp_q) - px);
...
namespace
{
inline constexpr f64 k64_log_p[6] = {
...
inline constexpr f64 k64_log_q[5] = {  // implicit leading 1
...
inline constexpr f64 k64_log_q1 = -2.121944400546905827679e-4;
inline constexpr f64 k64_log_q2 =  0.693359375;
...
f64 log(f64 x) noexcept
{
    ...
    f64 y = m * (z * polevl(m, k64_log_p) / p1evl(m, k64_log_q));
...
inline constexpr f64 k64_erf_t[5] = {
...
inline constexpr f64 k64_erf_u[5] = {
...
inline constexpr f64 k64_erfc_p[9] = {
...
inline constexpr f64 k64_erfc_q[8] = {
...
inline constexpr f64 k64_gamma_p[7] = {
...
inline constexpr f64 k64_gamma_q[8] = {
...
[[nodiscard]] f64 gamma_reduced(f64 x) noexcept
{
    ...
    const f64 t = x - 2.0;
    const f64 num = polevl(t, k64_gamma_p);
    const f64 den = polevl(t, k64_gamma_q);
    return z * num / den;
}
}  // namespace

f64 erfc(f64 x) noexcept;  // forward decl
...
f64 erf(f64 a) noexcept
{
    ...
    const f64 z = a * a;
    return a * polevl(z, k64_erf_t) / p1evl(z, k64_erf_u);
}
...
f64 erfc(f64 a) noexcept
{
    ...
    const f64 ez = exp(zm);
    const f64 num = polevl(ax, k64_erfc_p);
    const f64 den = p1evl(ax, k64_erfc_q);
    f64 y = ez * num / den;
...
inline constexpr f32 kS_4_over_pi   = 1.27323954473516F;
inline constexpr f32 kS_dp1         = 0.78515625F;
inline constexpr f32 kS_dp2         = 2.4187564849853515625e-4F;
inline constexpr f32 kS_dp3         = 3.77489497744594108e-8F;
inline constexpr f32 kS_sincof_p0   = -1.9515295891E-4F;
inline constexpr f32 kS_sincof_p1   =  8.3321608736E-3F;
inline constexpr f32 kS_sincof_p2   = -1.6666654611E-1F;
inline constexpr f32 kS_coscof_p0   =  2.443315711809948E-005F;
inline constexpr f32 kS_coscof_p1   = -1.388731625493765E-003F;
inline constexpr f32 kS_coscof_p2   =  4.166664568298827E-002F;

// Cephes f32 exp polynomial constants.
inline constexpr f32 kS_log2e       = 1.44269504088896341F;
inline constexpr f32 kS_ln2_hi      = 0.693359375F;
inline constexpr f32 kS_ln2_lo      = -2.12194440e-4F;
inline constexpr f32 kS_expcof_p0   = 1.9875691500E-4F;
inline constexpr f32 kS_expcof_p1   = 1.3981999507E-3F;
inline constexpr f32 kS_expcof_p2   = 8.3334519073E-3F;
inline constexpr f32 kS_expcof_p3   = 4.1665795894E-2F;
inline constexpr f32 kS_expcof_p4   = 1.6666665459E-1F;
inline constexpr f32 kS_expcof_p5   = 5.0000001201E-1F;
inline constexpr f32 kS_exp_max     = 88.722832F;
inline constexpr f32 kS_exp_min     = -87.336544F;

// Cephes f32 log polynomial constants.
inline constexpr f32 kS_logcof_p0   =  7.0376836292E-2F;
inline constexpr f32 kS_logcof_p1   = -1.1514610310E-1F;
inline constexpr f32 kS_logcof_p2   =  1.1676998740E-1F;
inline constexpr f32 kS_logcof_p3   = -1.2420140846E-1F;
inline constexpr f32 kS_logcof_p4   =  1.4249322787E-1F;
inline constexpr f32 kS_logcof_p5   = -1.6668057665E-1F;
inline constexpr f32 kS_logcof_p6   =  2.0000714765E-1F;
inline constexpr f32 kS_logcof_p7   = -2.4999993993E-1F;
inline constexpr f32 kS_logcof_p8   =  3.3333331174E-1F;
inline constexpr f32 kS_log_q1      = -2.12194440e-4F;
inline constexpr f32 kS_log_q2      =  0.693359375F;
...
[[nodiscard]] CRD_FORCEINLINE Vec8f simd_sin_v8(Vec8f xx) noexcept
{
    ...
    // octant integer
    Vec8f      j_f = truncate(abs_x * Vec8f(kS_4_over_pi));
    ...
    Vec8f z = abs_x - j_f * Vec8f(kS_dp1);
    z = z - j_f * Vec8f(kS_dp2);
    z = z - j_f * Vec8f(kS_dp3);
    const Vec8f zz = z * z;

    // sin polynomial
    const Vec8f sin_poly = ((Vec8f(kS_sincof_p0) * zz + Vec8f(kS_sincof_p1)) * zz
                           + Vec8f(kS_sincof_p2)) * zz * z + z;
    // cos polynomial
    const Vec8f cos_poly = ((Vec8f(kS_coscof_p0) * zz + Vec8f(kS_coscof_p1)) * zz
                           + Vec8f(kS_coscof_p2)) * zz * zz - Vec8f(0.5F) * zz + Vec8f(1.0F);
...
{
    // Clamp to safe range
    x = min(max(x, Vec8f(kS_exp_min)), Vec8f(kS_exp_max));

    // k = round(x * log2e); r = x - k*ln2_hi - k*ln2_lo
    const Vec8f fk = round_nearest(x * Vec8f(kS_log2e));
    const Vec8i k  = convert_truncate(fk);
    Vec8f r = x - fk * Vec8f(kS_ln2_hi);
    r = r - fk * Vec8f(kS_ln2_lo);

    // exp(r) = 1 + r + r²*P(r)
    const Vec8f rr = r * r;
    Vec8f p = Vec8f(kS_expcof_p0);
    p = p * r + Vec8f(kS_expcof_p1);
    p = p * r + Vec8f(kS_expcof_p2);
    p = p * r + Vec8f(kS_expcof_p3);
    p = p * r + Vec8f(kS_expcof_p4);
    p = p * r + Vec8f(kS_expcof_p5);
...
[[nodiscard]] CRD_FORCEINLINE Vec8f simd_log_v8(Vec8f x) noexcept
{
    ...
    // Polynomial in m
    const Vec8f z = m * m;
    Vec8f p = Vec8f(kS_logcof_p0);
    p = p * m + Vec8f(kS_logcof_p1);
    p = p * m + Vec8f(kS_logcof_p2);
    p = p * m + Vec8f(kS_logcof_p3);
    p = p * m + Vec8f(kS_logcof_p4);
    p = p * m + Vec8f(kS_logcof_p5);
    p = p * m + Vec8f(kS_logcof_p6);
    p = p * m + Vec8f(kS_logcof_p7);
    p = p * m + Vec8f(kS_logcof_p8);
    Vec8f y = p * m * z;

    const Vec8f fe = convert_to_float(exp_int);
    y = y + fe * Vec8f(kS_log_q1);
    y = y - Vec8f(0.5F) * z;
    y = y + m;
    y = y + fe * Vec8f(kS_log_q2);
