...
template <typename T>
SparseMatrix<T, SparseFormat::Csr> build_csr_rect(const CommandArgs& args, crd::u32 m, crd::u32 n)
{
    const auto        tr   = args.get_i64_array("triplet_rows");
    const auto        tc   = args.get_i64_array("triplet_cols");
    const auto        vals = args.get_f64_array("values");
...
template <typename T>
CommandResult impl_block_jacobi(const CommandArgs& args)
{
...
template <typename T>
CommandResult impl_ssor(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows)
...
template <typename T>
CommandResult impl_jacobi(const CommandArgs& args)
{
    const auto rows = args.get_u64("rows");
    if (!rows)
    {
...
template <typename T>
CommandResult impl_ic0(const CommandArgs& args)
{
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "ic0: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "ic0: r has wrong length"); }
    auto                         a = build_csr<T>(args, n);
...
template <typename T>
CommandResult impl_ilu0(const CommandArgs& args)
{
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "ilu0: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "ilu0: r has wrong length"); }
    auto                         a = build_csr<T>(args, n);
...
template <typename T>
CommandResult impl_ilup(const CommandArgs& args)
{
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "ilup: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "ilup: r has wrong length"); }
...
template <typename T>
CommandResult impl_ilut(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "ilut: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "ilut: r has wrong length"); }
...
template <typename T>
SpaiPattern read_spai_pattern(const CommandArgs& args)
{
...
template <typename T>
CommandResult impl_spai(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "spai: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "spai: r has wrong length"); }
...
template <typename T>
CommandResult impl_fspai(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "fspai: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "fspai: r has wrong length"); }
...
template <typename T>
CommandResult impl_chebyshev(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "chebyshev: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "chebyshev: r has wrong length"); }
...
template <typename T>
CommandResult impl_mlilu(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "mlilu: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "mlilu: r has wrong length"); }
    const crd::u32 lfil    = static_cast<crd::u32>(args.get_u64("lfil").value_or(0U));
    const R        droptol = static_cast<R>(args.get_f64("droptol").value_or(-1.0));
    auto                         a = build_csr<T>(args, n);
...
{
    const auto rows = args.get_u64("rows");
    if (!rows) { return error_result(args.alloc, "schwarz: rows (== cols) is required"); }
    const crd::u32   n      = static_cast<crd::u32>(*rows);
    const auto       rin    = args.get_f64_array("r");
    const crd::usize expect = is_complex_v<T> ? static_cast<crd::usize>(n) * 2 : static_cast<crd::usize>(n);
    if (rin.size() != expect) { return error_result(args.alloc, "schwarz: r has wrong length"); }
...
template <typename T>
CommandResult impl_pcg(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
...
template <typename T>
CommandResult impl_block_pcg(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_fgmres(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_bicgstab(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_minres(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_symmlq(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_qmr(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_gcr(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_rminres(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_gcrot(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T>
CommandResult impl_idrs(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
template <typename T, bool IsLsmr>
CommandResult impl_least_squares(const CommandArgs& args)
{
    using R         = RealType<T>;
    const auto rows = args.get_u64("rows");
    const auto cols = args.get_u64("cols");
...
CRD_HESAP_CLI_REGISTER_MODULE([](CommandRegistry& reg) {
    ...
    reg.register_command(make_block_jacobi_schema(alloc, "hesap.precond.block_jacobi.f32", "Apply block-Jacobi M⁻¹r (f32)."),
                         &impl_block_jacobi<crd::f32>);
    reg.register_command(make_block_jacobi_schema(alloc, "hesap.precond.block_jacobi.f64", "Apply block-Jacobi M⁻¹r (f64)."),
                         &impl_block_jacobi<crd::f64>);
    reg.register_command(make_block_jacobi_schema(alloc, "hesap.precond.block_jacobi.c32", "Apply block-Jacobi M⁻¹r (Complex<f32>)."),
                         &impl_block_jacobi<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_block_jacobi_schema(alloc, "hesap.precond.block_jacobi.c64", "Apply block-Jacobi M⁻¹r (Complex<f64>)."),
    ...
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ic0.f64", "Apply IC(0) M⁻¹r (f64; SPD)."),
                         &impl_ic0<crd::f64>);
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ic0.c32", "Apply IC(0) M⁻¹r (Complex<f32>; HPD)."),
                         &impl_ic0<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ic0.c64", "Apply IC(0) M⁻¹r (Complex<f64>; HPD)."),
    ...
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ilu0.f64", "Apply ILU(0) M⁻¹r (f64; general)."),
                         &impl_ilu0<crd::f64>);
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ilu0.c32", "Apply ILU(0) M⁻¹r (Complex<f32>; general)."),
                         &impl_ilu0<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_jacobi_schema(alloc, "hesap.precond.ilu0.c64", "Apply ILU(0) M⁻¹r (Complex<f64>; general)."),
    ...
    reg.register_command(make_ilut_schema(alloc, "hesap.precond.ilut.f64", "Apply ILUT M⁻¹r (f64; lfil/droptol)."),
                         &impl_ilut<crd::f64>);
    reg.register_command(make_ilut_schema(alloc, "hesap.precond.ilut.c32", "Apply ILUT M⁻¹r (Complex<f32>; lfil/droptol)."),
                         &impl_ilut<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_ilut_schema(alloc, "hesap.precond.ilut.c64", "Apply ILUT M⁻¹r (Complex<f64>; lfil/droptol)."),
                         &impl_ilut<crd::hesap::Complex<crd::f64>>);
    reg.register_command(make_ilup_schema(alloc, "hesap.precond.ilup.f32", "Apply ILU(p) M⁻¹r (f32; level-of-fill)."),
                         &impl_ilup<crd::f32>);
    reg.register_command(make_ilup_schema(alloc, "hesap.precond.ilup.f64", "Apply ILU(p) M⁻¹r (f64; level-of-fill)."),
                         &impl_ilup<crd::f64>);
    reg.register_command(make_ilup_schema(alloc, "hesap.precond.ilup.c32", "Apply ILU(p) M⁻¹r (Complex<f32>; level-of-fill)."),
                         &impl_ilup<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_ilup_schema(alloc, "hesap.precond.ilup.c64", "Apply ILU(p) M⁻¹r (Complex<f64>; level-of-fill)."),
                         &impl_ilup<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_spai_schema(alloc, "hesap.precond.spai.f32", "Apply SPAI M≈A⁻¹ z=M·r (f32; general)."),
                         &impl_spai<crd::f32>);
    reg.register_command(make_spai_schema(alloc, "hesap.precond.spai.f64", "Apply SPAI M≈A⁻¹ z=M·r (f64; general)."),
                         &impl_spai<crd::f64>);
    reg.register_command(make_spai_schema(alloc, "hesap.precond.spai.c32", "Apply SPAI M≈A⁻¹ z=M·r (Complex<f32>; general)."),
                         &impl_spai<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_spai_schema(alloc, "hesap.precond.spai.c64", "Apply SPAI M≈A⁻¹ z=M·r (Complex<f64>; general)."),
    ...
    reg.register_command(make_spai_schema(alloc, "hesap.precond.fspai.f64", "Apply FSPAI M=L·Lᴴ z=M·r (f64; SPD)."),
                         &impl_fspai<crd::f64>);
    reg.register_command(make_spai_schema(alloc, "hesap.precond.fspai.c32", "Apply FSPAI M=L·Lᴴ z=M·r (Complex<f32>; HPD)."),
                         &impl_fspai<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_spai_schema(alloc, "hesap.precond.fspai.c64", "Apply FSPAI M=L·Lᴴ z=M·r (Complex<f64>; HPD)."),
                         &impl_fspai<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_chebyshev_schema(alloc, "hesap.precond.chebyshev.f32", "Apply Chebyshev polynomial M⁻¹r (f32; SPD)."),
                         &impl_chebyshev<crd::f32>);
    reg.register_command(make_chebyshev_schema(alloc, "hesap.precond.chebyshev.f64", "Apply Chebyshev polynomial M⁻¹r (f64; SPD)."),
                         &impl_chebyshev<crd::f64>);
    reg.register_command(make_chebyshev_schema(alloc, "hesap.precond.chebyshev.c32", "Apply Chebyshev polynomial M⁻¹r (Complex<f32>; HPD)."),
                         &impl_chebyshev<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_chebyshev_schema(alloc, "hesap.precond.chebyshev.c64", "Apply Chebyshev polynomial M⁻¹r (Complex<f64>; HPD)."),
                         &impl_chebyshev<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_schwarz_schema(alloc, "hesap.precond.schwarz.f32", "Apply Schwarz domain-decomposition M⁻¹r (f32)."),
                         &impl_schwarz<crd::f32>);
    reg.register_command(make_schwarz_schema(alloc, "hesap.precond.schwarz.f64", "Apply Schwarz domain-decomposition M⁻¹r (f64)."),
                         &impl_schwarz<crd::f64>);
    reg.register_command(make_schwarz_schema(alloc, "hesap.precond.schwarz.c32", "Apply Schwarz domain-decomposition M⁻¹r (Complex<f32>)."),
                         &impl_schwarz<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_schwarz_schema(alloc, "hesap.precond.schwarz.c64", "Apply Schwarz domain-decomposition M⁻¹r (Complex<f64>)."),
                         &impl_schwarz<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_ilut_schema(alloc, "hesap.precond.mlilu.f32", "Apply multilevel ILU (MC64+ILUT) M⁻¹r (f32; general)."),
                         &impl_mlilu<crd::f32>);
    reg.register_command(make_ilut_schema(alloc, "hesap.precond.mlilu.f64", "Apply multilevel ILU (MC64+ILUT) M⁻¹r (f64; general)."),
                         &impl_mlilu<crd::f64>);
    reg.register_command(make_ilut_schema(alloc, "hesap.precond.mlilu.c32", "Apply multilevel ILU (MC64+ILUT) M⁻¹r (Complex<f32>)."),
    ...
    reg.register_command(
        ...
        &impl_pcg<crd::f32>);
    reg.register_command(
        make_pcg_schema(alloc, "hesap.iterative.pcg.f64", "Jacobi-preconditioned CG (f64). [iters,resid,converged,x]."),
        &impl_pcg<crd::f64>);
    ...
    reg.register_command(make_block_pcg_schema(alloc, "hesap.iterative.block_pcg.f32",
                                               "Jacobi-preconditioned block-CG multi-RHS (f32). [iters,resid,converged,X(n×s)]."),
                         &impl_block_pcg<crd::f32>);
    reg.register_command(make_block_pcg_schema(alloc, "hesap.iterative.block_pcg.f64",
    ...
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.fgmres.f32",
                                            "(Flexible) restarted GMRES (f32; precond none|jacobi|block_jacobi|ssor)."),
                         &impl_fgmres<crd::f32>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.fgmres.f64",
                                            "(Flexible) restarted GMRES (f64; precond none|jacobi|block_jacobi|ssor)."),
                         &impl_fgmres<crd::f64>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.fgmres.c32",
                                            "(Flexible) restarted GMRES (Complex<f32>)."),
                         &impl_fgmres<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.fgmres.c64",
                                            "(Flexible) restarted GMRES (Complex<f64>)."),
    ...
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.minres.f32",
                                         "MINRES (f32; symmetric/indefinite-OK; precond none|jacobi|block_jacobi|ssor, SPD only)."),
                         &impl_minres<crd::f32>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.minres.f64",
                                         "MINRES (f64; symmetric/indefinite-OK; precond none|jacobi|block_jacobi|ssor, SPD only)."),
                         &impl_minres<crd::f64>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.minres.c32", "MINRES (Complex<f32>; Hermitian/indefinite-OK)."),
                         &impl_minres<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.minres.c64", "MINRES (Complex<f64>; Hermitian/indefinite-OK)."),
                         &impl_minres<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.symmlq.f32",
                                         "SYMMLQ (f32; symmetric/indefinite-OK; precond none|jacobi|block_jacobi|ssor, SPD only)."),
                         &impl_symmlq<crd::f32>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.symmlq.f64",
                                         "SYMMLQ (f64; symmetric/indefinite-OK; precond none|jacobi|block_jacobi|ssor, SPD only)."),
                         &impl_symmlq<crd::f64>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.symmlq.c32", "SYMMLQ (Complex<f32>; Hermitian/indefinite-OK)."),
                         &impl_symmlq<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.symmlq.c64", "SYMMLQ (Complex<f64>; Hermitian/indefinite-OK)."),
                         &impl_symmlq<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.qmr.f32",
                                         "QMR (f32; general nonsymmetric; right precond none|jacobi|block_jacobi|ssor)."),
                         &impl_qmr<crd::f32>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.qmr.f64",
                                         "QMR (f64; general nonsymmetric; right precond none|jacobi|block_jacobi|ssor)."),
                         &impl_qmr<crd::f64>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.qmr.c32", "QMR (Complex<f32>; general non-Hermitian)."),
                         &impl_qmr<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_pcg_schema(alloc, "hesap.iterative.qmr.c64", "QMR (Complex<f64>; general non-Hermitian)."),
                         &impl_qmr<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.gcr.f32",
                                            "GCR(m) (f32; general nonsymmetric; restart + precond none|jacobi|block_jacobi|ssor)."),
                         &impl_gcr<crd::f32>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.gcr.f64",
                                            "GCR(m) (f64; general nonsymmetric; restart + precond none|jacobi|block_jacobi|ssor)."),
                         &impl_gcr<crd::f64>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.gcr.c32", "GCR(m) (Complex<f32>; general non-Hermitian)."),
                         &impl_gcr<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_fgmres_schema(alloc, "hesap.iterative.gcr.c64", "GCR(m) (Complex<f64>; general non-Hermitian)."),
                         &impl_gcr<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_gcrot_schema(alloc, "hesap.iterative.gcrot.f32",
                                           "GCROT(m,k) recycling GMRES (f32; restart=m, recycle=k; precond none|jacobi|block_jacobi|ssor)."),
                         &impl_gcrot<crd::f32>);
    reg.register_command(make_gcrot_schema(alloc, "hesap.iterative.gcrot.f64",
                                           "GCROT(m,k) recycling GMRES (f64; restart=m, recycle=k; precond none|jacobi|block_jacobi|ssor)."),
                         &impl_gcrot<crd::f64>);
    reg.register_command(make_gcrot_schema(alloc, "hesap.iterative.gcrot.c32", "GCROT(m,k) recycling GMRES (Complex<f32>)."),
                         &impl_gcrot<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_gcrot_schema(alloc, "hesap.iterative.gcrot.c64", "GCROT(m,k) recycling GMRES (Complex<f64>)."),
                         &impl_gcrot<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_rminres_schema(alloc, "hesap.iterative.rminres.f32",
                                             "RMINRES recycling MINRES (f32; symmetric/indefinite; restart=m, recycle=k)."),
                         &impl_rminres<crd::f32>);
    reg.register_command(make_rminres_schema(alloc, "hesap.iterative.rminres.f64",
                                             "RMINRES recycling MINRES (f64; symmetric/indefinite; restart=m, recycle=k)."),
                         &impl_rminres<crd::f64>);
    reg.register_command(make_rminres_schema(alloc, "hesap.iterative.rminres.c32", "RMINRES recycling MINRES (Complex<f32>; Hermitian/indefinite)."),
                         &impl_rminres<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_rminres_schema(alloc, "hesap.iterative.rminres.c64", "RMINRES recycling MINRES (Complex<f64>; Hermitian/indefinite)."),
                         &impl_rminres<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_idrs_schema(alloc, "hesap.iterative.idrs.f32",
                                          "IDR(s) (f32; general nonsymmetric; s + precond none|jacobi|block_jacobi|ssor)."),
                         &impl_idrs<crd::f32>);
    reg.register_command(make_idrs_schema(alloc, "hesap.iterative.idrs.f64",
                                          "IDR(s) (f64; general nonsymmetric; s + precond none|jacobi|block_jacobi|ssor)."),
                         &impl_idrs<crd::f64>);
    reg.register_command(make_idrs_schema(alloc, "hesap.iterative.idrs.c32", "IDR(s) (Complex<f32>; general non-Hermitian)."),
                         &impl_idrs<crd::hesap::Complex<crd::f32>>);
    reg.register_command(make_idrs_schema(alloc, "hesap.iterative.idrs.c64", "IDR(s) (Complex<f64>; general non-Hermitian)."),
                         &impl_idrs<crd::hesap::Complex<crd::f64>>);

    reg.register_command(make_least_squares_schema(alloc, "hesap.iterative.lsqr.f32",
                                                   "LSQR least-squares for rectangular A (f32; precond none|jacobi). [iters,resid,converged,x]."),
                         &impl_least_squares<crd::f32, false>);
    reg.register_command(make_least_squares_schema(alloc, "hesap.iterative.lsqr.f64",
    ...
    reg.register_command(make_least_squares_schema(alloc, "hesap.iterative.lsmr.f32",
                                                   "LSMR least-squares for rectangular A (f32; precond none|jacobi). [iters,ArNorm,converged,x]."),
                         &impl_least_squares<crd::f32, true>);
    reg.register_command(make_least_squares_schema(alloc, "hesap.iterative.lsmr.f64",
