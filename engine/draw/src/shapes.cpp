                     ...
                     Color color, PrimFlags flags, crd::f32 lifetime_s)
{
    ...
    constexpr crd::u32 nlong = 16; // matches sphere_wire's `segments_long` default
    constexpr crd::u32 nlat  = 8;  // matches sphere_wire's `segments_lat`  default
    const crd::f32 two_pi = 6.28318530717958647692F;
    const crd::f32 pi     = 3.14159265358979323846F;
    const crd::u32 packed = color.packed_rgba();
    ...
    for (crd::u32 r = 0; r < nlat; ++r)
    {
        const crd::f32 theta_lo = (static_cast<crd::f32>(r)     / static_cast<crd::f32>(nlat)) * pi;
        const crd::f32 theta_hi = (static_cast<crd::f32>(r + 1) / static_cast<crd::f32>(nlat)) * pi;
        for (crd::u32 m = 0; m < nlong; ++m)
        {
            const crd::f32 phi_lo = (static_cast<crd::f32>(m)     / static_cast<crd::f32>(nlong)) * two_pi;
            const crd::f32 phi_hi = (static_cast<crd::f32>(m + 1) / static_cast<crd::f32>(nlong)) * two_pi;
