...
u32 SpatialHash<T>::hash_cell(i32 ix, i32 iy, i32 iz) const noexcept
{
    ...
    const u32 h = (static_cast<u32>(ix) * k_p1)
                ^ (static_cast<u32>(iy) * k_p2)
                ^ (static_cast<u32>(iz) * k_p3);
...
{
    i32 min_x, min_y, min_z, max_x, max_y, max_z;
...
bool SpatialHash<T>::update(SpatialHashObjectId id, const AABB3<T>& new_aabb)
{
    ...
    i32 omin_x, omin_y, omin_z, omax_x, omax_y, omax_z;
    aabb_cell_range(obj.aabb, omin_x, omin_y, omin_z, omax_x, omax_y, omax_z);
    i32 nmin_x, nmin_y, nmin_z, nmax_x, nmax_y, nmax_z;
                                     ...
                                     MarkVisited& mark_visited) const noexcept
{
    ...
    constexpr usize k_max_voxel_steps = 1U << 20; // 1M cells worst case
    usize steps = 0;
