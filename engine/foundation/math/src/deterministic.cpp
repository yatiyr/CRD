...
f64 exp2(f64 x) noexcept { return exp(x * kLn2F64); }
...
f64 log2(f64 x)  noexcept { return log(x) * kInvLn2F64; }
f64 log10(f64 x) noexcept { return log(x) * kInvLn10F64; }
...
f64 gamma(f64 x) noexcept
{
    ...
    return -kPiF64 / (z * gamma_reduced(1.0 - x));
}
