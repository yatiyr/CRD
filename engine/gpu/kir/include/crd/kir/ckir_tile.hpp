...
// ADR-0098 §4 · AS-2/AS-6: one row of the auto-tuned schedule DB — a measured winner for an exact (op, DEVICE, M, N, K). The
// `device` tag (AS-6a: the GPU arch it was tuned on, e.g. "sm_89") makes the DB CROSS-DEVICE: each GPU replays its own winner,
// and an untuned device falls back to the heuristic. The best schedule genuinely differs per device (register file, smem, SM
// count), so a single-device DB would mis-tune everywhere else.
struct TuningEntry
{
    KOp          op;
    const char*  device;
...
// Look up the auto-tuned winner for (op, `device`, M×K·K×N) in the CHECKED-IN DB (generated by `kir_autotune`, compiled in as a
// static table ⇒ deterministic replay, no runtime tuning). `device==nullptr` matches any (wildcard). Returns false ⇒ the caller
// falls back to the hand heuristic, then Naive — which is also what an UNTUNED device gets.
[[nodiscard]] inline bool lookup_tuned(KOp op, const char* device, crd::i64 m, crd::i64 n, crd::i64 k, TileSchedule& out)
{
...
// AS-4 flash-attention: a checked-in tuned (BR,BC) tile keyed by (device, S, D) — the attention analogue of TuningEntry. The
// `time_attention` sweep found the winner per (S,D); this table replays it so `run()` emits the tuned kernel with no runtime search.
struct AttentionTuningEntry
{
    ...
    int         bc; // tuned key-tile width
};

// Look up the tuned flash-attention tile for (`device`, S, D) in the checked-in DB. `device==nullptr` is a wildcard. Returns false
// ⇒ the caller falls back to `select_attention_tile`'s shared-fitting heuristic (also what an untuned device/shape gets).
[[nodiscard]] inline bool lookup_attention_tuned(const char* device, crd::i64 s, crd::i64 d, int& br, int& bc) noexcept
{
    static const AttentionTuningEntry kDb[] = {
#include <crd/kir/ckir_attention_db.inc>
    };
...
// `device` (AS-6a) selects the per-GPU tuned schedule; nullptr = any (the CUDA backend passes its arch, e.g. "sm_89").
inline TileSchedule select_schedule(const KGraph& g, int node, const char* device = nullptr)
{
