...
//
// Measure: ∫ f·cos²θo dθo dφo — identical to the white-furnace gate in tests/gpu/kir/test_ckir_hair.cpp, which is what makes
...
[[nodiscard]] inline KEntry build_hair_scatter_lut_kernel(KGraph& g, const HairScatterLutConfig& cfg)
{
    ...
    const int sin_fixed = g.unary(KOp::Sin, theta_d);
    const int cos_fixed = g.unary(KOp::Cos, theta_d);
    g.stmt_materialize(sin_fixed);
    ...
    const int sin_sample = g.unary(KOp::Sin, th_o);
    const int cos_sample = g.unary(KOp::Cos, th_o);
    g.stmt_materialize(sin_sample);
    g.stmt_materialize(cos_sample);

    // Match the furnace oracle: fixed direction in the first BCSDF slot, integration sample in the second.
    const int f = crd::kir::hair::hair_bcsdf_eval_angles(g, sin_fixed, cos_fixed, ks(0.0), sin_sample, cos_sample, ph_o, hoff, ks(cfg.eta),
    ...
    const int fw = mul(g, f, mul(g, sq(g, cos_sample), ks(wscale))); // f · cos²θo · dθdφ/Nh
...
[[nodiscard]] inline KEntry build_dual_scatter_kernel(KGraph& g, const DualScatterConfig& cfg, int lut_bins)
{
    ...
    const int th_i = ld(1);
    const int th_o = ld(2);
    const int dp_di = ld(3);
    const int dp_io = ld(4);
    const int nst = ld(5);
...
[[nodiscard]] inline KEntry build_dom_build_kernel(KGraph& g, const DomConfig& cfg)
{
    ...
    const auto  cu  = [&](crd::u32 v) { return g.constant(static_cast<double>(v), shu, DType::U32); };
    const auto  ks  = [&](double v) { return g.constant(v, shu, DType::F32); };
    const int layer_count = cfg.layers;
    const int   nf  = cfg.frags_per_px;
    const double dz = cfg.span / static_cast<double>(layer_count);
    ...
    const int  mark = g.kernel_stmt_mark();
    const int  obase = g.binary(KOp::Mul, tid, cu(static_cast<crd::u32>(1 + layer_count)));
    ...
    for (int k = 0; k < layer_count; ++k) // K is a host constant ⇒ unrolled; no nested runtime loop
    {
...
[[nodiscard]] inline KEntry build_dom_lookup_kernel(KGraph& g, const DomConfig& cfg)
{
    ...
    const int  dbase = g.binary(KOp::Mul, px, cu(static_cast<crd::u32>(1 + layer_count)));
...
[[nodiscard]] inline KEntry build_volume_ms_kernel(KGraph& g, const VolumeMsConfig& cfg)
{
    ...
    const int stp = ld(0);
    const int cosrf = ld(1);
    const int alb = ld(2);
    const int phase = ld(3);
    const int dist = ld(4);
    ...
    const double k_iso = 1.0 / (4.0 * kPi); // isotropic phase
    int          acc  = ks(0.0);
    for (int i = 0; i < cfg.octaves; ++i) // N is a host constant ⇒ unrolled; powers fold to literals
    {
        ...
        const int pi   = add(g, mul(g, ks(ci), phase), mul(g, ks(1.0 - ci), ks(k_iso)));
...
[[nodiscard]] inline KEntry build_hair_voxelize_kernel(KGraph& g, int nseg, int gx, int gy, int gz, double cell)
{
    ...
    const int fx = g.cast(vx, DType::F32);
    const int fy = g.cast(vy, DType::F32);
    const int fz = g.cast(vz, DType::F32);
    ...
    const int dx0 = sub(g, bx, ax);
    const int dy0 = sub(g, by, ay);
    const int dz0 = sub(g, bz, az);
