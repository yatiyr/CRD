    ...
    const auto emit_body = [&](auto&& self_b, int begin, int count) -> void {
        ...
        while (i < begin + count) // a For/If body lives CONTIGUOUSLY after it → recurse then SKIP past it (never re-emit)
        {
            ...
            case KStmtKind::ReportHit: case KStmtKind::ExecuteCallable: // REN-38-F13: same RT-only family
                // Inline ray tracing / RT-pipeline statements are GLSL/HLSL-only; Metal has no inline-RT path here, so a kernel
...
// NOTE Metal cannot be compiled off macOS: this emitter is gated STRUCTURALLY in `tests/gpu/kir/test_ckir_msl.cpp`; the
