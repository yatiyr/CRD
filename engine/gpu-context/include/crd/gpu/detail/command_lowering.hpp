...
// RAF-12.4 Phase A: the command-model lowering, templatized. `CommandEncoder<Ctx>` records the backend-neutral
// command model (command_model.hpp) by lowering each RenderingDesc/RasterDrawPacket/DispatchDesc/TransferDesc/
// TraceDesc through `m_ctx`'s verbs. ONE source of truth for BOTH backends (no hand-copied divergence): the base
// factory instantiates `CommandEncoder<IRasterContext>` (virtual dispatch, byte-identical to the pre-RAF-12 path);
// each backend overrides `create_command_encoder()` to instantiate `CommandEncoder<Concrete>` so the verb calls
// resolve STATICALLY to that backend's methods. RAF-12.4 Phase B then de-virtualizes those verbs off the public
// IRasterContext into private backend methods this template calls; nothing here changes when it does.
...
{
inline IStorageBuffer* first_storage(const ResourceBindingTable& bindings) noexcept
{
...
inline const ResourceBinding* find_kind(const ResourceBindingTable& bindings, BindingKind kind) noexcept
{
    const crd::u32 n = static_cast<crd::u32>(bindings.size());
    for (crd::u32 i = 0; i < n; ++i)
    {
...
inline ITexture* shadow_atlas_from(const ResourceBindingTable& bindings) noexcept
{
    const crd::u32 n = static_cast<crd::u32>(bindings.size());
    for (crd::u32 i = 0; i < n; ++i)
    {
...
inline IRasterContext::BlitFilter to_blit_filter(SamplerFilter f) noexcept
{
...
inline ITexture* plain_sampled_texture(const ResourceBindingTable& bindings) noexcept
{
...
inline ITexture* map_texture(const ResourceBindingTable& bindings) noexcept
{
    const crd::u32 n = static_cast<crd::u32>(bindings.size());
    for (crd::u32 i = 0; i < n; ++i)
    {
...
{
public:
    explicit CommandEncoder(Ctx& ctx) noexcept : m_ctx(ctx) {}
