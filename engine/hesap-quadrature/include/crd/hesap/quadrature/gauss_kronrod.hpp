...
template <typename T>
[[nodiscard]] constexpr T qmin(T a, T b) noexcept
{
    return a < b ? a : b;
}
template <typename T>
[[nodiscard]] constexpr T qmax(T a, T b) noexcept
{
    return a > b ? a : b;
}
} // namespace detail
...
[[nodiscard]] GkResult<T> gauss_kronrod_21(F&& f, T a, T b)
{
    ...
    for (int j = 0; j < 5; ++j) // the 5 Gauss pairs: odd-indexed abscissae xgk[1,3,5,7,9]
    {
        ...
        resg += K::wg[j] * fsum;
        resk += K::wgk[idx] * fsum;
        resabs += K::wgk[idx] * (crd::math::fabs(f1) + crd::math::fabs(f2));
    }
    ...
    T       resasc = K::wgk[10] * crd::math::fabs(fc - reskh);
    for (int j = 0; j < 10; ++j)
    {
        resasc += K::wgk[j] * (crd::math::fabs(fv1[j] - reskh) + crd::math::fabs(fv2[j] - reskh));
    }
    GkResult<T> r;
    r.value      = resk * hlgth;
