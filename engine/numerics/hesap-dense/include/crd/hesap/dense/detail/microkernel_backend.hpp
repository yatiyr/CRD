...
// `engine/numerics/hesap-dense/src/asm/<arch>.S` files and flips the default below.
//
...
// These values select #if branches; C++ constexpr objects cannot serve that contract.
#define CRD_HESAP_MICROKERNEL_BACKEND_INTRINSICS 1 // NOLINT(cppcoreguidelines-macro-usage)
#define CRD_HESAP_MICROKERNEL_BACKEND_ASM        2 // NOLINT(cppcoreguidelines-macro-usage)
...
#error "CRD_HESAP_MICROKERNEL_BACKEND must be _INTRINSICS or _ASM"
#endif
...
// symbols in engine/numerics/hesap-dense/src/asm/<arch>.S files and the dispatcher
...
static_assert(false,
    ...
    "landed engine/numerics/hesap-dense/src/asm/<arch>.S files.");
#endif
