...
#include <crd/hesap/special/elliptic.hpp> // canonical ellipk (AGM) + ellipj (consolidated; SANITY rule 8)
...
template <typename T> [[nodiscard]] T ellipk(T m) noexcept
{
    ...
    return crd::hesap::special::ellint_k<T>(m); // canonical AGM (was a local copy of the same algorithm)
}
...
{
    crd::hesap::special::ellipj<T>(u, m, sn, cn, dn); // canonical Jacobi sn/cn/dn (consolidated)
}
...
template <typename T> [[nodiscard]] T ellipdeg(crd::usize N, T m1) noexcept
{
    ...
    const T q1 = crd::math::exp(-pi * K1p / K1);
    const T q = crd::math::pow(q1, T(1) / static_cast<T>(N));
    T num = T(0);
    T den = T(0);
    for (int j = 0; j < 25; ++j) // theta2 numerator: sum q^(j(j+1)); theta3 denom: 1 + 2 sum q^(j^2)
    {
        num += crd::math::pow(q, static_cast<T>(j * (j + 1)));
    }
    for (int j = 1; j < 25; ++j)
    {
        den += crd::math::pow(q, static_cast<T>(j * j));
    }
...
{
    const T r = crd::math::hypot(z.re, z.im);
    const T re = crd::math::sqrt((r + z.re) / T(2));
    T im = crd::math::sqrt((r - z.re) / T(2));
    if (z.im < T(0))
    {
        im = -im;
    }
...
{
    return Complex<T>{crd::math::log(crd::math::hypot(z.re, z.im)), crd::math::atan2(z.im, z.re)};
}
...
template <typename T> [[nodiscard]] T arc_jac_sc(T w, T m) noexcept
{
    ...
    for (int i = 1; i < 40; ++i)
    {
        const T kp = crd::math::sqrt((T(1) - ks[i - 1]) * (T(1) + ks[i - 1]));
        ks[i] = (T(1) - kp) / (T(1) + kp);
        nk = i;
