                                                             ...
                                                             crd::usize noverlap, crd::usize& nseg, crd::usize& nfreq)
{
    ...
    {
        // Generated FFT codelets can spill ~420 KiB in GCC Debug before nested calls.
        // Select the existing 2 MiB tier explicitly; the 64 KiB default cannot host this workload.
                                                     ...
                                                     crd::usize noverlap = 0)
{
    ...
    crd::containers::Array<T> sx(alloc);
    crd::containers::Array<T> sy(alloc);
    crd::containers::Array<Complex<T>> fx(alloc);
    crd::containers::Array<Complex<T>> fy(alloc);
    sx.resize(nperseg);
    sy.resize(nperseg);
    fx.resize(nfreq);
    fy.resize(nfreq);
